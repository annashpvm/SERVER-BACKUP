...
var UserId   = localStorage.getItem('ginuserid');
 ...
 var loadAreaGroupListDatastore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task:"loadAreaList2"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
	...
	'area_code','area_name','rate_areacode', 'rate_areaname'
      ]),
    });
...
var txtAreaName = new Ext.form.TextField({
    	...
    	listeners : {
	     ...
	     if (flxArea.store.find('area_name', txtAreaName.getRawValue()) != -1){
	     	flxArea.getStore().filter('area_name', txtAreaName.getRawValue());   
	     }
	     else
	     {
       ...
       //           flxArea.getStore().filter('area_name', txtAreaName.getValue());  
            }
        }
...
var cmbRateArea = new Ext.form.ComboBox({
   ...
   var flxArea = new Ext.grid.EditorGridPanel({
       ...
       id: 'my-grid',  
		...
		{header: "Area Code", dataIndex: 'area_code',sortable:true,width:90,align:'left',hidden : true},   
		{header: "Area Name", dataIndex: 'area_name',sortable:true,width:200,align:'left'},
		{header: "Rate Group", dataIndex: 'rate_areacode',sortable:true,width:50,align:'left',hidden : true},
		{header: "Rate Group", dataIndex: 'rate_areaname',sortable:true,width:330,align:'left'},
    ...
    return 'area_name'
       ...
       listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			txtAreaName.setRawValue(selrow.get('area_name'));
...
//save
				...
				else
				{
					...
					if (btn == 'yes')
						{
						Ext.Ajax.request({
		                            	url: 'MasAreaGroupSave.php',
                		       	        params:
						{
                                                        savetype  : saveflag,
                                                        areacode  : areacode,   
							areaname  : txtAreaName.getRawValue(),
                 					grpcode   : cmbRateArea.getValue(),
