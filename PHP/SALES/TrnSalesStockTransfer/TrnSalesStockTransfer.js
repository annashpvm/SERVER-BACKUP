...
var UserId   = localStorage.getItem('ginuserid');
   ...
   var loadEntryNoDetailsdatastore = new Ext.data.Store({
	...
	'tr_date','varname', 'sizecode', 'stksrno', 'stkwt', 'OldPartyName', 'OldPartyCode', 'oldsono', 'NewPartyName', 'NewPartyCode', 'newsono'
      ]),
    });
		            ...
		            {                        
			            for(var j=0; j<cnt; j++)
		                    { 
		                        ...
		                        new dgrecord({
						varname       : loadgriddetailsDatastore.getAt(j).get('var_name'),
						sizecode      : loadgriddetailsDatastore.getAt(j).get('stk_var_code'),
                                                stksrno       : loadgriddetailsDatastore.getAt(j).get('stk_sr_no'), 
                                                stkwt         : loadgriddetailsDatastore.getAt(j).get('stk_wt'),  
						OldPartyName  : cmbCustomerFrom.getRawValue(),
						OldPartyCode  : cmbCustomerFrom.getValue(),
						oldsono       : cmbSONOFrom.getValue(),
						NewPartyName  : cmbCustomerTo.getRawValue(),
						NewPartyCode  : cmbCustomerTo.getValue(),
                                         	newsono       : cmbSONOTo.getValue(),
		               		})
		    			);
//					grid_tot();
		                   }
...
if (sizechk ==1)
{
            if (Number(cmbCustomerTo.getValue()) == 0 || Number(cmbSONOTo.getValue()) == 0)
            {
                 alert("Select Customer To / SO NUMBER....");
            }             
            else
            {     
		loadgriddetailsDatastore.removeAll();
...
//    font-size:18px,
	...
	{header: "SizeCode",  dataIndex: 'sizecode',sortable:true,width:80,align:'left'},
	...
	{header: "Old Party",dataIndex: 'OldPartyName',sortable:true,width:80,align:'left'},
	{header: "Old Party",dataIndex: 'OldPartyCode',sortable:true,width:80,align:'left'},
	{header: "Old SO No.", dataIndex: 'oldsono',sortable:true,width:80,align:'left'},
	{header: "New Party",dataIndex: 'NewPartyName',sortable:true,width:80,align:'left'},
	{header: "New Party",dataIndex: 'NewPartyCode',sortable:true,width:80,align:'left'},
	{header: "New SO No.", dataIndex: 'newsono',sortable:true,width:80,align:'left'},


    ],
    store: loadEntryNoDetailsdatastore,
    listeners:{	
/*
...
//alert(cmbSONOFrom.getRawValue().length);
                cmbSize.reset();
                flxstartno.getStore().removeAll();
                flxendno.getStore().removeAll();
                loadsizedataStore.removeAll();
...
var cmbCustomerTo = new Ext.form.ComboBox({
        fieldLabel      : 'Customer',
        width           : 300,
        displayField    : 'cust_ref', 
        valueField      : 'cust_code',
	...
	tabIndex	: 0,
       ...
       labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",  
		...
		params: {
		    task: 'loadSOnoTO',
		    customer:cmbCustomerTo.getValue(),
		    fincode: GinFinid,
		    compcode: Gincompcode,
                    userid  : UserId,
		},
	...
	tabIndex	: 0,
        ...
        listeners:{
            select: function () {
                ...
                if (cmbCustomerTo.getValue() != 612)
                {    

                if (cmbSize.getValue() > 0)
                {    
                        sizechk = 0
		        loadItemCheckdataStore.removeAll();
			loadItemCheckdataStore.load({
			url: 'ClsSalesStockTransfer.php',
			params: {
			    task: 'itemcheck_in_so',
			    fincode  : GinFinid,
			    compcode : Gincompcode,
			    sono     : cmbSONOTo.getRawValue(),
		            sizecode : cmbSize.getValue(),
			},
			callback:function()
			{
	                   if (loadItemCheckdataStore.getAt(0).get('nos') == 0)
                           {
                               alert("Size not available in the Destination SO Number...");
                               sizechk = 0;
                           }  
...
//       alert(gstFlag);
                                               Ext.Ajax.request({
				               url: 'TrnSalesStockTransferSave.php',
				               params:
						{
                                                savetype:gstFlag,
	                                        cnt: finData.length,
