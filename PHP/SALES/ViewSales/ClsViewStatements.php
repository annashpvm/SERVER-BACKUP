...
$sql = "select cust_code,cust_ref ,round(sum(invt_wt)/1000,3) as despwt , sum(invt_value) as value1 from 
...
$sql = "select invh_date,invh_invrefno,round(invh_totwt/1000,3) as invwt,invh_netamt  from trnsal_invoice_header where  invh_party = $party and  invh_fincode= $finid  and invh_comp_code= $compcode and invh_date >= '$startdate' and invh_date <= '$enddate' order  by invh_date,invh_invrefno";
    $r = mysqli_query($conn, $sql);
...
//       $r = mysql_query("select cust_code,cust_ref from massal_customer";
        $sql = "select cust_code,cust_ref from massal_customer a, trnsal_invoice_header b  where a.cust_code = b.invh_party  and b.invh_comp_code = 1 and invh_fincode >= 22  group by cust_code,cust_ref order by cust_ref ";
...
$sql = "select UPPER(monthname(invh_date)) as rmonth,  0 as finwt, sum(invt_wt)/1000 as despwt , sum(invt_value) as value1 from trnsal_invoice_header a , trnsal_invoice_trailer b where a.invh_fincode= b.invt_fincode  and a.invh_comp_code = b.invt_compcode and a.invh_seqno = b.invt_seqno and  a.invh_fincode= $finid   and a.invh_comp_code= $compcode  and invh_date >= '$startdate'  and invh_date <=  curdate()  and invh_party = $custcode group by UPPER(monthname(invh_date))";

*/

$sql = "select UPPER(monthname(invh_date)) as rmonth,  0 as finwt, sum(invh_totwt)/1000 as despwt , sum(invh_netamt) as value1 from trnsal_invoice_header where invh_fincode= $finid   and invh_comp_code= $compcode  and invh_date >= '$startdate'  and invh_date <=  curdate()  and invh_party = $custcode group by UPPER(monthname(invh_date))";
...
$sql = "select DATE_FORMAT(invh_date, '%d-%m-%Y') as invdate,invh_date, round(sum(invh_totwt)/1000,3) invwt, sum(invh_netamt) as invamt from trnsal_invoice_header where invh_comp_code = '$compcode'  and invh_fincode = '$finid' and  invh_date >= '$startdate' and invh_date <= '$enddate' group by invh_date order by invh_date desc";
...
$sql = "select cust_ref,invh_invrefno,round(invh_totwt/1000,3) invwt, invh_netamt  from trnsal_invoice_header, massal_customer where invh_party = cust_code and  invh_comp_code = '$compcode'  and invh_fincode = '$finid' and  invh_date = '$repdate' order  by cust_ref , invh_invrefno";
...
$sql = "select var_bf,round(var_gsm,0) var_gsm ,var_size2,round(invt_wt/1000,3) invwt, round(invt_urate,0) invt_urate, concat(var_size2, '-', (case when var_inchcm = 'I' then 'Inch' else 'CM' end),'-',(case when var_shade = 'NS' then 'NAT' when var_shade = 'GY' then 'GYT' when var_shade = 'DP' then 'DP' when var_shade = 'SY' then 'SHYS' when var_shade = 'GB' then 'GB'   when var_shade = 'BB' then 'BB'   else 'OTH' end)) as sizedisp  from trnsal_invoice_header, trnsal_invoice_trailer , massal_variety ,masprd_variety  where  invh_comp_code = invt_compcode and invh_fincode = invt_fincode and invt_var = var_code and var_grpcode = var_groupcode and  invh_comp_code = $compcode and invh_fincode = $finid  and invt_seqno = invh_seqno and  invh_invrefno = '$invno'";
 ...
 function getShades()
    {
        global $conn;  
        $sql = "select var_shade from trnsal_finish_stock , massal_variety where stk_comp_code = 1 and stk_destag = '' and stk_var_code = var_code group by var_shade
";
    $r = mysqli_query($conn, $sql);
 ...
 function getStockList()
	...
	$optbfgsm  = $_POST['optbfgsm'];
        ...
        $sql = "call spsal_rep_shade_stock($compcode,'$shadecode','$bf','$gsm','$optbfgsm')";
    $r = mysqli_query($conn, $sql);
   ...
   function getReelNos()
	...
	$size     = $_POST['size'];
	$compcode = $_POST['compcode'];
	$sql = "call spsal_rep_reel_stocklist($compcode,$size )";
...
$sql = "  select DATE_FORMAT( reth_date, '%d-%m-%Y') as retdate, reth_date, round(sum(reth_return_wt)/1000,3) as retweight,sum(reth_amt) as retamount from trnsal_salret_header where  reth_comp_code = '$compcode'  and reth_fincode ='$finid' and reth_date between '$startdate' and  '$enddate' group by reth_date order by reth_date desc";
...
$sql = "select cust_ref,reth_invno,round(reth_return_wt/1000,3) as retweight,reth_amt as retamount, 
...
$sql = "select var_bf,var_gsm,var_name, round(sum(rett_return_wt)/1000,3) retwt , rett_urate from trnsal_salret_header a, trnsal_salret_trailer b , massal_variety c , masprd_variety d where c.var_grpcode = d.var_groupcode and b.rett_var = c.var_code and  a.reth_no = b.rett_no and a.reth_fincode = b.rett_fincode  
...
//echo $qry;
        ...
        $sql = "call spsal_rep_bfwisesales($compcode,'$finid','$startdate','$enddate')";
    $r = mysqli_query($conn, $sql);
...
//echo $qry ;
        ...
        $sql = "call spsal_rep_bf_partywisesales($compcode,'$finid','$startdate','$enddate','$allbf','$selectbf')";
    $r = mysqli_query($conn, $sql);
...
//echo $qry;
        ...
        $sql = "call spsal_rep_bf_party_Invwisesales($compcode,'$finid','$startdate','$enddate','$allbf','$selectbf',$custcode)";
    $r = mysqli_query($conn, $sql);
...
function getAllREPOrderList()
{
 global $conn;  
	...
	$enddate   = $_POST['enddate'];
        ...
        $sql = "call spsal_rep_AllREPwiseOrder($compcode,'$finid','$startdate','$enddate')";
    $r = mysqli_query($conn, $sql);
 ...
 global $conn;  
	...
	$enddate   = $_POST['enddate'];
 	$repcode   = $_POST['rep'];
        ...
        $sql = "call spsal_rep_REPwiseOrder($compcode,'$finid','$startdate','$enddate',$repcode)";
    $r = mysqli_query($conn, $sql);
 ...
 global $conn;  
	...
	$enddate   = $_POST['enddate'];
 	$repcode   = $_POST['rep'];
        ...
        $sql = "call spsal_rep_Order_History($compcode,'$finid','$startdate','$enddate')";
    $r = mysqli_query($conn, $sql);
