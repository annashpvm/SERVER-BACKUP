<html>
<head>
<title>REEL SEND FOR SAMPLE PURPOSE </title>
<link rel="stylesheet" type="text/css" href="/ext-3.4.1/resources/css/ext-all.css" />
<link rel="stylesheet" type="text/css" href="/ext-3.4.1/examples/shared/example.css">
<link rel="stylesheet" type="text/css" href="/SHVPM/styles.css" />
<link rel="stylesheet" type="text/css" href="/SHVPM/SALES/TrnSalesSample/fpdf.css" />

<script type="text/javascript" src="/ext-3.4.1/adapter/ext/ext-base.js"></script>
<script type="text/javascript" src="/ext-3.4.1/ext-all.js"></script>
<script type="text/javascript" src="TrnSalesSampleEntry.js"></script>
<script type="text/javascript" src="/SHVPM/SALES/SalesMainMenu.js"></script>
<?php
    require($_SERVER["DOCUMENT_ROOT"]."/SHVPM/SALES/SalesMainPage.php");
    require($_SERVER["DOCUMENT_ROOT"]."/SHVPM/SALES/TrnSalesSample/fpdf.php");
?>
</head>
<body>
 <div id="Grp-form" ></div>
</body>
</html>
