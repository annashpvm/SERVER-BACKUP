...
if ($invno === "0")
{
        $sql = "select cust_code,cust_ref from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_customer c where a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_invstat <> 'T'and a.pckh_party = c.cust_code and a.pckh_fincode = $fincode and a.pckh_comp_code =$compcode  group by cust_code,cust_ref order by cust_code,cust_ref");
}
else
{
        $sql = "select cust_code,cust_ref  from trnsal_invoice_header , massal_customer  where invh_party = cust_code and  invh_fincode= '$fincode'  and invh_comp_code= '$compcode' and invh_seqno = $invno ");
...
function getSONOlist()
	...
	$sql = "select  pckt_sono from trnsal_packslip_trailer  where pckt_comp_code = $compcode and pckt_fincode = $fincode  and pckt_no = $slipno  group by pckt_sono
");
