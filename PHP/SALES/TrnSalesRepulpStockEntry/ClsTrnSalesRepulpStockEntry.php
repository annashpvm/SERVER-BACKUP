  ...
  function getentryno()
    {
        ...
        $finid = $_POST['finid'];
        $r=mysql_query("select IFNULL(max(r_entno),0)+1 as no from trnsal_repulp where  r_compcode ='$compcode' and r_finyear ='$finid'");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
//	$r=mysql_query("select prd_rollno from trn_dayprod_roll_details a, trn_dayprod_roll_variety_details where prd_seqno = prdv_seqno and prd_rollno = prdv_rollno and prd_compcode = '$compcode' and prd_fincode = '$finid'  and prd_date = '$rdate' and prd_roll_status = 'A' group by prd_rollno  order by prd_rollno");
  ...
  function getDocNolist()
    {
        ...
        $compcode = $_POST['compcode'];
        $finid = $_POST['finid'];
	$r=mysql_query("select r_entno ent_no from trnsal_repulp where  r_compcode ='$compcode' and r_finyear ='$finid' group by r_entno ");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
  ...
  function getDocNodetail()
	...
	$r=mysql_query("select a.* , b.var_code sizecode ,b.var_name  sizename from  trnsal_repulp a left join massal_variety b on  a.r_sizecode = b.var_code where  r_compcode =  $compcode and r_finyear = '$finid' and r_entno = '$docno'");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
