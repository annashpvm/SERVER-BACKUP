  ...
  function getentryno()
    {
        ...
        $finid = $_POST['finid'];
        $sql = "select IFNULL(max(r_entno),0)+1 as no from trnsal_repulp where  r_compcode ='$compcode' and r_finyear ='$finid'";
    $r = mysqli_query($conn, $sql);
...
//	$sql = "select prd_rollno from trn_dayprod_roll_details a, trn_dayprod_roll_variety_details where prd_seqno = prdv_seqno and prd_rollno = prdv_rollno and prd_compcode = '$compcode' and prd_fincode = '$finid'  and prd_date = '$rdate' and prd_roll_status = 'A' group by prd_rollno  order by prd_rollno");
  ...
  function getDocNolist()
    {
        ...
        $compcode = $_POST['compcode'];
        $finid = $_POST['finid'];
	$sql = "select r_entno ent_no from trnsal_repulp where  r_compcode ='$compcode' and r_finyear ='$finid' group by r_entno ";
    $r = mysqli_query($conn, $sql);
  ...
  function getDocNodetail()
	...
	$sql = "select a.* , b.var_code sizecode ,b.var_name  sizename from  trnsal_repulp a left join massal_variety b on  a.r_sizecode = b.var_code where  r_compcode =  $compcode and r_finyear = '$finid' and r_entno = '$docno'";
    $r = mysqli_query($conn, $sql);
