<?php
 ...
 function getInvoiceNoList()
    ...
    $gsttype = $_POST['gsttype'];
        ...
        $r=mysql_query("select invh_invrefno , invh_seqno from trnsal_invoice_header where  invh_saltype = '$gsttype' and invh_fincode= $finid  and invh_comp_code= $compcode  order by invh_no desc");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
{
      if (  $gsttype  == "TN")
        $r=mysql_query("select cust_code,cust_ref  ,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip,cust_gstin,cust_name  from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_customer c , massal_repr d  where cust_state = 24 and cust_repr = repr_code and a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_invstat <> 'T'and a.pckh_party = c.cust_code and a.pckh_fincode = $fincode and a.pckh_comp_code =$compcode  group by cust_code,cust_ref,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip,cust_gstin,cust_name  order by cust_code,cust_ref");
      else
        $r=mysql_query("select cust_code,cust_ref  ,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip ,cust_gstin,cust_name from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_customer c , massal_repr d  where cust_state <> 24 and cust_repr = repr_code and a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_invstat <> 'T'and a.pckh_party = c.cust_code and a.pckh_fincode = $fincode and a.pckh_comp_code =$compcode  group by cust_code,cust_ref,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip,cust_gstin,cust_name  order by cust_code,cust_ref");
}
else
{
        $r=mysql_query("select cust_code,cust_ref ,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip  ,cust_gstin,cust_name  from trnsal_invoice_header , massal_customer , massal_repr  where cust_repr = repr_code and invh_party = cust_code and  invh_fincode= '$fincode'  and invh_comp_code= '$compcode' and invh_seqno = $invno ");
...
*/
	...
	$r=mysql_query("select Cast(sum(pckt_wt) as decimal(10,0)) as weight,count(pckt_sr_no)  as nos,pckh_noofreels, var_size2 as size,pckh_date,var_grpcode as varcode ,var_name,var_tariffno as hsncode,pckt_size as sizecode,var_size1,var_size2 , ordt_rate as rate, round(Cast(sum(pckt_wt) as decimal(10,0))/1000 * ordt_rate,2)  as amount, pckh_truck  from trnsal_packslip_header a,  trnsal_packslip_trailer b, massal_variety d , trnsal_order_trailer e where  b.pckt_sono = e.ordt_sono and a.pckh_fincode >= e.ordt_fincode and a.pckh_comp_code = e.ordt_comp_code and b.pckt_size = e.ordt_var_code  and a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_no =$slipno  and pckh_party = $custcode and b.pckt_size = d.var_code and a.pckh_no = b.pckt_no and b.pckt_fincode =$fincode   and a.pckh_comp_code = $compcode group by  pckh_noofreels, var_grpcode,var_name,var_tariffno,pckt_size,pckh_date,var_size1,var_size2, ordt_rate,pckh_truck");
...
*/
        ...
        $r=mysql_query("select ordh_creditdays,ordh_gracedays,ordh_tax, tax_name,ordh_sgst,ordh_cgst,ordh_igst,ordh_ins_yn ,ordh_insper,ordh_delivery_add1,
ordh_delivery_add2,ordh_delivery_add3,ordh_delivery_city,ordh_delivery_pin,ordh_delivery_gst,ordh_cust_rem,
...
//        $r=mysql_query("select sum(invh_netamt) as salesinvamt from trnsal_invoice_header where invh_comp_code = $compcode  and invh_fincode =  $finid   and invh_date >= '$startdate' and invh_party = $custcode");

        $r=mysql_query("select case when sum(invh_netamt) > 0  then  sum(invh_netamt) else 0 end  as salesinvamt  from trnsal_invoice_header, massal_customer where  invh_party = cust_code and cust_tcs_applied = 'Y'  and invh_comp_code = $compcode  and invh_fincode =  $finid   and invh_date >= '$startdate' and invh_party = $custcode");
...
function getPartyDistance()
{
        mysql_query("SET NAMES utf8");
	$custcode  = $_POST['custcode'];
        $r=mysql_query("select cust_distance from massal_customer where cust_code = '$custcode'");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
function getSalesLedgerCode()
	...
	$r=mysql_query("select * from trnsal_packslip_header a,  trnsal_packslip_trailer b, massal_variety c , masprd_variety d,masprd_type e where c.var_grpcode = d.var_groupcode and d.var_typecode = e.vargrp_type_code and a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_no = $slipno  and b.pckt_size = c.var_code and b.pckt_fincode  = $fincode and a.pckh_comp_code =  $compcode  limit 1");
...
function getslipNetWt()
	...
	$r=mysql_query("select * from trnsal_packslip_header where pckh_comp_code= $compcode and pckh_fincode = $fincode  and pckh_no = $slipno");
