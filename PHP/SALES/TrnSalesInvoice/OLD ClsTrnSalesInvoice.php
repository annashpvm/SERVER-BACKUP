...
if ($invno === "0")
{
        $r=mysql_query("select cust_code,cust_ref ,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_customer c, massal_repr d where cust_repr = repr_code and a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_invstat <> 'T'and a.pckh_party = c.cust_code and a.pckh_fincode = $fincode and a.pckh_comp_code =$compcode  group by cust_code,cust_ref order by cust_code,cust_ref");
}
else
{
        $r=mysql_query("select cust_code,cust_ref ,cust_phone,cust_email, cust_smsno , repr_mobile ,cust_zip from trnsal_invoice_header , massal_customer , massal_repr  where cust_repr = repr_code and invh_party = cust_code and  invh_fincode= '$fincode'  and invh_comp_code= '$compcode' and invh_seqno = $invno ");
...
mysql_query("BEGIN");
 ...
 function getEInvStatus()
	...
	$invno = $_POST['invno'];
        ...
        $r=mysql_query("select * from AIS_OEIV where invEntry = '$invno');
