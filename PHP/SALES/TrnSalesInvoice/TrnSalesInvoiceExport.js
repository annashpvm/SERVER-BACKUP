 ...
 */ 	
 ...
 'invh_transportname' ,'invh_transportGST','U_ReUpload','U_AckNo','U_EWayBillNo','U_QR', 'U_irnno','invh_frtparty','freightParty','invh_ex_rate'
...
//                   btnAdd.focus();
    ...
    var txtDistance = new Ext.form.NumberField({
     ...
     var txtExchageRate = new Ext.form.NumberField({
 ...
 */
 ...
 //alert(loadAllCustomerStore.getAt(0).get('cust_gstin').length);
                            ...
                            if (loadAllCustomerStore.getAt(0).get('cust_gstin') !=  'URP')
                            {    
                 ...
                 compcode :Gincompcode,
                           ...
                           callback:function()
                       {
                              ...
                              //   txtfromparty.setRawValue(loadInvoicedetailsDataStore.getAt(0).get('invh_frtparty);
                                 ...
                                 txtExchageRate.setValue(loadInvoicedetailsDataStore.getAt(0).get('invh_ex_rate'));
 ...
 */
 ...
 function calculateItemValue(){
   ...
   var store = flxDetailInv.getStore();

for (var i = 0; i < store.getCount(); i++) {
    var rec = store.getAt(i);

    var inrRate = rec.get('rate') * Number(txtExchageRate.getValue());
    inrRate = Math.round((inrRate + 0.0001 + Number.EPSILON) * 100) / 100;
    inrRate = inrRate.toFixed(2);

    rec.set('inrrate', inrRate);
}
    ...
    var Row= flxDetailInv.getStore().getCount();
         ...
         for(var i=0;i<Row;i++)
         {
           ...
           value1 = Number(sel[i].data.inrrate) *  Number(sel[i].data.weight /1000);
 ...
 //    font-size:18px,
     ...
     [
        ...
        {header: "USD Rate", dataIndex:'rate',sortable:false,width:100,align:'right'},
        {header: "INR Rate", dataIndex:'inrrate',sortable:false,width:100,align:'right'},
        ...
        {header: "Value", dataIndex:'value',sortable:false,width:100,align:'right'},
        {header: "Tax Value", dataIndex:'taxval',sortable:false,width:100,align:'right'},
 ...
 */
 ...
 //                            url: 'tempsave.php',
         ...
         cancelflag	  :'0', 
                 ...
                 invhexrate        : Number(txtExchageRate.getValue()),
         },
               callback: function(options, success, response)
               {
