<?php
 ...
 function getDNNo()
	...
	$compcode = $_POST['compcode'];
        $sql = "select ifnull(max(dn_no),0)+1 as dnno from trn_delivery_note where dn_fincode= $finid  and dn_comp_code= $compcode";

        $sql = "select con_value as dnno from control_details where con_fincode= $finid  and con_compcode= $compcode";
 ...
 function getDNNoedit()
	...
	$finid = $_POST['fincode'];
	$compcode = $_POST['compcode'];
        $sql = "select dn_no  from trn_delivery_note where dn_fincode= $finid  and dn_comp_code= $compcode  group by dn_no  order by dn_no desc";
    $r = mysqli_query($conn, $sql);
 ...
 function getEditDNNo()
    ...
    $sql = "select * from trn_delivery_note a,  massal_customer c, massal_variety e where  a.dn_no =$dnno and  a.dn_custcode = c.cust_code and a.dn_size = e.var_code and a.dn_fincode =  $finid  and a.dn_comp_code = $compcode order by var_name,dn_sr_no";
...
//        $sql = "select cust_code,cust_ref from massal_customer a,trnsal_desp_advice b where a.cust_code = b.da_cust and     b.da_fincode = $fincode and b.da_comp_code = $compcode group by cust_code,cust_ref order by cust_ref";
        ...
        $sql = "select cust_code,cust_ref from massal_customer a,trnsal_order_header b where a.cust_code = b.ordh_party and b.ordh_fincode <= $fincode  and length(ordh_sono) != 6 and ordh_comp_code = $compcode  group by cust_code,cust_ref order by cust_ref";
...
/*
     $sql = "select stk_finyear,stk_sr_no as rollno,stk_wt ,stk_sono,ordh_sodate   from  trnsal_finish_stock a,massal_variety b, 
