...
+sum(pulp_stk) +sum(wt_change) +  sum(conv_qty) - sum(bund_qty)  ) <=0 then 0  else (sum(stk_qty) +sum(sal_qty)- sum(prod_qty)-sum(salr_qty)+sum(pulp_stk) +sum(wt_change) +sum(salvage_stk)+   sum(conv_qty) - sum(bund_qty) - sum(reel_qty) - sum(salvage_recpt_qty) ) end as opstk, 
  ...
  from  (
   ...
   select stk_var_code as var_code,  0 as stk_qty, 
     ...
     case when var_unit = 1 then count(*) else 0  end as reel_nos, 0 as salvage_recpt_qty,0 as salvage_recpt_nos,
      0 as conv_qty,0 as conv_nos from trnsal_finish_stock, massal_variety where  stk_var_code = var_code and  stk_ent_date >= '$startdate'  and stk_ent_date <= '$enddate' and stk_comp_code = '$compcode' and stk_source = 'C'  group by stk_var_code 
   ...
   select stk_var_code as var_code,  0 as stk_qty, 
     ...
     0 as conv_qty,0 as conv_nos from trnsal_finish_stock, massal_variety where  stk_var_code = var_code and  stk_ent_date >= '$startdate'  and stk_ent_date <= '$enddate' and stk_comp_code = '$compcode' and stk_source = 'S'  group by stk_var_code 
...
select distinct(rett_var) as var_code ,0 as stk_qty,0 as prod_qty ,0 as sal_qty  , sum(rett_return_wt)/1000 as salr_qty,0 as wt_change,0 as stk_nos, 0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos,count(*)  as salret_nos,0 as wtchg_nos,0 as pulp_nos ,0 as pulp_stk,0 as salvage_stk,0 as salvage_nos , 0 as bund_qty,0 as bund_nos, 0 as reel_qty, 0  as reel_nos , 0 as salvage_recpt_qty,0 as salvage_recpt_nos, 0 as conv_qty,0 as conv_nos   from trnsal_salret_header a,  trnsal_salret_trailer b, massal_variety c where b.rett_var = c.var_code and a.reth_no = b.rett_no and a.reth_comp_code = '$compcode' and a.reth_comp_code = b.rett_comp_code and  a.reth_fincode = b.rett_fincode and a.reth_fincode = '$finid'  and a.reth_date >= '$startdate'  and a.reth_date <= '$enddate'  group by rett_var
...
select oldsize as var_code ,0 as stk_qty,0 as prod_qty ,0 as sal_qty  , 0 as salr_qty,0 as wt_change,0 as stk_nos,0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos,0 as salret_nos,0 as wtchg_nos,0 as pulp_nos, 0 as pulp_stk,sum(oldweight)/1000  as salvage_stk,count(*)  as salvage_nos , 0 as bund_qty,0 as bund_nos,  0 as reel_qty, 0  as reel_nos , 0 as salvage_recpt_qty,0 as salvage_recpt_nos ,0 as conv_qty,0 as conv_nos  from  (select oldreelno,oldsize,oldweight from trnsal_salvage , trnsal_finish_stock    where   stk_comp_code = comp_code and  fin_code >= stk_finyear  and oldreelno = stk_sr_no  and ent_date >= '$startdate' and ent_date <= '$enddate' and  comp_code = $compcode and  fin_code = $finid   group by oldreelno,oldsize,oldweight) a1 group by oldsize

union all


select r_sizecode as var_code ,0 as stk_qty,0 as prod_qty ,0 as sal_qty  , 0 as salr_qty,0 as wt_change,0 as stk_nos, 0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos,0 as salret_nos,0 as wtchg_nos,count(*) as pulp_nos, sum(r_wt/1000) as pulp_stk,0 as salvage_stk,0 as salvage_nos , 0 as bund_qty,0 as bund_nos,  0 as reel_qty, 0  as reel_nos, 0 as salvage_recpt_qty,0 as salvage_recpt_nos ,0 as conv_qty,0 as conv_nos  from trnsal_repulp where  r_compcode = $compcode and  r_finyear = $finid  and  r_date between '$startdate' and '$enddate' group by r_sizecode
...
sum(wt_change) as wt_change,case when sum(stk_nos) <=0 then 0 else sum(stk_nos) end As stk_nos, sum(sal_val) As sal_val, sum(cgst) As cgst,  sum(sgst) As sgst,sum(stk_qty) as stk_qty, sum(prod_qty) As prod_qty,  sum(sal_qty) As sal_qty,sum(salr_qty) as salr_qty,sum(stk_nos)as nos  , sum(prod_nos) as prod_nos,sum(sal_nos) as sal_nos,sum(salret_nos) as salret_nos,sum(wtchg_nos) as wtchg_nos,sum(pulp_nos) as pulp_nos,sum(pulp_stk) as pulp_stk,sum(salvage_stk) as salvage_stk,sum(salvage_nos) as salvage_nos,sum(conv_qty) as conv_qty, sum(conv_nos) as conv_nos,sum(bund_qty) as bund_qty,sum(bund_nos) as bund_nos,sum(reel_qty) as reel_qty, sum(reel_nos)  as reel_nos , sum(salvage_recpt_qty) as salvage_recpt_qty, sum(salvage_recpt_nos)  as salvage_recpt_nos
  from  (
   ...
   select stk_var_code as var_code,  0 as stk_qty, 
     ...
     case when var_unit = 1 then sum(stk_wt)/1000 else 0  end as reel_qty,
     case when var_unit = 1 then count(*) else 0  end as reel_nos, 0 as salvage_recpt_qty,0 as salvage_recpt_nos,
      0 as conv_qty,0 as conv_nos from trnsal_finish_stock, massal_variety where  stk_var_code = var_code and  stk_ent_date >= '$startdate'  and stk_ent_date <= '$enddate' and stk_comp_code = '$compcode' and stk_source = 'C' and length(stk_sono) = 6  group by stk_var_code 
   ...
   select stk_var_code as var_code,  0 as stk_qty, 
     ...
     sum(stk_wt)/1000  as salvage_recpt_qty,
     count(*)  as salvage_recpt_nos,
     0 as conv_qty,0 as conv_nos from trnsal_finish_stock, massal_variety where  stk_var_code = var_code and  stk_ent_date >= '$startdate'  and stk_ent_date <= '$enddate' and stk_comp_code = '$compcode' and stk_source = 'S' and length(stk_sono) = 6  group by stk_var_code 
...
union all
     ...
     select newsize as var_code ,case when  sum(oldweight-newweight)/1000 > 0 then sum(oldweight-newweight)/1000  else sum(oldweight-newweight)/1000 end as stk_qty,0 as prod_qty ,0 as sal_qty  ,  0 as salr_qty,0  wt_change, 0 as stk_nos, 0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos, 0 as salret_nos,0 as wtchg_nos,0 as pulp_nos,  0 as pulp_stk,0 as salvage_stk,0 as salvage_nos , 0 as bund_qty,0 as bund_nos, 0 as reel_qty, 0  as reel_nos , 0 as salvage_recpt_qty,0 as salvage_recpt_nos, 0 as conv_qty,0 as conv_nos  from trnsal_reelweight_change , trnsal_finish_stock  where oldweight <> newweight and   comp_code = stk_comp_code and  srno = stk_sr_no  and  ent_date >=  '2022-11-01'   and stk_ent_date <= '$enddate' and comp_code = $compcode and  fin_code  =  $finid and length(stk_sono) = 6 and  ent_date > '$enddate' and concat(year(ent_date)-2000, right(concat('0',month(ent_date)),2)) <> concat(year(stk_ent_date)-2000, right(concat('0',month(stk_ent_date)),2))  group by newsize


union all

     select newsize as var_code ,0 stk_qty,0 as prod_qty ,0 as sal_qty  ,  0 as salr_qty,case when  sum(oldweight-newweight)/1000 > 0 then sum(oldweight-newweight)/1000  else sum(oldweight-newweight)/1000 end  wt_change, 0 as stk_nos, 0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos, 0 as salret_nos,0 as wtchg_nos,0 as pulp_nos,  0 as pulp_stk,0 as salvage_stk,0 as salvage_nos , 0 as bund_qty,0 as bund_nos,  0 as reel_qty, 0  as reel_nos , 0 as salvage_recpt_qty,0 as salvage_recpt_nos,0 as conv_qty,0 as conv_nos from trnsal_reelweight_change , trnsal_finish_stock  where oldweight <> newweight and   comp_code = stk_comp_code and  srno = stk_sr_no  and  ent_date >=  '2022-11-01'   and stk_ent_date <= '$enddate' and comp_code = $compcode and  fin_code  =  $finid and  ent_date >= '$startdate' and ent_date <= '$enddate' and length(stk_sono) = 6  and concat(year(ent_date)-2000, right(concat('0',month(ent_date)),2)) <> concat(year(stk_ent_date)-2000, right(concat('0',month(stk_ent_date)),2))  group by newsize



union all

     select newsize as var_code , sum(oldweight-newweight)/1000 * -1   as stk_qty,0 as prod_qty ,0 as sal_qty  ,  0 as salr_qty, 0  wt_change, 0 as stk_nos, 0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos, 0 as salret_nos,0 as wtchg_nos,0 as pulp_nos,  0 as pulp_stk,0 as salvage_stk,0 as salvage_nos , 0 as bund_qty,0 as bund_nos, 0 as reel_qty, 0  as reel_nos, 0 as salvage_recpt_qty,0 as salvage_recpt_nos , 0 as conv_qty,0 as conv_nos  from trnsal_reelweight_change , trnsal_finish_stock  where oldweight <> newweight and   comp_code = stk_comp_code and  srno = stk_sr_no  and  ent_date >=  '2022-11-01'   and stk_ent_date <= '$enddate' and comp_code = $compcode and  fin_code  =  $finid and  ent_date > '$enddate' and length(stk_sono) = 6  group by newsize
...
select oldsize as var_code ,0 as stk_qty,0 as prod_qty ,0 as sal_qty  , 0 as salr_qty,0 as wt_change,0 as stk_nos,0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos,0 as salret_nos,0 as wtchg_nos,0 as pulp_nos, 0 as pulp_stk,sum(oldweight)/1000  as salvage_stk,count(*)  as salvage_nos , 0 as bund_qty,0 as bund_nos,  0 as reel_qty, 0  as reel_nos , 0 as salvage_recpt_qty,0 as salvage_recpt_nos ,0 as conv_qty,0 as conv_nos  from  (select oldreelno,oldsize,oldweight from trnsal_salvage , trnsal_finish_stock    where   stk_comp_code = comp_code and  fin_code >= stk_finyear  and oldreelno = stk_sr_no  and ent_date >= '$startdate' and ent_date <= '$enddate' and  comp_code = $compcode and  fin_code = $finid and length(stk_sono) = 6  group by oldreelno,oldsize,oldweight) a1 group by oldsize
...
sum(wt_change) as wt_change,case when sum(stk_nos) <=0 then 0 else sum(stk_nos) end As stk_nos, sum(sal_val) As sal_val, sum(cgst) As cgst,  sum(sgst) As sgst,sum(stk_qty) as stk_qty, sum(prod_qty) As prod_qty,  sum(sal_qty) As sal_qty,sum(salr_qty) as salr_qty,sum(stk_nos)as nos  , sum(prod_nos) as prod_nos,sum(sal_nos) as sal_nos,sum(salret_nos) as salret_nos,sum(wtchg_nos) as wtchg_nos,sum(pulp_nos) as pulp_nos,sum(pulp_stk) as pulp_stk,sum(salvage_stk) as salvage_stk,sum(salvage_nos) as salvage_nos,sum(conv_qty) as conv_qty, sum(conv_nos) as conv_nos,sum(bund_qty) as bund_qty,sum(bund_nos) as bund_nos,sum(reel_qty) as reel_qty, sum(reel_nos)  as reel_nos , sum(salvage_recpt_qty) as salvage_recpt_qty, sum(salvage_recpt_nos)  as salvage_recpt_nos
  from  (
   ...
   select stk_var_code as var_code,  0 as stk_qty, 
     ...
     select newsize as var_code ,case when  sum(oldweight-newweight)/1000 > 0 then sum(oldweight-newweight)/1000  else sum(oldweight-newweight)/1000 end as stk_qty,0 as prod_qty ,0 as sal_qty  ,  0 as salr_qty,0  wt_change, 0 as stk_nos, 0 as sal_val,0 as cgst,0 as sgst,0 as igst,  0 as prod_nos,0 as sal_nos, 0 as salret_nos,0 as wtchg_nos,0 as pulp_nos,  0 as pulp_stk,0 as salvage_stk,0 as salvage_nos , 0 as bund_qty,0 as bund_nos, 0 as reel_qty, 0  as reel_nos , 0 as salvage_recpt_qty,0 as salvage_recpt_nos, 0 as conv_qty,0 as conv_nos  from trnsal_reelweight_change , trnsal_finish_stock  where oldweight <> newweight and   comp_code = stk_comp_code and  srno = stk_sr_no  and  ent_date >=  '2022-11-01'   and stk_ent_date <= '$enddate' and comp_code = $compcode and  fin_code  =  $finid and  ent_date > '$enddate'  and length(stk_sono) != 6  and concat(year(ent_date)-2000, right(concat('0',month(ent_date)),2)) <> concat(year(stk_ent_date)-2000, right(concat('0',month(stk_ent_date)),2))  group by newsize
