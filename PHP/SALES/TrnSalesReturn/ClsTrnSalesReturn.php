...
//		break;
 ...
 function getSalesReturnNo()
	...
	$compcode = $_POST['compcode'];
        $sql = "select ifnull(max(reth_no),0)+1 as retno from trnsal_salret_header where reth_fincode= $finid and reth_comp_code= $compcode ";
 ...
 function getSRNolist()
	...
	$finid = $_POST['finid'];
	$compcode = $_POST['compcode'];
        $sql = "select reth_no  from trnsal_salret_header where reth_fincode= $finid and reth_comp_code= $compcode group by reth_no  order by reth_no desc";
    $r = mysqli_query($conn, $sql);
 ...
 function getSRNodetail()
    {
      ...
      $sql = "select a.*,b.*,d.*,e.*,g.* ,c.cust_ref as partyname from trnsal_salret_header a, trnsal_salret_trailer b, massal_customer c, massal_variety d, trnsal_invoice_header e,  massal_tax g  where  a.reth_no = b.rett_no and a.reth_fincode = b.rett_fincode  and a.reth_cust = c.cust_code and b.rett_var = d.var_code  and a.reth_invno = e.invh_invrefno and e.invh_taxtag = g.tax_code and   a.reth_comp_code = b.rett_comp_code  and  a.reth_fincode >= e.invh_fincode and  a.reth_comp_code = e.invh_comp_code  and a.reth_comp_code = '$compcode'  and a.reth_fincode =   '$finid'  and a.reth_no = '$srno'";  
...
//echo $r;
        ...
        $sql = "select cust_ref,cust_code from trnsal_invoice_header a,massal_customer b, trnsal_packslip_header c, trnsal_packslip_trailer d where a.invh_party = b.cust_code  and a.invh_fincode = $fincode and a.invh_invrefno = c.pckh_invno And a.invh_date = c.pckh_invdt and c.pckh_no = d.pckt_no and c.pckh_fincode =  d.pckt_fincode and d.pckt_rettag <> 'T' and a.invh_comp_code = d.pckt_comp_code and a.invh_comp_code = $compcode group by cust_code,cust_ref order by cust_ref ";
...
function getstartno()
{
    ...
    $sql = "select pckt_sr_no as no,pckt_srno_fincode as fincode from trnsal_packslip_trailer where pckt_no = '$slipno' and pckt_fincode = '$fincode'
...
mysqli_set_charset($conn, "utf8");
	...
	$endno = $_POST['endno'];
        $sql = "select * from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_variety c where b.pckt_no = $slipno
...
function getpackslipdespatchdetails()
{
	...
	$slipno = $_POST['slipno'];
        ...
        $sql = "select var_name,pckt_size as itemcode,pckt_sr_no,pckt_wt,invt_urate,pckt_srno_fincode,invt_hsncode  from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_variety c , trnsal_invoice_trailer d , trnsal_invoice_header e    
