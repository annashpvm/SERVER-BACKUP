 ...
 function getSearchLedgerlist()
    {
        ...
        $sql = "select * from massal_customer  where left(cust_name,2) != 'ZZ' and cust_name like '%$ledname%'";
        $r = mysqli_query($conn, $sql);
        $arr = [];
        while ($re = mysqli_fetch_assoc($r)) {
           $arr[] = $re;
 ...
 function getGraceDaysCutomerList()
    {
        ...
        $sql = "select * from massal_customer ,massal_repr where cust_repr = repr_code and cust_grace_days > 0 and left(cust_ref,2) != 'zz' order by repr_name,cust_ref";
    $r = mysqli_query($conn, $sql);
