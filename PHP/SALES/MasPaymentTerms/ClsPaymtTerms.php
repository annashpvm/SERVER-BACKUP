<?php
    ...
    switch($task){
		...
		getSearchLedgerlist();
		break;
             	case "loadGraceDaysCutomerList":
                getGraceDaysCutomerList();
		break;
 ...
 function getSearchLedgerlist()
    {
        ...
        $qry = "select * from massal_customer  where left(cust_name,2) != 'ZZ' and cust_name like '%$ledname%'";
        $r=mysql_query($qry);
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
 ...
 function getGraceDaysCutomerList()
    {
        mysql_query("SET NAMES utf8");

        $qry = "select * from massal_customer ,massal_repr where cust_repr = repr_code and cust_grace_days > 0 and left(cust_ref,2) != 'zz' order by repr_name,cust_ref";
        $r=mysql_query($qry);
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
