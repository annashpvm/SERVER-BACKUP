<?php
 ...
 function getinvtype()
    {
       global $conn;  
        $sql = "select type_name ,type_code  from massal_invtype";
 ...
 function getDespAdvNo()
	...
	$compcode = $_POST['compcode'];
        $sql = "select ifnull(max(da_no),0)+1 as advno from trnsal_desp_advice where da_fincode= '$finid'  and da_comp_code= '$compcode'";
    $r = mysqli_query($conn, $sql);
	...
	$finid = $_POST['finid'];
	$compcode = $_POST['compcode'];
        $sql = "select da_no from trnsal_desp_advice where da_fincode= '$finid'  and da_comp_code= '$compcode' group by da_no order by da_no desc";
    $r = mysqli_query($conn, $sql);
...
/* Modified on 16/06/2021
 $sql = "select  cust_ref,da_date,da_ackno,da_ackdt,ordt_qty,ordt_qty,ordt_adv_qty,ordt_adv_qty,var_name,da_desqty,da_slipqty,da_despdt,da_urate,da_rem,
...
*/
 $sql = "select  cust_ref,da_date,da_ackno,da_ackdt,ordt_qty,ordt_qty,var_name,da_desqty,da_slipqty,da_despdt,da_urate,da_rem,
...
'$compcode' and a.ordh_party = '$party' and b.ordt_var_code = c.var_code  and (b.ordt_qty - b.ordt_inv_wt) > 0 group by var_code,var_name,ordh_sodate,var_size1,var_size2 order by var_name";
        $sql = "select var_code,var_name,ordh_sodate,var_size1,var_size2 , concat(var_name,' - ',var_size2, '- ', (case when var_inchcm = 'I' then 'Inch' else 'CM' end),' - ',(case when var_shade = 'NS' then 'NAT' when var_shade = 'GY' then 'GYT' when var_shade = 'DP' then 'DP' when var_shade = 'SY' then 'SHYS' when var_shade = 'GB' then 'GB'  when var_shade = 'VV' then 'SHVV+'   when var_shade = 'BB' then 'BB'  when var_shade = 'CO' then 'CORE BOARD'  else var_shade end)) as sizedisp from trnsal_order_header a,trnsal_order_trailer b, massal_variety c where a.ordh_sono = b.ordt_sono and a.ordh_fincode = b.ordt_fincode  and a.ordh_sono = '$socno' and a.ordh_comp_code = b.ordt_comp_code and a.ordh_comp_code = 
...
function getqtydet()
{
	...
	$sizecode = $_POST['sizecode'];
        ...
        $sql = "select  * from trnsal_order_trailer a , massal_variety b , masprd_variety p  where var_grpcode = var_groupcode and  ordt_var_code = var_code and  ordt_sono =  $sono  and a.ordt_comp_code = $compcode  and a.ordt_fincode <= $finid and a.ordt_var_code = $sizecode";
...
function getstockdet()
{
	...
	$compcode = $_POST['compcode'];
        $sql = "select sum(stk_wt)/1000 as stkwt from trnsal_finish_stock   
