...
var GetRatecodeDatastore = new Ext.data.Store({
   ...
   {
      if (txtPassword.getRawValue() == "sorateok")
      {
...
//      autoLoad : true,
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
          ...
          'cust_code', 'cust_ref','cust_led_code','cust_cr_days' , 'cust_grace_days'
...
/* 
                      //  	  txtOverDue.setValue(Find90DaysDueDataStore.getAt(0).get('invbalance'));

 
		                  if (Find90DaysDueDataStore.getAt(0).get('invbalance') > 0 && Find90DaysDueDataStore.getAt(0).get('nos') == 0  )
		                  { 
		                             Ext.getCmp('cmbPriceno').setDisabled(true);   
                                             Ext.getCmp('save').setDisabled(true);   
//		                             alert("This customer having > 90 days outstanding. You can't raise the SO");
...
/*
                           OverDueCheckDataStore.removeAll(); 
				OverDueCheckDataStore.load({
		                url: '/SHVPM/SALES/TrnSalesOrder/ClsTrnSalesOrder.php',
		                params:
                                {
		                       task:"CheckOverdue",
				...
				callback:function()
			        {
                                  cnt = OverDueCheckDataStore.getCount();
				  if (cnt > 0)
                                  {   
                                     Ext.getCmp('cmbPriceno').setDisabled(true);   
                                     alert("This customer having > 90 days outstanding. You can't raise the SO");
...
//                                finid:  cmbPriceno.getValue() ,
			...
			callback:function()
		            {
//alert(GetRatecodeDatastore.getCount());
			    rateitemcode = GetRatecodeDatastore.getAt(0).get('rate_item_code');
...
'tax_sgst','tax_cgst','tax_igst', 'sup_name','repr_name','type_name','agentname','ordh_creditdays','ordh_gracedays','ordh_comm','ordh_apprno',
'ordh_payterm_30days_cdamt' , 'ordh_payterm_60days_cdamt1' , 'ordh_payterm_60days_cdamt2'
])
    });
...
alert(LoadordernoDetailsDatastore.getAt(0).get('ordh_payterm_30days_cdamt')); 

                                  txtCashDiscMT_7Days.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_payterm_30days_cdamt'));  
                                  txtCashDiscMT_30Days.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_payterm_60days_cdamt1'));  
                                  txtCashDiscMT_45Days.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_payterm_60days_cdamt2'));  
...
var cmbShade = new Ext.form.ComboBox({
        ...
        typeAhead       : true,
        mode            : 'local',
//        store           :  [['N','NAT'],['G','GYT'],['D','DP'],['Y','SHYS'],['B','GB'],['V','SHVV+']],
...
var flxDetail = new Ext.grid.EditorGridPanel({
    ...
    listeners:{	
			...
			{
				if (selected_rows[i].data.soclose == null)
                                    flag = 'N';
                                else                                   
...
//alert(flxDetail.getStore().getCount());		 
    ...
    else if  (Number(cmbPriceno.getValue()) == 0)
    {
...
//alert(cmbPO.getRawValue());
		...
		ordhigst: Number(txtIgstPer.getRawValue()),
                ...
                ordhapprno : Number(cmbPriceno.getValue()),
