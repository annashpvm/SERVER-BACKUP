...
var ratefincode = GinFinid;

var EnableDestination = 'N';
...
function flx_change()
{
			...
			if ((selrow != null)){
				...
				custname = selrow.get('cust_ref');
                                ...
                                EnableDestination = selrow.get('cust_destination_enable_yn')
	 		        txtPayTerms.setValue(0);
          			txtGraceDays.setValue(0);
...
//                            if (areacode == 24)
//                           if (custcode == 1615 || custcode == 1616  || custcode == 1619 || custcode == 2068  || custcode == 432 || custcode == 4887 )
                            if (EnableDestination == 'Y')  
                            {
...
//                                finid:  cmbPriceno.getValue() ,
     ...
     txtrate.setValue('0');
                            ...
                            {     
                                 ...
                                 cmbPriceno.show();
                                 cmbPriceno.focus();
//                                  alert(GetRatecodeDatastore.getAt(0).get('rate_fincode'));
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
		...
		{header: "", dataIndex: 'area_code',sortable:true,width:60,align:'left',hidden:true},   
		{header: "", dataIndex: 'area_rategrp',sortable:true,width:60,align:'left',hidden:true},   
		{header: "", dataIndex: 'cust_destination_enable_yn',sortable:true,width:60,align:'left',hidden:true},   
...
//        readOnly : true,
 ...
 var dptRate = new Ext.form.DateField({
        fieldLabel: '',
        id: 'dptRate',
        name: 'dptRate',
        format: 'd-m-Y',
...
//        readOnly    : true,
   ...
   var txtPriceFincode = new Ext.form.NumberField({
        fieldLabel  : '',
        id          : 'txtPriceFincode',
        name        : 'txtPriceFincode',
        width       :  30,
        labelStyle      : "font-size:14px;font-weight:bold;color:#0080ff",
        tabindex : 2,
...
'rate_bf34gsm120', 'rate_bf34gsm100', 'rate_bf34gsm90', 'rate_bf34gsm80', 'rate_bf34gsm70', 'rate_bf34gsm60', 'rate_bf34gsm50','rate_bf12_bit','rate_bf12','rate_pb_variety' , 'rate_pb_rate'
])
    });
...
'rate_rate', 'rate_crdays', 'rate_cashdisc_per', 'rate_cashdisc_days', 'rate_gst_per' , 'rate_bf14_bit', 'rate_bf16_bit', 'rate_bf18_bit', 'rate_bf20_bit', 'rate_bf22_bit', 'rate_bf24_bit', 'rate_bf26_bit','rate_bf28_bit','rate_bf30_bit', 'rate_bf32_bit','rate_price_terms','rate_grace_days' , 'rate_rate_difference' , 'rate_fincode','rate_appr_date' ,
...
var LoadordernoItemDetailsDatastore = new Ext.data.Store({
        ...
        reader: new Ext.data.JsonReader({
            ...
            totalProperty: 'total',
            id: 'id'
        }, ['ordt_comp_code','ordt_fincode','ordt_sono','ordt_var_code','ordt_qty','ordt_adv_qty','ordt_apprno','ordt_rate','ordt_cappr_no', 'ordt_despdt', 'ordt_crdays', 'ordt_clo_stat', 'ordt_clo_reason', 'ordt_adv_tag', 'ordt_des_tag', 'ordt_approved', 'ordt_ma_tag', 'var_code', 'var_name', 'var_grpcode', 'var_unit', 'var_size1', 'var_size2', 'var_reams', 'var_sheets','shade_shortname','ordt_stk_wt','ordt_stk_reels','ordt_slno', 'var_tariffno','var_desc','var_typecode','var_bf','var_gsm','var_shade','var_inchcm','ordt_no_of_reels','ordt_fin_wt','ordt_inv_wt'
,'ordt_ent_date'
])
    });
...
var cmbSONo = new Ext.form.ComboBox({
 ...
 Ext.getCmp("chkChange").setValue(false);
                   flxParty.getStore().removeAll();  
				...
				params: {
				    ...
				    compcode:Gincompcode,
                           	...
                           	callback:function()
				{
                                 txtPriceFincode.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_appr_fincode'));
                                ...
                                {     
                                    ...
                                    cmbAreaPriceno.show();
                                    cmbPriceno.hide();
                                    cmbAreaPriceno.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_appr_fincode'));  
                                ...
                                {     
                                    ...
                                    cmbAreaPriceno.hide();
                                    cmbPriceno.show();
                                  cmbPriceno.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_appr_fincode'));  
                                  cmbPriceno.setRawValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_apprno')); 
                                }                            

                                  areacode = LoadordernoDetailsDatastore.getAt(0).get('cust_area');


                                  ratefincode = LoadordernoDetailsDatastore.getAt(0).get('ordh_appr_fincode');
...
//alert(LoadordernoDetailsDatastore.getAt(0).get('ordh_ins_yn'));
                                  ...
                                  txtCashDiscMT_7Days_45dayPT.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_payterm_45days_7days_receipt'));  
                                  txtCashDiscMT_30Days_45dayPT.setValue(LoadordernoDetailsDatastore.getAt(0).get('ordh_payterm_45days_30days_receipt'));  
...
/*       
                                          ...
                                          if (LoadordernoItemDetailsDatastore.getAt(0).get('ordt_fin_wt') > 0)
                                          {
                                                 alert("Finished Entries are  in this ize.  You cannot Modify...");
                                                  Ext.getCmp('save').setDisabled(false);
                                          } 
...
//						   size      : LoadordernoItemDetailsDatastore.getAt(j).get('var_size2'),
						   ...
						   entrydate  : LoadordernoItemDetailsDatastore.getAt(j).get('ordt_ent_date'),
...
//                                finid    : GinFinid ,
                                finid    : txtPriceFincode.getValue() ,
                                compcode : Gincompcode,
                                apprno   : cmbPriceno.getRawValue()       
                            },
                         callback:function()
                           {
                             dptRate.setRawValue(Ext.util.Format.date(GetVarietylistDatastore.getAt(0).get('rate_appr_date'),"d-m-Y")); 
			     BitReelInch =  GetVarietylistDatastore.getAt(0).get('rate_bitreel_inch');
 ...
 //      enableKeyEvents: true,
    listeners:{
        ...
        select: function(){
           ...
           //  ratefincode =cmbPriceno.getValue(); 
                         ...
                         callback:function()
                           {
                             ...
                             dptRate.setRawValue(Ext.util.Format.date(GetVarietylistDatastore.getAt(0).get('rate_appr_date'),"d-m-Y"));     
                             txtPriceFincode.setValue(GetVarietylistDatastore.getAt(0).get('rate_fincode'));
                             txtPayTerms.setValue(GetVarietylistDatastore.getAt(0).get('rate_price_terms'));
                             txtGraceDays.setValue(GetVarietylistDatastore.getAt(0).get('rate_grace_days'));
                             txtRateDiff.setValue(GetVarietylistDatastore.getAt(0).get('rate_rate_difference'));
...
function get_ratedetails() {
        ...
        {
        getRatedetailsDataStore.removeAll();
...
//		finid      : GinFinid ,
		finid      : txtPriceFincode.getValue(), //cmbPriceno.getValue() ,
		compcode   : Gincompcode,
                apprno     : cmbPriceno.getRawValue(),
                varty      : cmbvarietylist.getValue() ,
	 ...
	 callback:function()
	    {
                ...
                if (cmbvarietylist.getRawValue().substring(0,2) == "KB")
                    txtPayTerms.setValue("7");
                else    
                    txtPayTerms.setValue(paytermsoriginal);
...
//                if ((cmbSizetype.getValue() == "I" && Number(cmbSizeList.getRawValue())) > 16 || (cmbSizetype.getValue() == "C" && Number(cmbSizeList.getRawValue()) > 41))  
                if ((cmbSizetype.getValue() == "I" && Number(cmbSizeList.getRawValue())) > BitReelInch || (cmbSizetype.getValue() == "C" && Number(cmbSizeList.getRawValue()) > BitReelCM))  
                {
...
////Modified on 27/11/24 informed by Mrs.Suganya for Add  Rs.1000/- Extra for Bit Reels with BF  
			...
			else if (bf == 30)
			{
               ...
               if (gsmto3 > 0)
               {
                    ...
                    {
                        alert("The entered GSM is not given in the Rate Master");  
                       txtrate.setValue('0');
                    } 
               } 
  //end
...
var flxDetail = new Ext.grid.EditorGridPanel({
    ...
    [ 	 
        ...
        {header: "Shade "  , dataIndex: 'shade',sortable:false,width:60,align:'left', menuDisabled: true},
        {header: "BF"      , dataIndex: 'bf',sortable:false,width:40,align:'left', menuDisabled: true},
	...
	{header: "Dia "    , dataIndex: 'dia',sortable:false,width:80,align:'left', menuDisabled: true},
        ...
        {header: "Close Reason"  , dataIndex: 'closereason',sortable:false,width:100,align:'left', menuDisabled: true,
   	editor:{
		    xtype:'textfield',
                    allowBlank: true,
                    enableKeyEvents: true,
        ...
        {header: "Entry Date "  , dataIndex: 'entrydate',sortable:false,width:100,align:'left', menuDisabled: true},
		      ...
		      else if (btn === 'no')
                      {
		                ...
		                { 
		                    ...
		                    var selrow = sm.getSelected();
                                    fwt = selrow.get('finwt');
                                    iwt = selrow.get('invwt');
                                    if (selrow.get('soclose') == "Y")
                                    {
                                       alert("SO Already Closed. You can't Delete");
                                    }

                                    if (fwt > 0 || iwt > 0) 
                                    {
                                       alert("Finished / Invoice Entries are Made aginst this SO. You can't Delete");
                                    }
                                    else
                                    {
...
function priceConfirm()
{
  ...
  cmbPriceno.setDisabled(true); 
          ...
          txtrate.setValue('0');
           ...
           paytermsoriginal = Number(txtPayTerms.getValue());
...
style: {
   	     ...
   	     { xtype   : 'fieldset',
                title   : 'If  Payment Terms + Grace Days = 75 & 90 Days',
                 width       : 430,
                 height      : 170,
                 x           : 630,
                 y           : 320,
...
//alert(cmbitem.getRawValue());
			...
			sel[idx].set('entrydate' , Ext.util.Format.date(entdate,"Y-m-d"));
			...
			txtGodownReels.setValue(0);
                ...
                {      
				   ...
				   despdate  : Ext.util.Format.date(dptDespdate.getValue(),"Y-m-d"), 
                                   ...
                                   reellist  : reelnolist,
                                   entrydate : Ext.util.Format.date(entdate,"Y-m-d"),
...
function save_click()
{
    ...
    if (cust_area_priceType == 'C')
        rateseqno = cmbPriceno.getRawValue();
    else
...
//alert(flxDetail.getStore().getCount());		 
    ...
    else if (txtPriceFincode.getValue()==0 || txtPriceFincode.getRawValue()=="")
            {
                Ext.Msg.alert('Sales-Order','Price Approval Fin year is empty..');
                gstSave="false";
            }
...
//alert(cmbPO.getRawValue());
		...
		ordhigst: Number(txtIgstPer.getRawValue()),
                ...
                ordhapprfincode :  txtPriceFincode.getValue(),  
