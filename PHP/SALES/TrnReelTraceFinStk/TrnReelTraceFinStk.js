...
var loadSalesReturnDatastore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task:"loadSalesReturn"}, // this parameter asks for listing
...
var loadReelNoDetailDatastore = new Ext.data.Store({
	...
	'stk_wt','stk_ent_no','stk_location','stk_retno', 'stk_retdt', 'stk_rettag','stk_deltag','stk_deldate', 'stk_ent_date','stk_destag','stk_slipno','stk_desdt','ordh_sono','ordh_sodate','cust_ref','pckh_invno','pckh_invdt','var_name'
      ]),
    });
...
var loadReelNoDetailDatastore2 = new Ext.data.Store({
	...
	'stk_wt','stk_ent_no','stk_location','stk_deltag','stk_deldate', 'stk_ent_date','stk_destag','stk_slipno','stk_desdt','ordh_sono','ordh_sodate','cust_ref','pckh_invno','pckh_invdt'
      ]),
    });
...
var loadReelNoDetailDatastore3 = new Ext.data.Store({
	...
	'stk_wt','stk_ent_no','stk_deltag','stk_deldate', 'stk_ent_date','stk_destag','stk_slipno','stk_desdt','ordh_sono','ordh_sodate','cust_ref','dn_no'     ]),
    });


var loadReelNoDetailDatastore4 = new Ext.data.Store({
      id: 'loadReelNoDetailDatastore4',
      autoLoad : true,
      proxy: new Ext.data.HttpProxy({
                url: 'ClsTrnReelTraceFinStk.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadReelNODetail4"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
	...
	'stk_wt','stk_ent_no','stk_deltag','stk_deldate', 'stk_ent_date','stk_destag','stk_slipno','stk_desdt','ordh_sono','ordh_sodate','cust_ref','dc_no'    ]),
    });
...
var lblClose = new Ext.form.Label({
	fieldLabel  : '',
	id          : 'lblClose',
	name        : 'lblClose',
        width       : 120,
...
var flxVarietyDetails = new Ext.grid.EditorGridPanel({
        ...
        columns: [   
            ...
            {header: "", dataIndex: 'var_groupcode',sortable:true,width:45,align:'left' ,  hidden : 'true'},    
        ],
...
var flxSizeDetails = new Ext.grid.EditorGridPanel({
        ...
        columns: [   
          ...
          {header: "", dataIndex: 'var_code',sortable:true, width:50, align:'left', hidden : 'true' } , 
...
var btnFind = new Ext.Button({
 ...
 listeners:{
		...
		params: {
			    task: 'loadReelNos',
                            fweight  :  Number(txtFindWt.getValue()),
                            fincode  :  finid,
                            compcode :  Gincompcode, 
                            stkopt   :  3,
...
var txtFindWt = new Ext.form.NumberField({
        fieldLabel  : 'Find Wt',
        id          : 'txtFindWt',
        name        : 'txtFindWt',
        width       : 80,
	...
	listeners : {
		...
		params: {
          ...
          //           lblClose.setText('');
            ...
            dtentdate.setRawValue('');
            dtSOdate.setRawValue('');
            dtsalreturndate.setRawValue('')
            dtRepulp.setRawValue('');
            dtslipdate.setRawValue('');
	    dtinvdate.setRawValue('');

//alert(loadReelNoDetailDatastore.getAt(0).get('stk_destag'));
		             ...
		             txtLocation.setRawValue(loadReelNoDetailDatastore.getAt(0).get('stk_location'));
		             ...
		             dtSOdate.setRawValue(Ext.util.Format.date(loadReelNoDetailDatastore.getAt(0).get('ordh_sodate'),"d-m-Y"));
                             if  (loadReelNoDetailDatastore.getAt(0).get('stk_rettag') == 'T')
                             {
		             txtsalreturnno.setValue(loadReelNoDetailDatastore.getAt(0).get('stk_retno'));
		             dtsalreturndate.setRawValue(Ext.util.Format.date(loadReelNoDetailDatastore.getAt(0).get('stk_retdt'),"d-m-Y"));
                             }

                             if  (loadReelNoDetailDatastore.getAt(0).get('stk_deltag') == 'T')
                             {
			             dtRepulp.setRawValue(Ext.util.Format.date(loadReelNoDetailDatastore.getAt(0).get('stk_deldate'),"d-m-Y"));
                             }


                             if  (loadReelNoDetailDatastore.getAt(0).get('stk_destag') == 'C')
                             {

				  lblClose.setText('CLOSED');
                             } 


                             if  (loadReelNoDetailDatastore.getAt(0).get('stk_destag') == 'D')
                             {
				 txtslipno.setValue(loadReelNoDetailDatastore.getAt(0).get('stk_slipno'));
				 dtslipdate.setRawValue(Ext.util.Format.date(loadReelNoDetailDatastore.getAt(0).get('stk_desdt'),"d-m-Y"));
/*
...
*/
                             ...
                             if  ( loadReelNoDetailDatastore.getAt(0).get('stk_destag') == 'B')
                             {
				loadReelNoDetailDatastore4.removeAll();

		     		loadReelNoDetailDatastore4.load({
		     		url: 'ClsTrnReelTraceFinStk.php',
				params: {
					    task: 'loadReelNODetail4',
				            reelno   : txtReelNo.getValue(),
				            compcode : Gincompcode, 
				            fincode  : finid,
				        },
	...
	//				     txtslipno.setValue('');
	//				     txtinvno.setRawValue('');
				     var cnt=loadReelNoDetailDatastore4.getCount(); 
				     if (cnt > 0)
				     {   

                           	             txtSONo.setValue('');
                      		             txtcust.setRawValue(loadReelNoDetailDatastore4.getAt(0).get('cust_ref')); 
					     txtslipno.setValue(loadReelNoDetailDatastore4.getAt(0).get('stk_slipno'));
					     dtslipdate.setRawValue(Ext.util.Format.date(loadReelNoDetailDatastore4.getAt(0).get('stk_desdt'),"d-m-Y"));

					     txtinvno.setRawValue('DC'+loadReelNoDetailDatastore4.getAt(0).get('dc_no'));
					     dtinvdate.setRawValue(Ext.util.Format.date(loadReelNoDetailDatastore4.getAt(0).get('stk_desdt'),"d-m-Y"));
				     }
				     } 
				});
                             }     
...
/*
		...
		params: {
			    task: 'loadSalesReturn',
               	...
               	callback:function()
		     {
                     var cnt=loadSalesReturnDatastore.getCount(); 
                     if (cnt > 0)
		       {    
		           txtsalreturnno.setValue(loadSalesReturnDatastore.getAt(0).get('reth_no'));
		           dtsalreturndate.setRawValue(Ext.util.Format.date(loadSalesReturnDatastore.getAt(0).get('reth_date'),"d-m-Y"));
                       }  
                    }  
               });  
...
var txtLocation = new Ext.form.NumberField({
        fieldLabel  : 'Location',
        id          : 'txtLocation',
        name        : 'txtLocation',
        width       : 100,
...
var dtRepulp = new Ext.form.DateField({
    fieldLabel : 'Date',
    id         : 'dtRepulp',
    name       : 'date',
    format     : 'd-m-Y',
...
var TrnReelTraceFinishedStkFormPanel = new Ext.form.FormPanel({
        renderTo    : Ext.getBody(),
        xtype       : 'form',
        title       : 'Reel Trace in the Finished Stock',
        width       : 800,
        height      : 460,
	...
	labelStyle : "font-size:12px;font-weight:bold;",
   	style      : "border-radius:5px;", 	
        frame       : false,
        id          : 'TrnReelTraceFinishedStkFormPanel',
        method      : 'post',
        layout      : 'absolute',
        reader      : new Ext.data.JsonReader({
                    root:'rows',
            ...
            items: [
                ...
                },'-',
               {
                ...
                },'-',
                {
                    ...
                    listeners:{
                        click: function () {
                            TrnReelTraceFinishedStkWindow.RefreshData();
                        }
                    }
                },'-',
                {
                    ...
                    listeners:{
                        click: function(){
                            TrnReelTraceFinishedStkWindow.hide();
                        }
                    }
                }
                ]
...
*/
     ...
     var TrnReelTraceFinishedStkWindow = new Ext.Window({
        height      : 600,
        width       : 1300,
        y           : 30,
        layout      : 'fit',
        items       : TrnReelTraceFinishedStkFormPanel,
        closable    : true,
        minimizable : true,
        maximizable : true,
        resizable   : false,
...
//    finid = 22;   
    cmbFinyear.setValue("24");
    cmbFinyear.setRawValue("2024-2025");
       TrnReelTraceFinishedStkWindow.show();
});
