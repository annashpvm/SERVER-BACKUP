	...
	tabindex : 1,
    	...
    	listeners : {
            keyup: function () {
                  ...
                  flxQly.getStore().filter('var_name', txtSizeSearch.getValue());  
            }
        }
    });
...
//alert(grpcode);
		...
		if ( sizetype == 'C') 
		{
		sizedisplayl = "0"+Ext.util.Format.number(txtLength.getValue(),'000.00');
		sizedisplayb = "0"+Ext.util.Format.number(txtBreadth.getValue(),'000.00');
		n = String(sizedisplay).length;

		sizedisplayl = sizedisplayl.slice(-6);
		sizedisplayb = sizedisplayb.slice(-6);
                sizedisplay = sizedisplayl + "X" + sizedisplayb; //  + "S"  + sheetsdisplay + "R"+reamdisplay;
		}
...
//save                
         {
                         	...
                         	else if(cmbVariety.getRawValue()=="" || cmbVariety.getValue()==0)
				{
					...
					cmbVariety.setFocus();
                         	...
                         	else if( RBtype == "1" && txtSizeCode.getRawValue().length != 17)
				{
					alert("Error in Size ");
