<?php
 ...
 function getInvoiceNoList()
	...
	$invhdate = $_POST['invhdate'];
        ...
        $sql = "select *   from trnsal_invoice_header where invh_fincode=  $fincode  and invh_comp_code= $compcode and invh_date = '$invhdate'  and invh_seqno in (select  max(invh_seqno) from trnsal_invoice_header where invh_fincode=  $fincode and invh_comp_code= $compcode and invh_date = '$invhdate' and E_inv_confirm = 'Y' )";

//        $sql = "select * from trnsal_invoice_header where invh_fincode=  $fincode  and invh_comp_code= $compcode and invh_date = '$invhdate'");

        $sql = "select *   from trnsal_invoice_header where invh_fincode=  $fincode  and invh_comp_code= $compcode and invh_date = '$invhdate' and E_inv_confirm = 'Y'  order by invh_seqno desc";
...
if ($invno === "0")
{
        $sql = "select cust_code,cust_ref ,cust_phone from trnsal_packslip_header a, trnsal_packslip_trailer b, massal_customer c where a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_invstat <> 'T'and a.pckh_party = c.cust_code and a.pckh_fincode = $fincode and a.pckh_comp_code =$compcode  group by cust_code,cust_ref order by cust_code,cust_ref";
}
else
{
        $sql = "select cust_code,cust_ref ,cust_phone from trnsal_invoice_header , massal_customer  where invh_party = cust_code and  invh_fincode= '$fincode'  and invh_comp_code= '$compcode' and invh_seqno = $invno ";
...
pckh_date,var_grpcode as varcode ,var_name,var_tariffno as hsncode,pckt_var as sizecode,var_size1,var_size2 , round(ordt_rate /112.1 *100,2) as rate, round(sum(pckt_wt)/1000* ordt_rate,2) as amount, ordt_cash_dis as cashdisc,ordt_cashtag as cashflg,ordt_dealer_dis as dealdisc, ordt_dealertag as dealflg, ordt_reel_dis as reeldisc,ordt_reeltag as reelflg,ordt_reg_dis as regdisc,ordt_regtag as regflg,ordt_addnl_dis as adddisc,ordt_addnltag as addflg, ordt_crdays,ordt_comm from trnsal_packslip_header a,  trnsal_packslip_trailer b, massal_variety d , trnsal_order_trailer e, massal_invtype f where  a.pckh_invtype = f.type_code and a.pckh_ackno = e.ordt_ackno and a.pckh_fincode = e.ordt_fincode and a.pckh_comp_code = e.ordt_comp_code and b.pckt_var = e.ordt_var_code  and a.pckh_no = b.pckt_no and a.pckh_comp_code = b.pckt_comp_code and a.pckh_fincode = b.pckt_fincode and pckh_no = $slipno and pckh_party = $custcode and b.pckt_var = d.var_code and b.pckt_fincode = $fincode  and a.pckh_comp_code = $compcode group by type_name,type_code,pckh_ordno,pckh_orddate, pckh_ackno,pckh_ackdt,pckh_noofreels,pckh_noofbun,pckt_unit, var_grpcode, var_name, var_tariffno,pckt_var,pckh_date, var_size1,var_size2,ordt_rate,ordt_cash_dis,ordt_cashtag,ordt_dealer_dis,ordt_dealertag,ordt_reel_dis,ordt_reeltag,ordt_reg_dis,ordt_regtag,ordt_addnl_dis,ordt_addnltag,ordt_crdays,ordt_comm";
 }
else
 {
        $sql = "select type_name,type_code,sum(pckt_wt) as weight,count(pckt_sr_no)  as nos, pckh_ordno,pckh_orddate,pckh_ackno,pckh_ackdt,pckh_noofreels,pckh_noofbun,case when pckt_unit = 1 then 'Reels' else 'Bundles' end as unit,case when pckt_unit = 1 then var_size2 else concat(convert(var_size1,char), 'X' ,convert(var_size2,char))   end as size,
...
function getslipdiscount()
	...
	$slipno = $_POST['slipno'];
        $sql = "select * from trnsal_order_trailer where ordt_fincode = $fincode and ordt_ackno = $slipno and ordt_var_code = $varcode and ordt_comp_code = $compcode";
    $r = mysqli_query($conn, $sql);
