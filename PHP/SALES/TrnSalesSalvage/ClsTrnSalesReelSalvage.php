  ...
  function getentryno()
    {
        ...
        $finid = $_POST['finid'];
	$r=mysql_query("select IFNULL(max(ent_no),0)+1 as no from trnsal_salvage where  comp_code ='$compcode' and fin_code ='$finid'");
  ...
  function getDocNolist()
    {
        ...
        $compcode = $_POST['compcode'];
        $finid = $_POST['finid'];
	$r=mysql_query("select ent_no  from trnsal_salvage where  comp_code ='$compcode' and fin_code ='$finid' group by ent_no order by ent_no desc ");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
//	$r=mysql_query("select prd_rollno from trn_dayprod_roll_details a, trn_dayprod_roll_variety_details where prd_seqno = prdv_seqno and prd_rollno = prdv_rollno and prd_compcode = '$compcode' and prd_fincode = '$finid'  and prd_date = '$rdate' and prd_roll_status = 'A' group by prd_rollno  order by prd_rollno");
 ...
 function getNumber()
	...
	$no       = $_POST['rbno'];
        ...
        $r=mysql_query("select count(*) as nos , stk_ent_no,stk_ent_date from trnsal_finish_stock where stk_comp_code =$compcode and stk_finyear = $finid and stk_sr_no = '$no'  and stk_destag = '' ");
