<?php
    ...
    switch($task){
		...
		case "loadDatewiseFuelArrivals":
                getDatewiseFuelArrivals();
		break;
...
(select rech_cust_code,  rect_grnqty , rech_totalamount from trnrm_receipt_header join trnrm_receipt_trailer on rech_seqno =  rect_hdseqno  where rech_date between '$startdate' and '$enddate'  and rech_compcode = $compcode and rech_fincode = $finid
union all
...
$r=mysql_query("select  DATE_FORMAT(rech_date, '%d-%m-%Y')  as rech_date,rech_no,rech_cust_code, rech_billno, DATE_FORMAT(rech_billdate, '%d-%m-%Y') as rech_billdate,rect_item_code,rect_itemrate, rect_grnqty , rect_itemvalue ,itmh_name, rech_truckno  from (select rech_date,rech_no,rech_cust_code, rech_billno,rech_billdate,rect_item_code,rect_itemrate, rect_grnqty , rect_itemvalue, rech_truckno from trnrm_receipt_header join trnrm_receipt_trailer on rech_seqno =  rect_hdseqno  where rech_date between '$startdate' and '$enddate' and rech_compcode = $compcode and rech_fincode = $finid 
union all
 select rech_date,rech_no,rech_cust_code, rech_billno,rech_billdate,rect_item_code,rect_itemrate, rect_grnqty , rect_itemvalue, rech_truckno  from trnirm_receipt_header join trnirm_receipt_trailer on rech_seqno =  rect_hdseqno  where rech_date between '$startdate' and '$enddate' and rech_compcode = $compcode and rech_fincode =$finid )
...
a  , massal_customer , masrm_item_header where rect_item_code = itmh_code and  rech_sup_code = cust_code   and  rech_sup_code = $supcode and rect_item_code = $itemcode  order by rech_date");
 ...
 function getDatewiseArrivals()
	...
	$startdate= $_POST['startdate'];
        ...
        $r=mysql_query("select DATE_FORMAT(qc_rm_ticketdate, '%d-%m-%Y') as ticketdate ,qc_rm_ticketdate  , sum(qc_rm_ticketwt) ticketwt , sum(qc_rm_acceptqty+qc_rm_degradeqty) acceptedwt from trn_qc_rm_inspection where qc_rm_fincode =  $finid and qc_rm_compcode =  $compcode and qc_rm_ticketdate between '$startdate' and '$enddate' and qc_rm_itemtype ='F' group by qc_rm_ticketdate order by qc_rm_ticketdate");
 ...
 function getDatewiseFuelArrivals()
    {
        mysql_query("SET NAMES utf8");
	...
	$startdate= $_POST['startdate'];
        ...
        $r=mysql_query("select DATE_FORMAT(qc_fuel_ticketdate, '%d-%m-%Y') as ticketdate ,qc_fuel_ticketdate  , sum(qc_fuel_ticketwt) ticketwt , sum(qc_fuel_acceptqty) acceptedwt from trn_qc_fuel_inspection where qc_fuel_fincode =  $finid and qc_fuel_compcode =  $compcode and qc_fuel_ticketdate between '$startdate' and '$enddate' group by qc_fuel_ticketdate order by qc_fuel_ticketdate desc");
 ...
 function getRMArrivalsDate()
	...
	$repdate= $_POST['repdate'];
        ...
        $r=mysql_query("select  cust_ref,cust_gstin,qc_rm_truck,qc_rm_entryno, qc_rm_ticketno, qc_rm_ticketwt,qc_rm_moisper_totalmaterial,qc_rm_moisforqty,qc_rm_moisper, qc_rm_moisqty, qc_rm_llessper, qc_rm_llessqty, qc_rm_rejectper, qc_rm_rejectqty, qc_rm_degradeqty, qc_rm_acceptqty, qc_rm_remarks, area_name from trn_qc_rm_inspection , massal_customer , masrm_item_header , mas_area where qc_rm_area = area_code and qc_rm_itemcode = itmh_code and qc_rm_supcode = cust_code and qc_rm_fincode = '$finid' and qc_rm_compcode ='$compcode' and qc_rm_ticketdate ='$repdate'  and qc_rm_itemtype ='F'  order by qc_rm_slno");
 ...
 function getFuelArrivalsDate()
	...
	$repdate= $_POST['repdate'];
        ...
        $r=mysql_query("select  cust_ref,cust_gstin,qc_fuel_truck,qc_fuel_entryno, qc_fuel_ticketno, qc_fuel_ticketwt,qc_fuel_tot_ded_qty, qc_fuel_acceptqty from trn_qc_fuel_inspection , massal_customer , masfu_item_header , mas_area where qc_fuel_area = area_code and qc_fuel_itemcode = itmh_code and qc_fuel_supcode = cust_code and qc_fuel_fincode = '$finid' and qc_fuel_compcode ='$compcode' and qc_fuel_ticketdate ='$repdate'");
