...
'qc_rm_debitnote_no','qc_rm_grnno','qc_rm_act_moisure_qty','qc_rm_mois_tolarance'
 ...
 var loadQCTicketNoCheckDatastore = new Ext.data.Store({
      id: 'loadQCTicketNoCheckDatastore',
      proxy: new Ext.data.HttpProxy({
                url: '/SHVPM/QC/ClsQC.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"checkQCTicketNumber"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
//alert(itemmois);
	    ...
	    txtActMoisWt.setValue(Ext.util.Format.number(Number(txtMoisForQty.getValue())  * Number(txtMoisPer.getValue()-itemmois)  / 100,"0"));

	    txtRemarks.setRawValue(txtRemarks.getRawValue()+ "Ex. Moisture " + Ext.util.Format.number((txtMoisPer.getValue() - itemmois), "0.00") + "%" + " for the Qty : "+ Ext.util.Format.number(txtMoisForQty.getValue(), "0") + " Kgs. Qty Deducted for moisture is "  + Ext.util.Format.number(txtActMoisWt.getValue(), "0") + " Kgs." + " ,");
	    }
	    else
	    {
	       txtMoisWt.setValue(0);
	      ...
	      if (Number(txtMoisPer.getValue()) - Number(itemmois) > 0 && Number(txtMoisTolarance.getValue()) > 0)
	    {
                 var tolmoiswt = Number(txtMoisForQty.getValue())  * (Number(txtMoisPer.getValue())-itemmois-Number(txtMoisTolarance.getValue()) )  / 100;
                 
if (Number(tolmoiswt) < 0)  tolmoiswt = 0;

    txtMoisWt.setValue(Ext.util.Format.number(tolmoiswt, "0"));



	//    txtMoisWt.setValue(Ext.util.Format.number(Number(txtMoisForQty.getValue())  * Number(txtMoisPer.getValue()-itemmois-Number(txtMoisTolarance.getValue()) )  / 100,"0"));

	    txtRemarks.setRawValue(txtRemarks.getRawValue() + " Moisture Tol :" + txtMoisTolarance.getRawValue() + "% Final Mois Qty : " + Ext.util.Format.number(txtMoisWt.getValue(), "0") + " Kgs" );
	    }
...
function refresh()
{
	...
	txtMoisForQty.setValue('');
        txtBales.setValue('');
        txtActMoisWt.setValue('');
        txtMoisTolarance.setValue('');
...
var txtActMoisWt = new Ext.form.NumberField({
        fieldLabel  : '',
        id          : 'txtActMoisWt',
        name        : 'txtActMoisWt',
        width       :  60,        
...
var txtMoisTolarance = new Ext.form.NumberField({
        fieldLabel  : '',
        id          : 'txtMoisTolarance',
        name        : 'txtMoisTolarance',
        width       :  50,        
        labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
        allowBlank  :  false,
        decimalPrecision: 2,
	...
	listeners:{
          specialkey:function(f,e){
             if (e.getKey() == e.ENTER)
             {
                   Ext.getCmp('txtDegradeQty').focus(false, 200);
...
var lblActMois = new Ext.form.Label({
    fieldLabel  : 'Act Mois',
    id          : 'lblActMois',
     labelStyle : "font-size:11px;font-weight:bold;color:#cc00cc",
    width       : 60
...
var lblMoisTolarance = new Ext.form.Label({
    fieldLabel  : 'Mois(Tol)',
    id          : 'lblMoisTolarance',
     labelStyle : "font-size:11px;font-weight:bold;color:#cc00cc",
    width       : 60
...
var btnTEMPSubmit = new Ext.Button({
    style   : 'text-align:center;',
    text    : "MODIFY",
    id      :'btnTEMPSubmit',
    width   : 80,
				...
				supcode    : supcode,
                              ...
                              callback: function(options, success, response)
                              {
                                var obj = Ext.decode(response.responseText);
                                 if (obj['success']==="true")
					{                                
                                    Ext.MessageBox.alert("QC -RM Inspection Modified No.-" + obj['EntryNo']);
...
// 				    QCPanel.getForm().reset();
					...
					{
			Ext.MessageBox.alert("QC -RM Inspection Entry Not Modified! Pls Check!- " + obj['EntryNo']);         
 QCPanel.getForm().reset();                                         
 RefreshData();
                                    }
                                }
                            ...
                            params :
                             {
                              ...
                              callback: function(options, success, response)
                              {
                                var obj = Ext.decode(response.responseText);
                                 if (obj['success']==="true")
			             		{                                
                                    Ext.MessageBox.alert("QC -RM Inspection Deleted No.-" + obj['EntryNo']);
                                         flxDetail.getStore().removeAll();
                                         flxTicket.getStore().removeAll();
                                         flxItem.getStore().removeAll();
...
// 				    QCPanel.getForm().reset();
					...
					{
			Ext.MessageBox.alert("QC -RM Inspection Entry Not Deleted! Pls Check!- " + obj['EntryNo']);         
                     QCPanel.getForm().reset();    
                     RefreshData();                                     
                                    }
                                }
...
//                     swt = Number(bwt)-Number(mwt);
                ...
                for (var i=0;i<selrows;i++)
		{
			...
			sel[idx].set('actmois'  , Number(txtActMoisWt.getValue()));
			sel[idx].set('moistol'  , Number(txtMoisTolarance.getValue()));
		...
		if (ticketchk > 0)
		{
				...
				remarks2    :  txtRemarks2.getValue(),
                                ...
                                fixedmois   :  itemmois,
                                shortage    :  shortagewt,
                                actmois     :  Number(txtActMoisWt.getValue()),
                                moistol     :  Number(txtMoisTolarance.getValue()),
...
//                        cmbSupplier.setRawValue(loadQCEntryNoDetailDatastore.getAt(0).get('cust_ref'));
	                ...
	                cmbTruckNo.setRawValue(loadQCEntryNoDetailDatastore.getAt(0).get('qc_rm_truck'));
                        ...
                        grnno  = loadQCEntryNoDetailDatastore.getAt(0).get('qc_rm_grnno');
...
var flxTicket = new Ext.grid.EditorGridPanel({
    ...
    listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
                ...
                if (txtQCEntNo.getValue() > 0  &&  gstFlag == "Add")
                {    

		var sm = flxTicket.getSelectionModel();
		var selrow = sm.getSelected();

		loadQCTicketNoCheckDatastore.removeAll();
		loadQCTicketNoCheckDatastore.load({
		     url: '/SHVPM/QC/ClsQC.php',
		     params: {
				...
				{ 
					Ext.Msg.show({
		              ...
		              originalrate = 0; 
                        ...
                        {
                             alert("Ticket alreday processed vide QC Entry No. : "+loadQCTicketNoCheckDatastore.getAt(0).get('qc_rm_entryno') + "   Please check ..")
                        }
                  }
		});
         }
		...
		//		txtAcceptWt.setValue(selrow.get('wc_acceptedwt'));
				...
				{ 
					Ext.Msg.show({
                		...
                		dedrate = 0;
		              originalrate = 0; 
         //     alert("QC Entry Number is Blank . Please click Refresh and Continue..");
         }        
    }
...
var flxDetail = new Ext.grid.EditorGridPanel({
    ...
    [
        ...
        {header: "Act Mois", dataIndex: 'actmois',sortable:true,width:100,align:'right'},
    ...
    listeners:{	
        ...
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
		     ...
		     if (btn === 'yes')
				...
				txtActMoisWt.setValue(selrow.get('actmois'));
				txtMoisTolarance.setValue(selrow.get('moistol'));
...
var cmbTruckNo = new Ext.form.ComboBox({
   ...
   function save_data()
   {
	            ...
	            Ext.each(grnData, function (record) {
                   ...
                   var TicketData = flxTicket.getStore().getRange();                                        
	            var TicketupdData = new Array();
	            Ext.each(TicketData, function (record) {
	                TicketupdData.push(record.data);
	            });
...
//save
	...
	for(var i=1;i<Row;i++)
        {       
            if ( Number(sel[i].data.rate) == Number(sel[i].data.dedrate))
            {
                 SameRate = 1;
                            ...
                            if (btn === 'yes')
				{
	                    ...
	                    {  
				          ...
				          Ext.Msg.show({
				            ...
				            icon: Ext.Msg.QUESTION,
				            buttons: Ext.MessageBox.YESNO,
				            msg: 'Billing Rate and Deduction Rate are SAME . Confirm Again to SAVE QC ',
				            fn: function(btn)
						{
				            if (btn === 'yes')
						{    
...
*/
   ...
   function RefreshData()
   {
       txtRate.setValue('');
       Ext.getCmp('save').setDisabled(false);  
       Ext.getCmp('btnTEMPSubmit').setDisabled(true);  
       Ext.getCmp('btnDelete').setDisabled(true);  
       flxItem.hide(); 
       gstFlag = "Add";
