		              	...
		              	callback:function()
				    {
				      ...
				      if (cnt>0)
				      {
				          for(var j=0; j<cnt; j++) 
				          {
				              ...
				              flxParty.getStore().insert(
				                 flxParty.getStore().getCount(),
				                 new dgrecord({
...
//                    RefreshGridData();
    ...
    var findLedgerdatastore = new Ext.data.Store({
     ...
     function grid_tot(){
        ...
        var selrows = flxParty.getSelectionModel().getCount();
...
function edit_click()
{
    ...
    var btnGenerate = new Ext.Button({
		...
		var crjvamt = 0;
                var drcr = '';
		for (var i=0;i<selrows;i++){
                    party     = sel[i].data.led_name;
                    partycode = sel[i].data.led_code;
		    dbamt     = Number(sel[i].data.dbamt);
		    cramt     = Number(sel[i].data.cramt);                  
                    partytype = sel[i].data.led_type;
                    jvamt     = Number(txtJVAmount.getValue());
                    dbjvamt = 0;
		    crjvamt = 0;
                    if (Number(dbamt) >  Number(cramt))
                    {   
                       drcr = "Cr";
                       crjvamt = jvamt
                    } 
                    ...
                    totjvamt = Number(dbjvamt)+Number(crjvamt);
                    ...
                    flxDetail.getStore().insert(
                        ...
                        new dgrecord({
                            ledname : party,   
                            type    : drcr,
                            dbamt   : Number(dbjvamt),
                            cramt   : Number(crjvamt),
	                    ledseq  : partycode,
                            totamt  : totjvamt,
                            ledtype : partytype
                        })
...
renderer: function (val, metaData, r){
		...
		//         style: 'currency',
            ...
            {header: "C / S",  dataIndex: 'led_type',sortable:true,width:60,align:'left',hidden:false},
            {header: "Ledger type",  dataIndex: 'ledger_type',sortable:true,width:120,align:'left',hidden:false},
        ],
        store:[],
    ...
    items       : [
                   {
                     xtype: 'panel',
                     id   : 'tab1', 
                     title: 'PARTY SELECTION',bodyStyle:{"background-color":"#ebebdf"},
                     layout: 'absolute',
                     items: [
			{
			    xtype       : 'fieldset',
