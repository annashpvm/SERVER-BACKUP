...
where
 ...
 function getcmbacctnamenew()
	...
	$finyear=$_POST['finyear'];
        $compcode=$_POST['compcode'];
	//$r=mysql_query("CALL general_sp_mas_selcontrolmaster('ST','$finyear','IRN','STORES ISSUE RETURN NO','$compcode')");
        $r=mysql_query("select ifnull(max(convert(substring(accref_vouno,4),signed)),0) +1 as con_value from acc_ref where accref_vou_type = 'GJV' and accref_finid = '$finyear' and accref_comp_code = '$compcode';");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
    ...
    function getHeadBankAccountName()
    {
        ...
        $r=mysql_query("select led_code, led_name from acc_ledger_master where led_grp_code = 28 and led_name like '%BANK%'");            
        $r=mysql_query("select led_code, led_name from acc_ledger_master where led_grp_code = 28");            
    ...
    function getHeadCashAccountName()
    {
        $compcode = $_POST['compcode'];
        $finid    = $_POST['finid'];
        $r=mysql_query("select led_code, led_name from acc_ledger_master where  led_code = 1729 order by led_name");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
