...
new Ext.KeyMap( Ext.getBody(), [{
    ...
    var LoadSalesVouNoDetailsdatastore = new Ext.data.Store({
        id: 'LoadSalesVouNoDetailsdatastore',
        proxy: new Ext.data.HttpProxy({
                  url: 'ClsSalesAccount.php',      // File to connect to
                  method: 'POST'
              }),
              baseParams:{task: "LoadSalVouNoDetails"}, // this parameter asks for listing
        reader: new Ext.data.JsonReader({
                    // we tell the datastore where to get his data from
          root: 'results',
          totalProperty: 'total',
          id: 'id'
        },['sal_seqno', 'sal_compcode', 'sal_finid', 'sal_vouno', 'sal_date','sal_partycode', 'sal_billno', 'sal_billdate', 'sal_slno', 'sal_description', 'sal_hsn', 'sal_rate', 'sal_qty', 'sal_unit', 'sal_value', 'sal_taxvalue', 'sal_taxledger', 'sal_cgst_per', 'sal_cgst_amt', 'sal_cgst_ledger', 'sal_sgst_per', 'sal_sgst_amt', 'sal_sgt_ledger','sal_igst_per', 'sal_igst_amt', 'sal_igst_ledger', 'sal_rounding', 'sal_round', 'sal_totalamount', 'sal_accseqno', 'cust_code', 'cust_name', 'led_grp_code','cust_type', 'led_custcode'])
    });
...
SalesEntryFormPanel.getForm().reset();
...
gstFlag = "Add";
	   ...
	   VouNodatastore.load({
		...
		callback: function(){
		    ...
		    txtVouNo.setRawValue("SDE"+VouNodatastore.getAt(0).get('con_value'));
		}
	    });
        ...
        listeners:{
           select: function(){
                      ...
                      flxDetail.getStore().removeAll();
                       ...
                       Ext.getCmp('editchk').hide();
     	               LoadSalesVouNoDetailsdatastore.load({
                           url: 'ClsSalesAccount.php',
	                   params: {
			        task: 'LoadSalVouNoDetails',
			        fincode : ginfinid,
			        compcode: gincompcode,
                                vouno   : cmbVouno.getRawValue(),
	                  },
		          callback: function () {
                              var cnt=LoadSalesVouNoDetailsdatastore.getCount();
                                       var sno = 1
                              ...
                              if (cnt>0)
                              {
                                   ...
                                   dtpVoudate.setRawValue(Ext.util.Format.date(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_date'),"d-m-Y"));  
                                   txtPartyBillNo.setRawValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_billno'));
                                   dtpBilldate.setRawValue(Ext.util.Format.date(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_billdate'),"d-m-Y"));  

                                   accseqno = LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_accseqno');
                                   docseqno = LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_seqno');

                                   cmbDebitLedger.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_taxledger'));
                                   txtTaxableValue.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_taxvalue'));
                                   txtCgstper.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_cgst_per'));
                                   txtCgstvalue.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_cgst_amt'));
                                   txtSgstper.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_sgst_per'));
                                   txtSgstvalue.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_sgst_amt'));
                                   txtIgstper.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_igst_per'));
                                   txtIgstvalue.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_igst_amt'));
                                   cmbCGSTledger.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_cgst_ledger'));
                                   cmbSGSTledger.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_sgst_ledger'));
                                   cmbIGSTledger.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_igst_ledger'));
                                   txtRounding.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_rounding'));
                                   txtPartyAmount.setValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_totalamount'));


				accledcode = LoadSalesVouNoDetailsdatastore.getAt(0).get('sal_partycode');
				txtPartyName.setRawValue(LoadSalesVouNoDetailsdatastore.getAt(0).get('cust_name'));
                                ledDRCR = LoadSalesVouNoDetailsdatastore.getAt(0).get('cust_type'); 
                                ledpartycode = LoadSalesVouNoDetailsdatastore.getAt(0).get('led_custcode');
                                  ...
                                  { 
	                                 ...
	                                 flxPurchase.getStore().getCount(),
                                         new dgrecord({
					     description : LoadSalesVouNoDetailsdatastore.getAt(j).get('sal_description'),      
                                             hsn   : LoadSalesVouNoDetailsdatastore.getAt(j).get('sal_hsn'),
					     qty   : LoadSalesVouNoDetailsdatastore.getAt(j).get('sal_qty'),  
                                             rate  : Number(LoadSalesVouNoDetailsdatastore.getAt(j).get('sal_rate')),
                                             value : LoadSalesVouNoDetailsdatastore.getAt(j).get('sal_value'), 
                                          })  
                                          );
                                  }
        flxPurchase.getStore().insert(
...
var tabAccSales = new Ext.TabPanel({
    id          : 'tabAccSales',
...
listeners: {
     ...
     'tabchange': function(tabPanel, tab) {
                    var activeTab = tabAccSales.getActiveTab();
...
function add_btn_click()
{
                ...
                if (cmbDebitLedger.getValue() == 0 && txtTaxableValue.getValue() > 0 ) {
                    gstInsert = "false";
                    Ext.MessageBox.alert("Purchases", "Select Debit Ledger");
                }
...
//alert(seqno);
		...
		Ext.Msg.prompt('Reason for Modification', '', function(btn, text){
            ...
            Ext.MessageBox.alert("Direct Sales", "Transactions Details Not Avaliable ..");
        } else if (Number(txtTotDebit.getRawValue()) !== Number(txtTotCredit.getRawValue())) {
            Ext.MessageBox.alert("Direct Sales", "The Transactions Debit and Credit Amount are not  Equal");
        } 

      	else
        		{
        		 Ext.Msg.show({
                        title: 'Direct Sales Voucher',
                        icon: Ext.Msg.QUESTION,
                        buttons: Ext.MessageBox.YESNO,
                        ...
                        Ext.Ajax.request({
                            url: 'FrmTrnSalesAccountSave.php',
                            params: {
...
*/
    ...
    var SalesEntryFormPanel = new Ext.FormPanel({
...
//edit 
			...
			var p3 = "&vouno=" + encodeURIComponent(v3);
	                var param = (p1+p2+p3) ;
                window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepVouPrintDirectSales.rptdesign&__format=pdf&' + param, '_blank'); 
                        }
                        }
                    }
                }, '-',
 ...
 var SalesEntryWindow = new Ext.Window({
        height: 600,
        width: 1300,
        y: 40,
        title: 'Sales Account Entry',
        items: SalesEntryFormPanel,
        layout: 'fit',
        closable: true,
        minimizable: true,
        maximizable: true,
