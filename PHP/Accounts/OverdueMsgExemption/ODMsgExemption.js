...
var UserId   = localStorage.getItem('ginuserid');
 ...
 var loadODMsgExemptionListDatastore = new Ext.data.Store({
      id: 'loadODMsgExemptionListDatastore',
      autoLoad : true,
      proxy: new Ext.data.HttpProxy({
                url: 'clsODMsgExemption.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadExemptionList"}, // this parameter asks for listing
	...
	'cust_code','cust_ref','cust_overdue_msg'
...
var cmbCustomer = new Ext.form.ComboBox({
   ...
   function RefreshData(){
        ...
        loadODMsgExemptionListDatastore.removeAll();
	loadODMsgExemptionListDatastore.load({
          url:'clsODMsgExemption.php',
          params:
       	  {
           task:"loadExemptionList"
          }
	});
   };
   ...
   var flxDetail = new Ext.grid.EditorGridPanel({
       ...
       id: 'my-grid',  
		...
		{header: "OD Msg(Y/N)", dataIndex: 'cust_overdue_msg',sortable:true,width:100,align:'left'},

        ],
       store:loadODMsgExemptionListDatastore,
       ...
       listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			{
				if (selected_rows[i].data.cust_overdue_msg == 'N')
                                    flag = 'N';
                                else                                   
                                    flag = 'Y';
                                             
                               	colname = 'cust_overdue_msg';
				if (flag == 'N')
				{
   ...
   var MasODMsgExemptionpanel = new Ext.FormPanel({
        renderTo    : Ext.getBody(),
        xtype       : 'form',
        title       : 'OVER DUE MESSAGES EXEMPTION LIST',
        header      : false,
        width       : 1260,	
...
//save
				...
				else
				{
					...
					if (btn == 'yes')
                                            	...
                                            	if (obj['success'] === "true") 
						{
						    ...
						    MasODMsgExemptionpanel.getForm().reset();
							RefreshData();
                ...
                },'-',
                {
                    ...
                    handler: function(){	
                            MasOverdueMsgExemptionWindow.hide();
                        }
                }]
        },
        items: [
            { xtype   : 'fieldset',
                title   : 'OVER DUE MESSAGE EXEMPTION LIST',
                layout  : 'hbox',
                border  : true,
                height  : 100,
                width   : 600,
    ...
    var MasOverdueMsgExemptionWindow = new Ext.Window({
	height      : 600,
        width       : 1280,
        y           : 35,
        title       : 'OVER DUE MESSAGE EXEMPTION LIST',
        items       : MasODMsgExemptionpanel,
        layout      : 'fit',
...
onEsc:function(){
    ...
    MasOverdueMsgExemptionWindow.show();  
});
