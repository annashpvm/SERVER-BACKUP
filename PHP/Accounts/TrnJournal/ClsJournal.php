...
$r=mysql_query("select * from acc_ref ref  join acc_tran tran on  tran.acctran_accref_seqno = ref.accref_seqno   join massal_customer mas on  tran.acctran_led_code = mas.led_code  where accref_vouno = '$vouno' and  accref_comp_code = $compcode and accref_finid = $fincode");
 ...
 function getBillAdjustmentDetails()
    {
        ...
        $dbcr = $_POST['dbcr']; 
        if ($dbcr == "P") 
        $qry = "select * from acc_adjustments , acc_trail  where acctrail_led_code =  $ledcode  and ref_compcode = $compcode and ref_adjseqno = acctrail_accref_seqno and acctrail_amtmode = 'D' and  ref_docseqno = $seqno";
        else
        $qry = "select * from acc_adjustments , acc_trail  where  acctrail_led_code =  $ledcode  and ref_compcode = $compcode and ref_adjseqno = acctrail_accref_seqno and acctrail_amtmode = 'C' and  ref_docseqno = $seqno";
