...
function UpdateReceiptBillsAdjusted(){
        ...
        if (selrow.get('voutype')=='AD'){
            ...
            if (Number(txtTotadjamt.getRawValue())<Number(txtPartyCredit.getRawValue())){
                if(Number(txtPartyCredit.getRawValue())-Number(txtTotadjamt.getRawValue())>selrow.get('pendingamt') && selrow.get('pendingamt')>0){
                    selrow.set('adjamt',selrow.get('pendingamt'));
                }else if (selrow.get('pendingamt')>0){
                    selrow.set('adjamt',Ext.util.Format.number(Number(txtPartyCredit.getRawValue())-Number(txtTotadjamt.getRawValue()),"0.00"));
                }else{
                    selrow.set('adjamt',0);
                }
    ...
    var flxAdjdocDetail = new Ext.grid.EditorGridPanel({
        ...
        columns: [         
            ...
            {header: "Pending Amt", dataIndex: 'pendingamt',sortable:true,width:110,align:'left'},
                    ...
                    listeners:{
                        ...
                        keyup: function(){
                            ...
                            if (Number(this.getRawValue())>Number(pendingamt)){
                                ...
                                if (Number(txtTotadjamt.getRawValue())<Number(txtPartyCredit.getRawValue())){
                                    if(Number(txtPartyCredit.getRawValue())-Number(txtTotadjamt.getRawValue())>Number(this.getRawValue())){
                                        
                                    }else {
                                        this.setValue(Ext.util.Format.number(Number(txtPartyCredit.getRawValue())-Number(txtTotadjamt.getRawValue()),"0.00"));
                                    }
                                }else{
                                    this.setValue("");
                                }
                                    ...
                                    Ext.Msg.show({
		                               ...
		                               Ext.each(accadjData, function (record) {
		                                    accadjupdData.push(record.data);
                                                ...
                                                Ext.Ajax.request({
                                                    url: 'FrmTrnCreditNoteSaveNew.php',
                                                    params: {
...
//''                                                        cnt: accData.length,
					                ...
					                totaladjusted : txtTotadjamt.getRawValue(),
...
btnUpdate,
        ...
        {
            xtype: 'panel',
            title: 'Bill Adjustment Details',bodyStyle:{"background-color":"#ffffcc"},
            layout: 'absolute',
            items: [
