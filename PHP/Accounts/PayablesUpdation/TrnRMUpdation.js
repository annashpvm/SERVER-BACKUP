...
'tax_name', 'purledcode', 'purledname', 'tax_cgst_per', 'tax_sgst_per','tax_igst_per', 'tax_cgst_ledcode', 'tax_sgst_ledcode', 'tax_igst_ledcode', 
 'cgstledname', 'sgstledname', 'igstledname','tax_cgstper', 'tax_sgstper','tax_igstper', 'tax_cgstledcode', 'tax_sgstledcode', 'tax_igstledcode', 'tax_sgstledger','tax_cgstledger','tax_igstledger',
...
//	readOnly : true,
	...
	style      : "border-radius:5px;",     
        ...
        listeners:{
          keyup:function(){
                 txtDiffTCS.setValue(Number(txtPartyTCS.getValue())-Number(txtGRNTCS.getValue()));
                 flxaccupdation(); 
          }
        }   
   });
...
'rech_tcs_per', 'rech_tcs_amt', 'rech_servicecharge', 'rech_freight', 'rech_totalamount', 'rech_billno', 'rech_billdate', 'rech_roundingoff','rech_itemvalue',
...
'rect_grnqty','rect_itemrate', 'rect_partyitemcode', 'rect_freightvalue', 'rect_itemvalue', 'rect_costrate', 'itmh_name', 'itmh_ledcode','rech_crdays',
'led_name','sup_refname','sup_led_code','sup_taxcode','rect_degqty','sup_type',
        ...
        listeners:{
           select: function(){
                           	...
                           	callback:function()
				{
                                   ...
                                   rounding = loadGRNdetailsStore.getAt(0).get('rech_roundingoff');
                                   purvalue = loadGRNdetailsStore.getAt(0).get('rech_itemvalue');
                                   if (loadGRNdetailsStore.getAt(0).get('sup_type') == 1)
                                   {
                                        purledgercode = '1783';
                                        purledgername = 'WASTE PAPER -GST';                       
                                   }
                                   else
                                   {
                                        purledgercode = '1784';
                                        purledgername = 'WASTE PAPER-IGST';                       
                                   }


      txtVouNo.setRawValue('PLW'+cmbGRNNo.getRawValue()); 



                                   crdays = loadGRNdetailsStore.getAt(0).get('rech_crdays');
//alert(crdays);

                                    dtVouNo.setRawValue(Ext.util.Format.date(loadGRNdetailsStore.getAt(0).get('rech_date'),"d-m-Y"));
				    ...
				    txtBillAmt.setRawValue(loadGRNdetailsStore.getAt(0).get('rech_billvalue'));
                       		    ...
                       		    txtFreight.setRawValue(loadGRNdetailsStore.getAt(0).get('rech_freight')); 
...
/*
                                   var billcgst = Math.round((billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight'))) * Number(loadGRNdetailsStore.getAt(0).get('rech_cgst_per'))/100);
                                   var billsgst = Math.round((billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight'))) * Number(loadGRNdetailsStore.getAt(0).get('rech_sgst_per'))/100);
                                   var billigst = Math.round((billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight'))) * Number(loadGRNdetailsStore.getAt(0).get('rech_igst_per'))/100);
...
*/
                                   ...
                                   var billcgst = (billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight'))) * Number(loadGRNdetailsStore.getAt(0).get('rech_cgst_per'))/100;
                                   var billsgst = (billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight'))) * Number(loadGRNdetailsStore.getAt(0).get('rech_sgst_per'))/100;
                                   var billigst = (billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight'))) * Number(loadGRNdetailsStore.getAt(0).get('rech_igst_per'))/100;
                                  var billtcs  = ((billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight')))+billcgst+billsgst+billigst) * Number(loadGRNdetailsStore.getAt(0).get('rech_tcs_per'))/100;
                                   var billcess = ((billvalue + Number(loadGRNdetailsStore.getAt(0).get('rech_freight')))+billcgst+billsgst+billigst) * Number(loadGRNdetailsStore.getAt(0).get('rech_servicecharge'))/100;
			   	   

                                    billcgst = number = Math.round(billcgst * 100) / 100;
                                    billsgst = number = Math.round(billsgst * 100) / 100;
                                    billigst = number = Math.round(billigst * 100) / 100;
                                    billtcs  = number = Math.round(billtcs * 100) / 100;
                                    billcess = number = Math.round(billcess * 100) / 100;
		                   	...
		                   	callback:function()
					{
                                             ...
                                             cgstledgercode = LoadGSTDataStore.getAt(0).get('tax_cgstledcode');    
                                             cgstledgername = LoadGSTDataStore.getAt(0).get('tax_cgstledger');
                                             sgstledgercode = LoadGSTDataStore.getAt(0).get('tax_sgstledcode');    
                                             sgstledgername = LoadGSTDataStore.getAt(0).get('tax_sgstledger');
                                             igstledgercode = LoadGSTDataStore.getAt(0).get('tax_igstledcode');    
                                             igstledgername = LoadGSTDataStore.getAt(0).get('tax_igstledger');
                                             
                                             dncgstper = LoadGSTDataStore.getAt(0).get('tax_cgstper');
                                             dnsgstper = LoadGSTDataStore.getAt(0).get('tax_sgstper');
                                             dnigstper = LoadGSTDataStore.getAt(0).get('tax_igstper');
...
//alert(purvalue);
	      ...
	      dramt = txtPartyValue.getRawValue()-txtPartyCGST.getRawValue()-txtPartySGST.getRawValue()-txtPartyIGST.getRawValue()-txtPartyCess.getRawValue()-txtPartyTCS.getRawValue()+Number(rounding);
...
var flxAccounts = new Ext.grid.EditorGridPanel({
	...
	{header: "Ledger Name", dataIndex: 'ledname',sortable:true,width:250,align:'left'},
	{header: "Dedit",       dataIndex: 'debit',sortable:true,width:85,align:'right'},
	{header: "Credit",      dataIndex: 'credit',sortable:true,width:85,align:'right'},
...
//                                savetype:gstFlag,
    ...
    function RefreshData() {
                  VouNodatastore.load({
                        ...
                        callback: function(){
                            txtVouNo.setRawValue("PLW"+VouNodatastore.getAt(0).get('con_value'));
                        }
                    }); 
