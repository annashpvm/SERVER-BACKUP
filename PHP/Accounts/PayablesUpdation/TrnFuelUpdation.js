...
//         txttotCreditDNote.setValue(Ext.util.Format.number(Math.round(cr*100/100),'0.00'));
         ...
         txttotDebitDNote.setValue(Ext.util.Format.number(dr,'0.00'));
         txttotCreditDNote.setValue(Ext.util.Format.number(cr,'0.00'));
...
'itmh_impledcode', 'itmh_hsncode', 'itmh_cgstper', 'itmh_sgstper', 'itmh_igstper', 'itmh_sgstledcode', 'itmh_cgstledcode', 'itmh_igstledcode', 'sup_type',
'sup_code', 'sup_refname','sup_agentcode','sup_led_code','purledname', 'purledcode', 'cgstledname', 'tax_cgstledcode', 'sgstledname', 'tax_sgstledcode','rech_billno','rech_crdays', 'igstledname','tax_igstledcode' ,'led_name','rech_totalamount','sup_name','rech_handling_pmt','rech_handling_cgstamt','rech_handling_sgstamt'
,'tax_sgstledger','tax_cgstledger','tax_igstledger','tax_cgstper','tax_sgstper','tax_igstper','rech_roundingoff',
'itmh_tnpurledcode','itmh_ospurledcode','led_name','rect_purgrp'
...
*/ 
        ...
        rounding = 0;
                                   ...
                                   if (loadGRNdetailsAllStore.getAt(0).get('sup_type') == 1)
                                   {
                                        purledgercode = loadGRNdetailsAllStore.getAt(0).get('itmh_tnpurledcode');
                   
                                   }
                                   else
                                   {
                                        purledgercode = loadGRNdetailsAllStore.getAt(0).get('itmh_ospurledcode');
                                   }


                                   purledgercode = loadGRNdetailsAllStore.getAt(0).get('rect_purgrp'); 
//alert(purledgercode);
                                   purledgername = loadGRNdetailsAllStore.getAt(0).get('led_name');    

//alert(loadGRNdetailsAllStore.getAt(0).get('rech_roundingoff'));
                                   rounding = loadGRNdetailsAllStore.getAt(0).get('rech_roundingoff');
//alert(rounding);
                                   txtVouNo.setRawValue('PPF'+cmbGRNNo.getRawValue()); 

                                   txtSupplier.setRawValue(loadGRNdetailsAllStore.getAt(0).get('sup_name'));
                                   txtGRNAmt.setValue(loadGRNdetailsAllStore.getAt(0).get('rech_totalamount'));

                                   crdays = loadGRNdetailsAllStore.getAt(0).get('rech_crdays');
//alert(crdays);

                                    dtGRNNo.setRawValue(Ext.util.Format.date(loadGRNdetailsAllStore.getAt(0).get('rech_date'),"d-m-Y"));
                                    dtVouNo.setRawValue(Ext.util.Format.date(loadGRNdetailsAllStore.getAt(0).get('rech_date'),"d-m-Y"));
				    ...
				    txtGRNIGST.setValue(loadGRNdetailsAllStore.getAt(0).get('rech_igst_amt')); 
		                    ...
		                    txtGRNValue2.setValue(Ext.util.Format.number(loadGRNdetailsAllStore.getAt(0).get('rech_totalamount'),"0.00")); 

                                    var cnt=loadGRNdetailsAllStore.getCount();


                                    cgstledgercode = loadGRNdetailsAllStore.getAt(0).get('tax_cgstledcode');    
                                    cgstledgername = loadGRNdetailsAllStore.getAt(0).get('tax_cgstledger');
                                    sgstledgercode = loadGRNdetailsAllStore.getAt(0).get('tax_sgstledcode');    
                                    sgstledgername = loadGRNdetailsAllStore.getAt(0).get('tax_sgstledger');
                                    igstledgercode = loadGRNdetailsAllStore.getAt(0).get('tax_igstledcode');    
                                    igstledgername = loadGRNdetailsAllStore.getAt(0).get('tax_igstledger');
                                             
                                    dncgstper = loadGRNdetailsAllStore.getAt(0).get('tax_cgstper');
                                    dnsgstper = loadGRNdetailsAllStore.getAt(0).get('tax_sgstper');
                                    dnigstper = loadGRNdetailsAllStore.getAt(0).get('tax_igstper');
/*
...
*/
	...
	//			   txtHandlingAmount.setValue(loadGRNdetailsAllStore.getAt(0).get('rech_handling_pmt'));
...
*/
                                   ...
                                   billcgst = (billvalue * Number(loadGRNdetailsAllStore.getAt(0).get('rech_cgst_per'))/100);
                                   billsgst = (billvalue * Number(loadGRNdetailsAllStore.getAt(0).get('rech_sgst_per'))/100);
                                   billigst = (billvalue * Number(loadGRNdetailsAllStore.getAt(0).get('rech_igst_per'))/100);
                                   billtcs  = ((billvalue+billcgst+billsgst+billigst) * Number(loadGRNdetailsAllStore.getAt(0).get('rech_tcs_per'))/100);


                                   billcgst = Math.round(billcgst * 100) / 100;
                                   billsgst = Math.round(billsgst * 100) / 100;
                                   billigst = Math.round(billigst * 100) / 100;
                                   billtcs = Math.round(billtcs * 100) / 100;
			   	    ...
			   	    grntcs = Math.round(grntcs * 100) / 100;   
...
//alert(grntcs);
//alert(grncess);
				   txtHandlingAmount.setValue(loadGRNdetailsAllStore.getAt(0).get('rech_handling_pmt')*grnqty);

//                 hcgst = Ext.util.Format.number(txtHandlingAmount.getRawValue() * txtHandlingcgstper.getRawValue()/100,"0.00");
//                 hsgst = Ext.util.Format.number(txtHandlingAmount.getRawValue() * txtHandlingsgstper.getRawValue()/100,"0.00");


                 hcgst = Ext.util.Format.number(loadGRNdetailsAllStore.getAt(0).get('rech_handling_cgstamt'),"0.00");
                 hsgst = Ext.util.Format.number(loadGRNdetailsAllStore.getAt(0).get('rech_handling_sgstamt'),"0.00");


                 txtHandlingcgstamt.setValue(hcgst);
                 txtHandlingsgstamt.setValue(hsgst);


                                    txtGRNWt.setValue(Ext.util.Format.number(grnqty,"0.000")); 
                                    txtGRNWt2.setValue(Ext.util.Format.number(grnqty,"0.000")); 
...
//Party Account - Credit
       ...
       if (Number(rounding) > 0)
	      ...
	      dramt = Number(txtPartyValue.getRawValue())-Number(txtPartyCGST.getRawValue())-Number(txtPartySGST.getRawValue())-Number(txtPartyIGST.getRawValue())-Number(hcgst)-Number(hcgst)- Number(txtPartyCess.getRawValue()) - Number(txtHandlingAmount.getValue())+Number(rounding);
       else
	      dramt = Number(txtPartyValue.getRawValue())-Number(txtPartyCGST.getRawValue())-Number(txtPartySGST.getRawValue())-Number(txtPartyIGST.getRawValue())-Number(hcgst)-Number(hcgst)- Number(txtPartyCess.getRawValue()) - Number(txtHandlingAmount.getValue())-Number(rounding);


              dramt = Math.round(dramt * 100) / 100;
...
//Purchase Account - Debit
        ...
        flxAccounts.getStore().insert(
	      ...
	      debit     : Ext.util.Format.number(dramt,"0.00"),
              credit    : 0,
              billno    : txtInvNo.getRawValue(),
              billdt    : Ext.util.Format.date(dtInvNo.getValue(),"Y-m-d"),
              ledtype   : "G",
...
//HANDLING Account - Debit
         ...
         if (txtHandlingAmount.getValue() > 0 )
        {
		var RowCnt1 = flxAccounts.getStore().getCount() + 1;
		flxAccounts.getStore().insert(
		  flxAccounts.getStore().getCount(),
		  new dgrecord({
		      ...
		      debit     : txtHandlingAmount.getRawValue(),
		      credit    : 0,
              billno    : txtInvNo.getRawValue(),
              billdt    : Ext.util.Format.date(dtInvNo.getValue(),"Y-m-d"),
                      ledtype   : "G",
    ...
    function RefreshData() {
        ...
        flxAccountsDNote.getStore().removeAll();
                    VouNodatastore.load({
                        ...
                        callback: function(){
                            txtVouNo.setRawValue("PPF"+VouNodatastore.getAt(0).get('con_value'));
