...
$r=mysql_query("  select DATE_FORMAT( reth_date, '%d-%m-%Y') as retdate, reth_date, round(sum(reth_totwt)/1000,3) as retweight,sum(reth_amt) as retamount from trnsal_salret_header where  reth_comp_code = '$compcode'  and reth_fincode ='$finid' and reth_date between '$startdate' and  '$enddate' group by reth_date order by reth_date desc");
...
$r=mysql_query("select cust_ref,reth_invno,round(reth_totwt/1000,3) as retweight,reth_amt as retamount, 
...
$r=mysql_query("select var_bf,var_gsm,var_name, round(sum(rett_wt)/1000,3) retwt , rett_urate from trnsal_salret_header a, trnsal_salret_trailer b , massal_variety c , masprd_variety d where c.var_grpcode = d.var_groupcode and b.rett_var = c.var_code and  a.reth_no = b.rett_no and a.reth_fincode = b.rett_fincode  
