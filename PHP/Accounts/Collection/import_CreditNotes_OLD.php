...
$skip = 8;
        ...
        while (($packData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            { 
              ...
              $colJ = strtoupper(trim($colJ));
              $colJ = trim(str_replace(" ", "", $colJ));
...
//echo "<p> $colJ.. $colE <br /></p>\n";


            if (colC  != '' && $colJ == 'CREDITNOTE' && $colE  > 0 )    
            { 
		$query1   = "select count(*) as recfound from acc_collections where c_date = '$colA' and  c_party = '$colB' and c_invno = '$colC' and c_collamt = $colE and c_type ='C'";
...
//echo $query1;
		...
		$recfound = $rec1['recfound'];
		  ...
		  $query1 = "insert into acc_collections (c_date, c_invno, c_collamt,c_party,c_type) values ( '$colA',  '$colC','$colE','$colB','C')";
	...
	$query1   = "select * from acc_collections where c_upd = 'N' and c_type ='C'";
	$result1  = mysql_query($query1);
	      ...
	      $collamt =  $row['c_collamt'];
	       ...
	       if ($insert->execute()) {
	          $cnnt=$cnnt+1;
               ...
               $update = $pdo->prepare("update acc_collections set c_upd = 'Y' where c_invno = '$invno' and c_type ='C'");
		...
		<div class="row">
			...
			<div class="span6" id="form-login">
				<form class="form-horizontal well"  action="" method="POST" name="upload_excel" enctype="multipart/form-data">
					<fieldset>
						<center><legend>Excel Upload - for Credit Note (Bills) </legend></center>
