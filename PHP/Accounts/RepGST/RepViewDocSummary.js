...
var filename = "/SHVPM/Report/DOCSUMMARY_"+month+"_"+year+".json";
		      ...
		      {
//			 var redirectWindow = window.open('http://10.0.0.251/SHVPM/Report/docsummary.json', '_blank');
...
style: {
        ...
        listeners: {
		   ...
		   var p2 = "&fincode=" + encodeURIComponent(GinFinid);
 	 	   ...
 	 	   var param = (p1 + p2 + p3 + p4);   
                   ...
                   if (printtype == "PDF") 
		   window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepDocumentSummary.rptdesign&__format=pdf'+ param); 
	           else if (printtype == "XLS") 
		   window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepDocumentSummary.rptdesign&__format=xlsx'+ param); 
                   else
		   window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepDocumentSummary.rptdesign'+ param); 
   ...
   var btnDocSummaryDetailPrint = new Ext.Button({
        style   : 'text-align:center;',
        text    : "DOCUMENT SUMMARY DETAIL PRINT",
        width   : 210,
        id      :'btnDocSummaryDetailPrint',
        x       : 10,
        y       : 200,
	border: 1,
	style: {
	...
	borderStyle: 'solid',
        ...
        listeners: {
		   ...
		   var p2 = "&fincode=" + encodeURIComponent(GinFinid);
                   var p3 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-  d"));	
                   var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
	           var p5 = "&voutype=" + encodeURIComponent(voutype);


 	 	   var param = (p1 + p2 + p3 + p4 + p5 );   
		   window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepDocumentSummary_Detailed.rptdesign&__format=pdf'+ param); 
...
labelStyle  : "font-size:14px;font-weight:bold;color:#fc9403",
 ...
 var loadDayBookDatastore = new Ext.data.Store({
   ...
   var loadDocumentSummaryListDataStore = new Ext.data.Store({
      id: 'loadDocumentSummaryListDataStore',
      proxy: new Ext.data.HttpProxy({
                url:'ClsViewStatements.php',    // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadDocumentSummary"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
	...
	'doc_type','saltype', 'salestype', 'from_no', 'to_no', 'totalnos', 'cancelled', 'netissues'
      ]),
    });


   var loadDocSummaryInvDataStore = new Ext.data.Store({
      id: 'loadDocSummaryInvDataStore',
      proxy: new Ext.data.HttpProxy({
                url:'ClsViewStatements.php',    // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadDocSummaryInvoiceDetails"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
//    alert(monthenddate);  
	...
	loadDocumentSummaryListDataStore.removeAll();
	loadDocumentSummaryListDataStore.load({
	 url: 'ClsViewStatements.php',
                params: {
	    	task: 'loadDocumentSummary',
                compcode:Gincompcode,
                finid:GinFinid,
...
['7','JULY'],['8','AUGUST'],['9','SEPTEMBER'],[10,'OCTOBER'],['11','NOVEMBER'],['12','DECEMBER']],
        ...
        listeners:{
           select: function(){
                  ...
                  loadDocSummaryInvDataStore.removeAll();
...
var lblDocumentDetail = new Ext.form.Label({
    fieldLabel  : '',
    id          : 'lblDocumentDetail',
    width       : 700,
    labelStyle  : "font-size:14px;font-weight:bold;color:#0080ff",
	style: {
            'color':'#001a66',
...
var flxSummaryList = new Ext.grid.EditorGridPanel({
    frame: false,
    sm: new Ext.grid.RowSelectionModel(),
    stripeRows : true,
    scrollable: true,
    ...
    columns:
    [ 	 
        {header: "DOC TYPE", dataIndex: 'doc_type',sortable:true,width:250,align:'left',hidden:false}, 
        {header: "SAL TYPE", dataIndex: 'saltype',sortable:true,width:100,align:'left',hidden:false},
        {header: "Nature of Document ", dataIndex: 'salestype',sortable:true,width:200,align:'left',hidden:false},
        {header: "From Number", dataIndex: 'from_no',sortable:true,width:150,align:'center'},	
        {header: "TO Number" ,  dataIndex: 'to_no',sortable:false,width:150,align:'center', menuDisabled: true},
        {header: "TOAL Nos", dataIndex: 'totalnos',sortable:true,width:100,align:'center'},	
        {header: "Cancelled", dataIndex: 'cancelled',sortable:true,width:100,align:'center'},	
        {header: "NET ISSUES" , dataIndex: 'netissues',sortable:false,width:100,align:'center', menuDisabled: true},
    ],

    store:loadDocumentSummaryListDataStore,
    listeners:{	

            'cellclick': function (flxSummaryList, rowIndex, cellIndex, e) {
		var sm = flxSummaryList.getSelectionModel();
...
//          	find_dates(selrow.get('cust_code'));
            ...
            lblDocumentDetail.setText("Document List for : " + selrow.get('saltype'));
	    loadDocSummaryInvDataStore.removeAll();
	    loadDocSummaryInvDataStore.load({
		url: 'ClsViewStatements.php',
		params: {
	    	task: 'loadDocSummaryInvoiceDetails',
                compcode:Gincompcode,
                finid   :GinFinid,
                startdate: Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"), 
                enddate: Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"), 
...
var flxInvoiceList = new Ext.grid.EditorGridPanel({
  ...
  //  id: 'my-grid',  
    ...
    store:loadDocSummaryInvDataStore,
    listeners:{	
     ...
     } ,
    {
        ...
        items: [
			...
			{ 
		             ...
		             x       : 1000,
			     y       : 20,
                             items: [btnDocSummaryDetailPrint]
                        },
