                                            ...
                                            Ext.MessageBox.show({
                                                ...
                                                fn: function (btn) {
                                                    if (btn === 'yes') {
                                                        ...
                                                        var accadjData = flxAdjustDetailsfinal.getStore().getRange();
...
//        txtRecAmt.setValue(Number(txtTotCredit.getRawValue()) - Number(txtTotDebit.getRawValue()));
        ...
        for (var i = 0; i < rcnt; i++) {
            ...
            if (Number(txtTotAdjAmt.getRawValue()) < Number(ledgerdebit)) {



                if (Number(ledgerdebit) - Number(txtTotAdjAmt.getRawValue()) > selrow.get('PendAmount') && selrow.get('PendAmount') > 0) {
                    selrow.set('Adjusted', selrow.get('PendAmount'));
                } else if (selrow.get('PendAmount') > 0) {
                    selrow.set('Adjusted', Number(ledgerdebit) - Number(txtTotAdjAmt.getRawValue()));
                } else {
...
*/
  ...
  //                     Ext.getCmp('editchk').show();
        ...
        flxDetail.getStore().removeAll();
        flxAdjustDetails.getStore().removeAll();
        flxAdjustDetailsfinal.getStore().removeAll();
...
function grid_move() {
    // Get selected records from source grid
    var selectedRecords = flxAdjustDetails.getSelectionModel().getSelections();
    let ledgerCodesToRemove = new Set();

    // Collect ledger codes from selected records
    selectedRecords.forEach((record) => {
        ledgerCodesToRemove.add(Number(record.data.ledcode));
    });

    // Remove records from final grid store if their ledcode matches any selected
    let finalStore = flxAdjustDetailsfinal.getStore();


    for (let i = finalStore.getCount() - 1; i >= 0; i--) {
        let rec = finalStore.getAt(i);
       let recCode = Number(rec.get('ledcode'));

	       if (ledgerCodesToRemove.has(recCode))  {
// alert(rec.get('ledcode'));
        ...
        if (Number(recData.Adjusted) > 0) {
            ...
            for (var j = 0; j < finalStore.getCount(); j++) {
                ...
                if (
                    ...
                    existing.set('Adjusted', Number(recData.Adjusted) );
                    exists = true;
                    break;
                }
            }
            ...
            if (!exists) {
                ...
                finalStore.add(new dgrecord({
                    ...
                    PendAmount: Number(recData.PendAmount),
                    Adjusted: Number(recData.Adjusted),
...
CalcSum();
    ...
    var flxAdjustDetails = new Ext.grid.EditorGridPanel({
        ...
        columns: [
            {header: "Led Name", dataIndex: 'ledname', sortable: true, width: 80, align: 'left', hidden: true},
            {header: "Ledcode", dataIndex: 'ledcode', sortable: true, width: 80, align: 'left', hidden: true},
            ...
            {header: "Inv.Amount", dataIndex: 'InvAmt', sortable: true, width: 100, align: 'right'},
            {header: "DN/CN", dataIndex: 'DNCN', sortable: true, width: 75, align: 'right', hidden: true},
            {header: "Total Amount", dataIndex: 'TotAmt', sortable: true, width: 100, align: 'right', hidden: true},
            {header: "Pending Amount", dataIndex: 'PendAmount', sortable: true, width: 100, align: 'right'},
                ...
                editor: {
                    ...
                    listeners: {
                     ...
                     focus: function () {
                        ...
                        keyup: function () {
//flxAdjustDetails.getSelectionModel().clearSelections();
                            var sm = flxAdjustDetails.getSelectionModel();
                            var selrow = sm.getSelected();
                            var pendingamt = 0;
...
alert(pendingamt);
                            if (Number(this.getRawValue()) > Number(pendingamt)) {
                                ...
                                if (Number(txtTotAdjAmt.getRawValue()) < Number(ledgerdebit)) {
                        if (Number(ledgerdebit) - Number(txtTotAdjAmt.getRawValue()) > Number(this.getRawValue())) {

                                    } else {
                                        this.setValue(Number(ledgerdebit) - Number(txtTotAdjAmt.getRawValue()));
                                    }
                                } else {
                                    this.setValue("");
                                }
...
grid_move();
            ...
            {header: "Balance", dataIndex: 'Balance', sortable: true, width: 100, align: 'left',
                renderer: function (v, params, record) {
                    ...
                    if (Number(record.data.Adjusted) > 0) {
                        if (Number(record.data.PendAmount) >= Number(record.data.Adjusted))
                           retval = Number(record.data.PendAmount) - Number(record.data.Adjusted);
                        else
                        {
                           retval = 0;
                           record.data.Adjusted = 0;
                        } 
                    ...
                    return Ext.util.Format.number(Number(retval),'0.00');
                }
            },
    ...
    var flxAdjustDetailsfinal = new Ext.grid.EditorGridPanel({
        frame: false,
        fieldLabel: '',
        sm: new Ext.grid.RowSelectionModel(),
        autoShow: true,
       ...
       renderTo: Ext.getBody(),
        columns: [
            ...
            {header: "Ledcode", dataIndex: 'ledcode', sortable: true, width:50, align: 'left', hidden: false},
            ...
            {header: "Pending Amount", dataIndex: 'PendAmount', sortable: true, width: 10, align: 'left', hidden: true},
            {header: "Adjusted", dataIndex: 'Adjusted', sortable: true, width: 100, align: 'right'},
            ...
            {header: "seqno", dataIndex: 'Billseqno', sortable: true, width: 80, align: 'left',hidden : false},
...
{
        flxAdjustDetails.getEl().mask("Disabled - You can change to ADVANCE / OTHERS option and SAVE the record. ");  
        deletechk = 1;
}  
else
{
     flxAdjustDetails.getEl().unmask();  
     deletechk = 0;
       ...
       LoadAdjustmentDetailsdatastore.load({
		...
		for (var i = 0; i < selrows; i++) {
		        ...
		        {  
			        ...
			        flxAdjustDetailsfinal.getStore().insert(
	                        flxAdjustDetailsfinal.getStore().getCount(),
	                        new dgadjrecord({
                                    ledname : ledgername,
                                    ledcode : ledgercode, 
                                    Vocno: adjvouno,
...
flxLedger.hide();
    ...
    //    txtNarration.setRawValue("");
        ...
        for (var i = 0; i < selrows; i++) {
            var rec = flxAdjustDetails.getStore().getAt(i);
            if (Number(rec.get('Adjusted')) > 0 && Number(rec.get('ledcode'))== ginledcode  ) {
                if (rec.get('Type') !== "AD") {
    ...
    //    txtDebit.setValue(ginadjtotal);
        ...
        var selrows1 = flxAdjustDetailsfinal.getStore().getCount();
        var ginadjtotal1 = 0;
        txtBillAdjAmt.setValue("");

        for (var i = 0; i < selrows1; i++) {
            var rec = flxAdjustDetailsfinal.getStore().getAt(i);
...
flxAdjustDetails.getSelectionModel().clearSelections();
//flxAdjustDetailsfinal.getSelectionModel().clearSelections();
    }
