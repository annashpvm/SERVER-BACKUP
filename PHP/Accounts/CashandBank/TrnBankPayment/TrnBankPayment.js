                                            ...
                                            Ext.MessageBox.show({
                                                ...
                                                fn: function (btn) {
                                                    if (btn === 'yes') {
                                                        ...
                                                        var accadjData = flxAdjdocDetailfinal.getStore().getRange();
...
/*
                if (gstPaytype === "BB") {
                    ...
                    val = "";
                    var RowCnt = flxAdjdocDetail.getStore().getCount();
                    flxAdjdocDetail.getSelectionModel().selectAll();
                    var sel1 = flxAdjdocDetail.getSelectionModel().getSelections();
...
sel[idx].set('ledseq'  , ledgercode);
			...
			flxDetail.getSelectionModel().clearSelections();
                        ...
                        ledgerdebit = Number(txtDebit.getValue());
...
function grid_chk_flxLedger()
{
	...
	if ((selrow != null)){
                ...
                ginledcode =selrow.get('cust_code');
		ledgercode = selrow.get('cust_code');
		ledtype    = selrow.get('cust_type');
                ledgername = selrow.get('cust_ref');
...
'cust_name', 'led_addr1', 'led_addr2','cust_type', 'led_custcode' ,'cust_acc_group','acctran_narration'])
    });
   ...
   var findLedgerdatastore = new Ext.data.Store({
    ...
    var PaymentBilldetailsDataStore = new Ext.data.Store({
        ...
        reader: new Ext.data.JsonReader({
            ...
            'acctrail_inv_no', 'acctrail_inv_date', 'acctrail_inv_value', 'acctrail_adj_value', 'acctran_cramt', 'acctran_totamt', 'acctran_led_code', 'dbcr_invvalue', 'dbcr_adjvalue', 'dbcr_ledcode', 'dbcr_invno', 'dbcr_invdate', 'dbcr_compcode','dbcr_finid','dbcr_type', 'acctrail_led_code','cust_ref'
        ])
    });
                ...
                if (cnt1 > 0) {
                    for (var i = 0; i < cnt1; i++) {
                        ...
                        if (acctrail_inv_value > 0) {
                            ...
                            ledgername  = PaymentBilldetailsDataStore.getAt(i).get('cust_ref');
                            if (acctrailadjvalue !== acctrail_inv_value) {
                                ...
                                ledgercode = PaymentBilldetailsDataStore.getAt(i).get('acctrail_led_code');
    ...
    function calc_sum() {
        ...
        for (var i = 0; i < rowcnt; i++) {
            if (Number(sel1[i].data.Adjusted) > 0) {
                if (sel1[i].data.Type !== "AD" && sel1[i].data.ledcode == ginledcode  ) {
...
function UpdateReceiptBillsAdjusted() {
    ...
    if (selrow.get('voutype') !== 'AD') {
        var totalAdj = Number(txtTotAdjAmt.getRawValue());
        var totalDebit = Number(txtTotDebit.getRawValue());
        var pending = Number(selrow.get('pendingamt'));

        if (totalAdj < totalDebit) {
            var available = totalDebit - totalAdj;
            if (available > pending && pending > 0) {
                selrow.set('adjamt', pending);
            } else if (pending > 0) {
                selrow.set('adjamt', Ext.util.Format.number(available, "0.00"));
            } else {
                selrow.set('adjamt', 0);
            }
            selrow.set('balamt', pending - Number(selrow.get('adjamt')));
            CalcSum();
        }
    }
}
...
var flxAdjdocDetail = new Ext.grid.EditorGridPanel({
    ...
    store: new Ext.data.ArrayStore({
        fields: ['ledname', 'ledcode', 'Vocno', 'InvNo', 'InvDate', 'InvAmt', 'DNCN', 'TotAmt', 'pendingamt', 'adjamt', 'balamt', 'voutype', 'Type', 'Billseqno']    }),
    sm: new Ext.grid.RowSelectionModel(),
    autoShow: true,
    stripeRows: true,
    scrollable: true,
    ...
    columns: [
        ...
        { header: "DN/CN", dataIndex: 'DNCN', hidden: true },
        { header: "Total Amount", dataIndex: 'TotAmt', hidden: true },
        { header: "Pending Amt", dataIndex: 'pendingamt', width: 110, align: 'right'},
        {header: "Adjusted", dataIndex: 'adjamt', width: 110, align: 'right', editor: {
                xtype: 'numberfield',
                allowBlank: true,
                enableKeyEvents: true,
                listeners: {
			...
			focus: function () {
			    ...
			    var pending = Number(selrow.get('pendingamt')) || 0;
			    var existingAdj = Number(selrow.get('adjamt')) || 0;

			    var totalAdj = Number(txtTotAdjAmt.getRawValue()) || 0;
			    var totalDebit =  Number(ledgerdebit); //  Number(txtTotDebit.getRawValue()) || 0;
			    var available = totalDebit - (totalAdj - existingAdj); // consider old value

			    // Only set if adjamt is blank or zero
			    if (existingAdj === 0) {
				var setValue = Math.min(pending, available);
				selrow.set('adjamt', setValue);
				this.setValue(setValue);
				selrow.set('balamt', pending - setValue);
				CalcSum();
			    }

			    // Store original value for future subtraction
			    this.originalValue = existingAdj;
			    txtTotAdjAmt.setValue(totalAdj - existingAdj);
                            grid_move();
			},
                     blur: function () {
                        CalcSum();
        ...
        {
            header: "Balance", dataIndex: 'balamt', width: 90, align: 'right',
            renderer: function (v, params, record) {
                var adj = Number(record.data.adjamt) || 0;
                var pending = Number(record.data.pendingamt) || 0;
                return Ext.util.Format.number(pending - adj, "0.00");
            }
        },
        { header: "Type", dataIndex: 'Type', width: 60 },
        { header: "seqno", dataIndex: 'Billseqno', hidden: true }
    ]
});

// 🔒 Hard validation — reject invalid edits before they save to the store
flxAdjdocDetail.on('validateedit', function (e) {
    if (e.field === 'adjamt') {
        var pending = Number(e.record.get('pendingamt')) || 0;
        var input = Number(e.value) || 0;

        if (input > pending) {
            Ext.Msg.alert("Bill Adjustment", "Adjusted amount cannot be greater than pending amount.");
            return false;
        }

        var totalAdj = Number(txtTotAdjAmt.getRawValue()) || 0;
        var debit = Number(txtTotDebit.getRawValue()) || 0;
        var oldValue = Number(e.record.get('adjamt')) || 0;
        var available = debit - (totalAdj - oldValue);

        if (input > available) {
            Ext.Msg.alert("Bill Adjustment", "Adjusted amount exceeds available debit.");
            return false;
        }

        e.record.set('balamt', pending - input);
        return true;
    }
});
  ...
  //                     Ext.getCmp('editchk').show();
        ...
        flxDetail.getStore().removeAll();
        flxAdjdocDetail.getStore().removeAll();
        flxAdjdocDetailfinal.getStore().removeAll();
...
//alert(LoadVouNoDetailsdatastore.getAt(j).get('cust_acc_group'));
//                    if (Number(LoadVouNoDetailsdatastore.getAt(j).get('cust_acc_group')) != 28 &&    Number(LoadVouNoDetailsdatastore.getAt(j).get('cust_acc_group')) != 42)
                    if (Number(LoadVouNoDetailsdatastore.getAt(j).get('cust_acc_group')) != cmbHeadAccount.getValue())
                                      {
                                     ledtype = LoadVouNoDetailsdatastore.getAt(j).get('cust_type'); 
                                     ledgerdebit = LoadVouNoDetailsdatastore.getAt(j).get('acctran_dbamt'); 
                                        ...
                                        if (cnt > 1) {
                                            RecPayDataStore.load({
                                                ...
                                                callback: function () {
                                                    ...
                                                    var RowCnt = flxAdjdocDetail.getStore().getCount() + 1;
                                                    flxAdjdocDetail.getStore().insert(
                                                            flxAdjdocDetail.getStore().getCount(),
                                                            new dgrecord({
                                                                recpay_ref_no: InvNo,
...
function grid_move() {
    const selectedRecords = flxAdjdocDetail.getSelectionModel().getSelections();
    const finalStore = flxAdjdocDetailfinal.getStore();

    // 1. Get selected ledcodes (convert to numbers)
    const ledgerCodesToRemove = new Set();
    selectedRecords.forEach((record) => {
        const led = Number(record.get('ledcode'));
        ledgerCodesToRemove.add(led);
    });

   // console.log("Ledger codes to remove:", Array.from(ledgerCodesToRemove));

    // 2. Remove records from finalStore with matching ledcode
    for (let i = finalStore.getCount() - 1; i >= 0; i--) {
        const rec = finalStore.getAt(i);
        const recCode = Number(rec.get('ledcode'));
//     console.log("TEST ledcode:", recCode);
        if (ledgerCodesToRemove.has(recCode)) {
   //s         console.log("REMOVING record with ledcode:", recCode);
            finalStore.remove(rec);
        }
    }

    // 3. Add adjusted records to finalStore
    flxAdjdocDetail.getSelectionModel().selectAll();
    const sel = flxAdjdocDetail.getSelectionModel().getSelections();

    for (let i = 0; i < sel.length; i++) {
        const recData = sel[i].data;

        if (Number(recData.adjamt) >= 0) {
            let exists = false;

            for (let j = 0; j < finalStore.getCount(); j++) {
                const existing = finalStore.getAt(j);

                if (
                    Number(existing.get('ledcode')) === Number(recData.ledcode) &&
                    existing.get('InvNo') === recData.InvNo
                ) {
                    existing.set('Adjusted', Number(recData.adjamt));
                    exists = true;
                    break;
                }
            }
            ...
            if (!exists) {
                finalStore.add(new dgrecord({
                    ...
                    TotAmt: Number(recData.TotAmt),
                    pendingamt: Number(recData.pendingamt),
                    Adjusted: Number(recData.adjamt),
                    Balance: Number(recData.Balance),
                    Billseqno: Number(recData.Billseqno),
                    Type: recData.Type,
                }));
            }
        }
    ...
    var flxAdjdocDetailfinal = new Ext.grid.EditorGridPanel({
        frame: false,
        fieldLabel: '',
        sm: new Ext.grid.RowSelectionModel(),
        autoShow: true,
       ...
       renderTo: Ext.getBody(),
        columns: [
            ...
            {header: "DN/CN", dataIndex: 'DNCN', sortable: true, width: 75, align: 'left', hidden: true, hidden: true},
            {header: "Total Amount", dataIndex: 'TotAmt', sortable: true, width: 100, align: 'left', hidden: true, hidden: true},
            {header: "Pending Amount", dataIndex: 'pendingamt', sortable: true, width: 10, align: 'left', hidden: true},
            {header: "Adjusted", dataIndex: 'Adjusted', sortable: true, width: 100, align: 'right'},
    ...
    var txtBillAdjAmt = new Ext.form.NumberField({
       ...
       LoadAdjustmentDetailsdatastore.load({
          ...
          callback: function () {
              ...
              if (cnt>0)
              {
                ...
                ginledcode =  LoadAdjustmentDetailsdatastore.getAt(0).get('acctrail_led_code');
...
if (gstFlag == "Edit" && noofledgers > 1)
{
        flxAdjdocDetail.getEl().mask("Disabled - You can change to ADVANCE / OTHERS option and SAVE the record. ");  
        deletechk = 1;
}  
else
{
     flxAdjdocDetail.getEl().unmask();  
     deletechk = 0;
    ...
    flxAdjdocDetail.getStore().removeAll();
       ...
       LoadAdjustmentDetailsdatastore.load({
		...
		for (var i = 0; i < selrows; i++) {
		        ...
		        if  (Number(adjusted) >  0 )
		        {  

			        flxAdjdocDetailfinal.getStore().insert(
	                        flxAdjdocDetailfinal.getStore().getCount(),
	                        new dgadjrecord({
                                    ledname : ledgername,
                                    ledcode : ledgercode, 
                                    Vocno: adjvouno,
...
flxLedger.hide();
    ...
    //    txtNarration.setRawValue("");
        ...
        for (var i = 0; i < selrows; i++) {
            var rec = flxAdjdocDetail.getStore().getAt(i);
            if (Number(rec.get('adjamt')) > 0 && Number(rec.get('ledcode'))== ginledcode  ) {
                if (rec.get('Type') !== "AD") {
    ...
    //    txtDebit.setValue(ginadjtotal);
        ...
        var selrows1 = flxAdjdocDetailfinal.getStore().getCount();
        var ginadjtotal1 = 0;
        txtBillAdjAmt.setValue("");

        for (var i = 0; i < selrows1; i++) {
            var rec = flxAdjdocDetailfinal.getStore().getAt(i);
            ginadjtotal1 = ginadjtotal1 + Number(rec.get('Adjusted'));
/*
...
*/
        ...
        txtBillAdjAmt.setRawValue(Ext.util.Format.number(ginadjtotal1, '0.00'));
//grid_move();
flxAdjdocDetail.getSelectionModel().clearSelections();
//flxAdjdocDetailfinal.getSelectionModel().clearSelections();
    }
