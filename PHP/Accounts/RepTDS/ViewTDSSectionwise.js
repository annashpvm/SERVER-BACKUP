...
style: {
   ...
   var btnSectionPrint = new Ext.Button({
        style   : 'text-align:center;',
        text    : "DATEWISE PRINT",
        width   : 180,
        id      :'btnSectionPrint',
 ...
 var loadDayBookDatastore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        totalProperty: 'total',
        id: 'id' 
      }, [ 'sno' ,'accref_voudate', 'cust_name', 'voutype', 'accref_payref_no', 'accref_vouno', 'acctran_dbamt', 'acctran_cramt', 'acctran_cust_code', 'accref_seqno'
...
labelStyle  : "font-size:14px;font-weight:bold;color:#fc9403",
   ...
   var loadTDSGroupDataStore = new Ext.data.Store({
      id: 'loadTDSGroupDataStore',
      proxy: new Ext.data.HttpProxy({
                url:'ClsViewStatements.php',    // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadTDSGroupList"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
   ...
   var loadTDSListDataStore = new Ext.data.Store({
	...
	'eh_expvouno', 'eh_voudate2',  'partyledname','cust_panno', 'eh_refno', 'eh_refdate2','billamt','taxable','cgstamt', 'sgstamt', 'igstamt', 'ledgername', 'tdsvalue','et_tdsper', 'tdsamount'
      ]),
    });
...
//    alert(monthenddate);  
	...
	loadTDSGroupDataStore.removeAll();
	loadTDSGroupDataStore.load({
	 url: 'ClsViewStatements.php',
                params: {
	    	task: 'loadTDSGroupList',
...
var flxTDSGroup = new Ext.grid.EditorGridPanel({
    frame: false,
    sm: new Ext.grid.RowSelectionModel(),
    stripeRows : true,
    scrollable: true,
    ...
    columns:
    [ 	 
        {header: "Ledger Code", dataIndex: 'cust_code',sortable:true,width:50,align:'left',hidden:true},
        {header: "TDS Ledger", dataIndex: 'cust_name',sortable:true,width:250,align:'left',hidden:false},
        {header: "Amount", dataIndex: 'tdsamount',sortable:true,width:100,align:'right',hidden:false},


    ],


    store:loadTDSGroupDataStore,
    listeners:{	

       'cellclick': function (flxTDSGroup, rowIndex, cellIndex, e) {
		var sm = flxTDSGroup.getSelectionModel();
...
var flxTDSDatewise = new Ext.grid.EditorGridPanel({
    ...
    [ 	 
        ...
        {header: "PAN" ,  dataIndex: 'cust_panno',sortable:false,width:110,align:'center', menuDisabled: true},
        ...
        {header: "TDS Leger" , dataIndex: 'ledgername',sortable:false,width:200,align:'right'},
