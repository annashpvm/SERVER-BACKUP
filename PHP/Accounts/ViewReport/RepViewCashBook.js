...
function grid_tot(){
       ...
       for(var i=0;i<Row;i++)
...

        {
         ...
         txtTotalClosing2.setRawValue(Ext.util.Format.number( txtOpening.getValue()+dr-cr,'0.00'));
   

                       var clo = txtOpening2.getValue()+dr-cr; 
...
var loop = new Date(monthstartdate.getValue()); // Clone the start date to avoid modifying the original
...
style:{
        ...
        columns: [
            ...
            {header: "Date ", dataIndex: 'accref_voudate',width:110,align:'left',sortable: true,menuDisabled: false},
            {header: "Account Name", dataIndex: 'cust_name',width:300,align:'left',sortable: true,defaultSortable: true,menuDisabled: false,
		renderer : function(value, meta ,record) {
            ...
            {header: "Led Code", dataIndex: 'led_code',width:100,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right'},
        ],
         listeners :{
...
var columnCount = flxdetails.getColumnModel().getColumnCount(true);
		    ...
		    var p2 = "&compcode="+encodeURIComponent(compcode);      
	            ...
	            var p7 = "&lename=" + encodeURIComponent('CASH BOOK');
	            var p8 = "&prtdsp=" + encodeURIComponent('C');
...
var lblText = Ext.getCmp('lblCrDr3').getEl().dom.innerHTML;
...
//alert(finstartdate);
 ...
 //       monthStartdate  = finstartdate;
      ...
      if (Ext.util.Format.date(dt_today,"Y-m-d") > Ext.util.Format.date(finenddate,"Y-m-d"))
          ...
          monthenddate.setValue(Ext.util.Format.date(ToDate.getValue(),"Y-m-d"));
...
//                Ext.getCmp('yrstartdate').setDisabled(true);           
	    ...
	    Ext.getCmp('lblCASHBANK').setText("CASH BOOK");
            RefreshData();
        }
    }
});
