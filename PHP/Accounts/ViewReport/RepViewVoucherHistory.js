...
new Ext.KeyMap( Ext.getBody(), [{
  ...
  var VouNoClickDetailsNewDataStore = new Ext.data.Store({
        ...
        reader: new Ext.data.JsonReader({
         ...
         'cust_name','acctran_dbamt','acctran_cramt'
        ])
    });
...
labelStyle  : "font-size:14px;font-weight:bold;color:#fc9403",
 ...
 var loadVouNoDataStore = new Ext.data.Store({
  ...
  var flxld = new Ext.grid.EditorGridPanel({
        frame: false,
        store: VouNoClickDetailsNewDataStore,
...
//        y: 40,
        columns: [
            {header: "Ledger Name", dataIndex: 'cust_name',width:280,align:'left', sortable: false,defaultSortable: false,menuDisabled: true,
            renderer : function(value, meta ,record) {
		    var deb=record.get('acctran_dbamt');
		    var cre=record.get('acctran_cramt');
		    if(deb>0) {
...
var dgrecord = Ext.data.Record.create([]);
  ...
  function grid_tot_trans(){
	pvr_dbamt="";
        ...
        for(var i=0;i<Row;i++)
        {
            pvr_dbamt=Number(pvr_dbamt)+Number(sel[i].data.acctran_dbamt);
            pvr_cramt=Number(pvr_cramt)+Number(sel[i].data.acctran_cramt);
        }
 ...
 grid_tot_trans();
			   ...
			   VouNoHistoryDataStore.removeAll();
                           VouNoHistoryDataStore.load({
                                ...
                                callback:function(){
                                    var cnt=VouNoHistoryDataStore.getCount();
...
var flxVouNoDetail = new Ext.grid.EditorGridPanel({
    ...
    columns:
    [ 	 	
        {header: "Seq.No" , dataIndex: 'accref_seqno',sortable:false,width:90,align:'left', menuDisabled: true},
