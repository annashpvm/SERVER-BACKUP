...
var optRepOpt = new Ext.form.FieldSet({
    ...
    items: [
    {
        ...
        id: 'optRepOpt',
        items: [
		{boxLabel: 'Bills Outstanding', name: 'optRepOpt', id:'optBills', inputValue: 1,checked:true,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
					    repopt ='Bills Outstanding';

					}
				}
			}
		},
		{boxLabel: 'Ledger ', name: 'optRepOpt', id:'optLedger', inputValue: 2,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
						repopt ='Ledger';
 ...
 var loadBillsDetailsDatastore = new Ext.data.Store({
        ...
        baseParams:{
                     task:"load_Payable_Bills_Details"
        ...
        reader: new Ext.data.JsonReader({
            ...
            totalProperty: 'total',
            id: 'id'
        },['cust_name', 'acctrail_led_code', 'acctrail_inv_no','acctrail_inv_date', 'acctrail_inv_value', 'acctrail_adj_value', 'acctrail_crdays', 'duedate', 'oddays','accref_vouno','acctrail_amtmode'])
    });
...
'curbal_led_code', 'curbal_obdbamt', 'curbal_obcramt', 'acctran_led_code', 'trn_opdr', 'trn_opcr', 'accref_seqno', 'accref_vouno', 'accref_vou_type', 'accref_voudate', 'accref_payref_no', 'accref_payref_date', 'accref_narration', 'acctran_led_code', 'acctran_dbamt', 'acctran_cramt', 'acctran_accref_seqno', 'acctran_serialno', 'ledgercode', 'ledgername', 'partyledger', 'cust_code'
 ...
 var loadTBGrp1DetailsDatastore = new Ext.data.Store({
    ...
    var loadAPGroupDatastore = new Ext.data.Store({
        id: 'loadAPGroupDatastore',
        proxy: new Ext.data.HttpProxy({
                  url: 'ClsViewStatements.php',      // File to connect to
                  method: 'POST'
              }),
              baseParams:{task:"loadPayableGroup"}, // this parameter asks for listing
        reader: new Ext.data.JsonReader({
                    // we tell the datastore where to get his data from
          root: 'results',
          totalProperty: 'total',
          id: 'id' 
        },[ 'subgrp2', 'grp_name','closing','debit','credit'
 ...
 var loadTBLedgerDetailsDatastore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        totalProperty: 'total',
        id: 'id' 
      },[  'acctran_led_code', 'cust_name', 'closing', {name: 'checked', type: 'bool', defaultValue: false}
 ...
 /*  var yearchangeDataStore = new Ext.data.Store({
  ...
  var MonthClickVocDataStore = new Ext.data.Store({
        ...
        reader: new Ext.data.JsonReader({
            ...
            totalProperty: 'total',
            id: 'id'
        },['voudate', 'cust_name', 'voutype', 'accref_payref_no', 'acctran_dbamt', 'acctran_cramt', 'accref_vou_type','accref_vouno','accref_seqno', 'led_code'  ])
    });
...
//                   var voutype = cmbvoc.getRawValue().substring(0,3);                
		   ...
		   window.open('http://10.0.0.251:8080/birt/frameset?__report=Stores/RptOtherSalesInvoice.rptdesign&__format=xls'+ param); 
		    ...
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepVouPrintRecpt.rptdesign&__format=xls&' + param2, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepVouPrintRecpt.rptdesign' + param2, '_blank');
                    }       
                    else if (voutype == "CIP"  || voutype == "BKP" )
...
function grid_tot(){
        ...
        var cr = 0;
        var clo = 0;
        var Row= flxGroupwiseDue.getStore().getCount();
        flxGroupwiseDue.getSelectionModel().selectAll();
        var sel=flxGroupwiseDue.getSelectionModel().getSelections();
       for(var i=0;i<Row;i++)
...
//	      bal=Number(bal)+Number(sel[i].data.invbalamt);
        ...
        var Dr2 = formatter.format(drbal);
        var Cr2 = formatter.format(crbal);
        var Clo2 = formatter.format(drbal-crbal);  

        txtTotalDebitAmount.setRawValue(Dr2);
        txtTotalCreditAmount.setRawValue(Cr2);
        txtTotalBalAmt.setRawValue(Clo2);
}
  ...
  function grid_closebal(){
	clsbal="";
        var Row1= flxGroupwiseDue.getStore().getCount();
        flxGroupwiseDue.getSelectionModel().selectAll();
        var sele=flxGroupwiseDue.getSelectionModel().getSelections();
        for(var i=0;i<Row1;i++)
        {
...
var dgrecord = Ext.data.Record.create([]);
var flxGroupwiseDue = new Ext.grid.EditorGridPanel({
    frame: false,
    sm: new Ext.grid.RowSelectionModel(),
    stripeRows : true,
    scrollable: true,
...
style: {
    ...
    [ 	 	
        ...
        {header: "Description " , dataIndex: 'grpname',sortable:false,width:280,align:'left', menuDisabled: true,},
    ...
    listeners:{	
       ...
       'cellDblclick': function (flxGroupwiseDue, rowIndex, cellIndex, e) {
		var sm = flxGroupwiseDue.getSelectionModel();
		var selrow = sm.getSelected();
                grpcode = selrow.get('grpcode');
                grpcode = selrow.get('grpcode');
                GrpName = selrow.get('grpname');
...
var flxSupplier = new Ext.grid.EditorGridPanel({
    ...
    columns: [
        ...
        {header: "GRP Code", dataIndex: 'grpcode', hidden: true},

        {header: "Supplier", dataIndex: 'grpname', width: 300},
		...
		callback:function()
       		{
 	             ...
 	             { 
                           ...
                           cr = Ext.util.Format.number(Math.abs(loadBillsDetailsDatastore.getAt(j1).get('closing')),"0.00");

//alert(loadBillsDetailsDatastore.getAt(j).get('acctrail_inv_date'));
                       balamt = loadBillsDetailsDatastore.getAt(j1).get('acctrail_inv_value')-loadBillsDetailsDatastore.getAt(j1).get('acctrail_adj_value');
                       flxBillsDetails.getStore().insert(
                       flxBillsDetails.getStore().getCount(),
                       new dgrecord({

                           acctrail_inv_date  : Ext.util.Format.date(loadBillsDetailsDatastore.getAt(j1).get('acctrail_inv_date'),"d-m-Y"),
			   accref_vouno : loadBillsDetailsDatastore.getAt(j1).get('accref_vouno'),
			   acctrail_inv_no : loadBillsDetailsDatastore.getAt(j1).get('acctrail_inv_no'),

 	                   acctrail_inv_value : Ext.util.Format.number(loadBillsDetailsDatastore.getAt(j1).get('acctrail_inv_value'),"0.00"),
 			   acctrail_adj_value  : Ext.util.Format.number(loadBillsDetailsDatastore.getAt(j1).get('acctrail_adj_value'),"0.00"),
                           invbalamt : Ext.util.Format.number(balamt,"0.00"),
 			   acctrail_crdays  : loadBillsDetailsDatastore.getAt(j1).get('acctrail_crdays'),
                           duedate : loadBillsDetailsDatastore.getAt(j1).get('duedate'),
			   oddays : loadBillsDetailsDatastore.getAt(j1).get('oddays'),
			   acctrail_amtmode  : loadBillsDetailsDatastore.getAt(j1).get('acctrail_amtmode'),	
...
style: {
    ...
    columns:
    [ 	 	
        {header: "Vou Date" , dataIndex: 'acctrail_inv_date',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "Vou No" , dataIndex: 'accref_vouno',sortable:false,width:100,align:'left', menuDisabled: true,
},
        {header: "Bill No" , dataIndex: 'acctrail_inv_no',sortable:false,width:100,align:'left', menuDisabled: true,
},
        {header: "Vou Amount" , dataIndex: 'acctrail_inv_value',sortable:false,width:130,align:'right', menuDisabled: true},
        {header: "Adj Amount"  , dataIndex: 'acctrail_adj_value',sortable:false,width:130,align:'right', menuDisabled: true},
        {header: "Balance"    , dataIndex: 'invbalamt',sortable:false,width:130,align:'right', menuDisabled: true},
        {header: "DB/CR"   , dataIndex: 'acctrail_amtmode',sortable:false,width:50,align:'right'},
        {header: "Paymnt Terms", dataIndex: 'acctrail_crdays',sortable:false,width:90,align:'right', menuDisabled: true},
...
style:{
        ...
        columns: [
            ...
            {header: "Description", dataIndex: 'cust_name',width:300,align:'left',sortable: false,defaultSortable: false,menuDisabled: true,
		renderer : function(value, meta ,record) {
            ...
            {header: "Led Code", dataIndex: 'led_code',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: true,align: 'right', hidden : true},
        ],
         listeners :{
	        ...
	        flxld.getSelectionModel().selectAll();
		        ...
		        for(var i=0; i<selected_rows.length; i++)
		        {
		          var voouno=selected_rows[i].data.accref_vouno;
                         voutype =selected_rows[i].data.accref_vou_type;
	...
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p5 = "&maingroup="+encodeURIComponent(51);
		    var p6 = "&maingrpname="+encodeURIComponent('Sundry Creditors');
		    var p7 = "&fsdate=" + encodeURIComponent(finstartdate,"Y-m-d");

 		    var param = (p1+p2+p3+p4+p5+p6+p7) ;
 if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
                    { 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupOutstanding.rptdesign&__format=xlsX&' + param, '_blank');


      //              window.src ('AccRepAPGroupOutstanding.xls');  
                    }
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupOutstanding.rptdesign' + param, '_blank');	
                  /*  if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupOutstanding.rptdesign' + param, '_blank');*/	
	...
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p5 = "&grpcode="+encodeURIComponent(51);
 ...
 if (printtype == "PDF") 
		    ...
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepPartywiseOutstanding.rptdesign&__format=xlsX&' + param, '_blank');
                    else
    ...
    var btnAllPartyGroupBillwiseDetails = new Ext.Button({
        style: 'text-align:center;',
        text: " All Group Billwise Details ",
        width: 60,
        id: 'btnAllPartyBillwiseDetails',
        ...
        listeners: {
            click: function () {
		        ...
		        var p5 = "&ledcode="+encodeURIComponent(grpcode);
       		    var p6 = "&allopt="+encodeURIComponent('A');
 ...
 if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_Groupwise_Partywise.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_Groupwise_Partywise.rptdesign&__format=xlsX&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_Groupwise_Partywise.rptdesign' + param, '_blank');
                }
            }
    ...
    var btnAllPartyBillwiseDetails = new Ext.Button({
        style: 'text-align:center;',
        text: " All Party - Billwise Details ",
        width: 60,
        id: 'btnAllPartyBillwiseDetails',
        ...
        listeners: {
		    ...
		    var p5 = "&ledcode="+encodeURIComponent(grpcode);
                ...
                var p11 = "&drcropt=" + encodeURIComponent('A');
                   ...
                   var param = (p1+p2+p3+p4+p5+p6+p7+p8+p9+p10+p11);
 if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_Partywise.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_Partywise.rptdesign&__format=xlsX&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_Partywise.rptdesign' + param, '_blank');
                }
            }
    ...
    var btnGroupBillwiseDetails = new Ext.Button({
        style: 'text-align:center;',
        text: "Billwise Details ",
        width: 60,
        id: 'btnGroupBillwiseDetails',
                ...
                var win = new Ext.Window({
                    ...
                    items: [{
                        ...
                        items: [{
                            ...
                            vertical: true,
                            items: [
                                { boxLabel: 'All', name: 'drcropt', inputValue: 'A', checked: true },
                                { boxLabel: 'Only Debit Balance', name: 'drcropt', inputValue: 'D' },
                                { boxLabel: 'Only Credit Balance', name: 'drcropt', inputValue: 'C' },
                                { boxLabel: 'UnAdjusted List', name: 'drcropt', inputValue: 'U' }                                
                            ]
                        }]
                    }],
                    ...
                    buttons: [{
                        ...
                        handler: function () {
                            ...
                            Ext.each(selected, function(rec){
                                selectedCodes.push(rec.get('grpcode'));
                            });
                            
                            console.log("Selected Codes:", selectedCodes);
                        
                            if(selectedCodes.length === 0){
                                Ext.Msg.alert("Warning", "Please select at least one supplier");
                                return;
                            }
                        
                            var grpcodeList = selectedCodes.join(',');
        
                            var drcropt = Ext.getCmp('rdDrcropt').getValue().inputValue;
        
                            var p1 = "&ledname=" + encodeURIComponent('G');
                            var p2 = "&compcode=" + encodeURIComponent(compcode);
                            var p3 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"));
                            var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
                            var p5 = "&ledcode=" + encodeURIComponent(grpcodeList);
                            var p6 = "&allopt=" + encodeURIComponent('P');
                            var p7 = "&days1=" + encodeURIComponent(0);
                            var p8 = "&days2=" + encodeURIComponent(0);
                            var p9 = "&days3=" + encodeURIComponent(0);
                            var p10 = "&days4=" + encodeURIComponent(0);
        
                            var p11 = "&drcropt=" + encodeURIComponent(drcropt);
    ...
    var btnGroupBill_AgewiseDetails = new Ext.Button({
        style: 'text-align:center;',
        text: " Group-Agewise- From Due date ",
        width: 60,

        id: 'btnGroupBill_AgewiseDetails',
   tdCls: 'wrap',
	border: 1,
	style: {
	borderColor: 'blue',
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p5 = "&ledcode="+encodeURIComponent(grpcode);
       		    ...
       		    var p7 = "&agedays1="+encodeURIComponent(txtTo1.getValue());
       		    var p8 = "&agedays2="+encodeURIComponent(txtTo2.getValue());
       		    var p9 = "&agedays3="+encodeURIComponent(txtTo3.getValue());
       		    var p10 = "&agedays4="+encodeURIComponent(txtTo4.getValue());
 ...
 if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_GroupwiseAgeWise.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_GroupwiseAgeWise.rptdesign&__format=xlsX&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_GroupwiseAgeWise.rptdesign' + param, '_blank');	
                /*    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepPartywiseOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepPartywiseOutstanding.rptdesign' + param, '_blank');	*/
    ...
    var btnGroupBill_AgewiseDetails_FromBillDate = new Ext.Button({
        style: 'text-align:center;',
        text: " Group-Agewise - From Bill date ",
        width: 60,
        id: 'btnGroupBill_AgewiseDetails_FromBillDate',
 ...
 if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_GroupwiseAgeWise_FromBillDate.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_GroupwiseAgeWise_FromBillDate.rptdesign&__format=xlsX&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPBillwiseDue_GroupwiseAgeWise_FromBillDate.rptdesign' + param, '_blank');	
                /*    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepPartywiseOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepPartywiseOutstanding.rptdesign' + param, '_blank');	*/
	...
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p2 ="&compcode="+encodeURIComponent(compcode);      
	            var p3 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-  d"));	    var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
		    var p5 = "&maingroup="+encodeURIComponent(grpcode);
		    var p6 = "&maingrpname="+encodeURIComponent(GrpName);
		    var p7 = "&fsdate=" + encodeURIComponent(finstartdate,"Y-m-d");
                    var p8 = "&groupingcode=" + encodeURIComponent(0);
 	      	    var param = (p1+p2+p3+p4+p5+p6+p7+p8);
if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupPartyOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupPartyOutstanding.rptdesign&__format=xlsX&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupPartyOutstanding.rptdesign' + param, '_blank');	
                   /* if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupPartyOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepAPGroupPartyOutstanding.rptdesign' + param, '_blank');*/
...
*/
		    ...
		    var p6 = "&ledname="+encodeURIComponent(ledname);
       		    var p7 = "&ledtype="+encodeURIComponent('G');
...
if (printtype == "PDF") 
    ...
    function ProcessGroupData()
    {
        ...
        flxGroupwiseDue.getStore().removeAll();
        flxSupplier.getStore().removeAll();
        flxBillsDetails.getStore().removeAll();

        flxDetails.getStore().removeAll();
        loadAPGroupDatastore.removeAll();
        loadAPGroupDatastore.load({
	    url: 'ClsViewStatements.php',
        params: {
	    sk: 'loadPayableGroup',
                compcode:Gincompcode,
		...
		callback:function()
       		{
                   var cnt=loadAPGroupDatastore.getCount();
 		   ...
 		   { 
                   ...
                   if (loadAPGroupDatastore.getAt(j).get('closing') != 0 )
                   {
		           var dr = 0;
		           var cr = 0;
...
//                           cr = Ext.util.Format.number(Math.abs(loadTBDetailsDatastore.getAt(j).get('closing')),"0.00");
                        dr = Ext.util.Format.number(loadAPGroupDatastore.getAt(j).get('debit'),"0.00");
                        cr = Ext.util.Format.number(Math.abs(loadAPGroupDatastore.getAt(j).get('credit')),"0.00");


                       flxGroupwiseDue.getStore().insert(
                       flxGroupwiseDue.getStore().getCount(),
                       new dgrecord({	
 			   grpcode : loadAPGroupDatastore.getAt(j).get('subgrp2'),
			   grpname : loadAPGroupDatastore.getAt(j).get('grp_name'),
			   debit   : dr,
			  credit   : cr ,
                        })
                       );
...
//			   cust_name : loadLedgerDetailsDatastore.getAt(j).get('ledgername'),
			   cust_name : displedname,
   ...
   var tabOverall = new Ext.TabPanel({
...

	{
...

		{
		    ...
		    width       : 200,
                    labelWidth  : 50,
		    items : [btnAllPartyGroupBillwiseDetails]
		},

		{
		    ...
		    width       : 200,
                    labelWidth  : 50,
		    items : [btnAllPartyBillwiseDetails]
		},
...

		{
		    ...
		    width       : 200,
                    labelWidth  : 50,
		    items : [btnGroupBill_AgewiseDetails]
		},
...

		{
		    ...
		    width       : 200,
                    labelWidth  : 50,
		    items : [btnGroupBill_AgewiseDetails_FromBillDate]
		},
