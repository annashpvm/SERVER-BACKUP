...
var optprinttype = new Ext.form.FieldSet({
 ...
 var loadSearchGroupListDatastore = new Ext.data.Store({
      id: 'loadSearchGroupListDatastore',
//      autoLoad : true,
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
          ...
          'grp_code', 'grp_name', 'grp_parent_code'
...
function GroupSearch()
{
        ...
        loadSearchGroupListDatastore.removeAll();
        loadSearchGroupListDatastore.load({
		url: 'ClsViewStatements.php',
		params:
		{
			task   :"loadSearchGrouplist",
			group  : txtGroupName.getRawValue(),
		},
        });
}
	...
	listeners:{
	    keyup: function () {
               ...
               flxGroup.getEl().setStyle('z-index','10000');
                flxGroup.show();

   //             Ext.getCmp('tabAccPurchase').hide();   
                loadSearchGroupListDatastore.removeAll();
                  if (txtGroupName.getRawValue() != '')
                     GroupSearch();
            },
          specialkey:function(f,e){
             ...
             if (e.getKey() == e.DOWN)
             {
               Ext.getCmp('flxGroup').focus(false, 200);  
             }
          }
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
		{header: "Led Code", dataIndex: 'grp_code',sortable:true,width:60,align:'left',hidden:true},   
		{header: "", dataIndex: 'grp_name',sortable:true,width:330,align:'left'},
		{header: "Parent", dataIndex: 'grp_parent_code',sortable:true,width:60,align:'left',hidden:true},   
 
 
        ],
        store:loadSearchGroupListDatastore,
    ...
    listeners:{	
        'cellclick' : function(flxGroup, rowIndex, cellIndex, e){
			...
			var chkitem = (selrow.get('grp_code'));
			if ((selrow != null)){
				ledgroupcode  = selrow.get('grp_code');
				ledgroupname  = selrow.get('grp_name');
				txtGroupName.setValue(selrow.get('grp_name'));
                                flxGroup.hide();   
                                dataRefresh();
			}
...
labelStyle  : "font-size:14px;font-weight:bold;color:#fc9403",
        store:loadGroupOutstandingDatastore,
 ...
 var loadGroupOutstandingDatastore = new Ext.data.Store({
      id: 'loadGroupOutstandingDatastore',
      proxy: new Ext.data.HttpProxy({
                url: 'ClsViewStatements.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadGroupLedgerDetails"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
 ...
 var loadGroupOpeningDatastore = new Ext.data.Store({
      id: 'loadGroupOpeningDatastore',
      proxy: new Ext.data.HttpProxy({
                url: 'ClsViewStatements.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadGroupOpeningDetails"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
//                                                txtRefDate.setRawValue(AccInvNoDataStore.getAt(0).get('acctrail_inv_date1'));
  ...
  function grid_tot(){
	totdb="";
        ...
        txtLedgerDebit.setRawValue(Ext.util.Format.number(totdb,"0.00"));
        txtLedgerCredit.setRawValue(Ext.util.Format.number(totcr,"0.00"));
}
	...
	emptyText: 'Select Year',
        ...
        allowblank:false,
         listeners:{
			...
			txtLedgerDebit.setRawValue('0.00');
			txtLedgerCredit.setRawValue('0.00');
		        LedgerClick();
		        grpcodetds = 0;
...
style:{
        ...
        columns: [
            ...
            {header: "Date ", dataIndex: 'accref_voudate',width:110,align:'left',sortable: true,defaultSortable: false,menuDisabled: true,             },
            {header: "Account Name", dataIndex: 'cust_name',width:300,align:'left',sortable: true,defaultSortable: false,menuDisabled: true,
		renderer : function(value, meta ,record) {
            ...
            {header: "Voucher Type", dataIndex: 'voutype',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: true,align: 'center'},
            {header: "Doc. No.", dataIndex: 'accref_payref_no',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: true,align: 'center'},
            {header: "Vou. No.", dataIndex: 'accref_vouno',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: true,align: 'right'},
            ...
            {header: "Led Code", dataIndex: 'acctran_led_code',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: true,align: 'right'},
...
var btnProcess = new Ext.Button({
	...
	loadGroupOpeningDatastore.removeAll();
	loadGroupOpeningDatastore.load({
	url: 'ClsViewStatements.php',
                params: {
	    	task: 'loadGroupOpeningDetails',
                compcode:Gincompcode,
                finid:GinFinid,
                startdate: Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"), 
                enddate: Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"), 
                groupcode : ledgroupcode,
		},
		scope:this,
		callback:function()
       		{
                    opamt =  Number(loadGroupOpeningDatastore.getAt(0).get('balamt')) ;
			...
			loadGroupOutstandingDatastore.removeAll();
			loadGroupOutstandingDatastore.load({
			 url: 'ClsViewStatements.php',
				params: {
			    	task: 'loadGroupLedgerDetails',
				compcode:Gincompcode,
				finid:GinFinid,
				startdate: Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"), 
				enddate: Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"), 
				...
				callback:function()
		       		{
				       grid_tot();
                                       cloamt = Number(txtOpening_Debit.getValue()) - Number(txtOpening_Credit.getValue() )+ Number(txtLedgerDebit.getValue()) - Number(txtLedgerCredit.getValue());
                    if (cloamt > 0)      
