...
var optprinttype = new Ext.form.FieldSet({
 ...
 var loadSalesDetailsDatastore = new Ext.data.Store({
      id: 'loadSalesDetailsDatastore',
                            ...
                            for(var i=0;i<cnt;i++){
                               ...
                               flxdetails.getStore().insert(
                                ...
                                new dgrecord({
                                    ...
                                    led_gst_no   : MonthClickVocDataStore.getAt(i).get('cust_gstin'),
                                    ...
                                    invh_frt_rate: MonthClickVocDataStore.getAt(i).get('invh_frt_rate'),
                                    invh_frt_amt: MonthClickVocDataStore.getAt(i).get('invh_frt_amt'),
                                    invh_taxableamt: MonthClickVocDataStore.getAt(i).get('invh_taxableamt'),
                                    invh_sgst_amt: MonthClickVocDataStore.getAt(i).get('invh_sgst_amt'),
                                    invh_cgst_amt: MonthClickVocDataStore.getAt(i).get('invh_cgst_amt'),
                                    invh_igst_amt: MonthClickVocDataStore.getAt(i).get('invh_igst_amt'),
                                    invwt    : MonthClickVocDataStore.getAt(i).get('invwt'),
...
function grid_tot(){
       ...
       for(var i=0;i<Row;i++)

        {
         ...
         txtTotalClosing.setValue(dr-cr);
 ...
 var LedgerClickLoadDataStore = new Ext.data.Store({
  ...
  var MonthClickVocDataStore = new Ext.data.Store({
        ...
        reader: new Ext.data.JsonReader({
            ...
            totalProperty: 'total',
            id: 'id'
        },['voudate', 'cust_name', 'voutype', 'accref_payref_no', 'acctran_dbamt', 'acctran_cramt', 'accref_vou_type','accref_vouno','accref_seqno', 'cust_code' , 'invh_saltype', 'invh_frt_rate', 'invh_frt_amt', 'invh_taxableamt', 'invh_sgst_amt', 'invh_cgst_amt', 'invh_igst_amt', 'invwt','cust_gstin' ])
    });
...
style:{
		...
		renderer : function(value, meta ,record) {
            ...
            {header: "Voucher Type", dataIndex: 'voutype',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'center' ,hidden : 'true' },
            ...
            {header: "Taxable", dataIndex: 'invh_taxableamt',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},
            {header: "SGST", dataIndex: 'invh_sgst_amt',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},
            {header: "CGST", dataIndex: 'invh_cgst_amt',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},
            {header: "IGST", dataIndex: 'invh_igst_amt',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},
            {header: "FRT/MT", dataIndex: 'invh_frt_rate',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},
            {header: "FRT AMT", dataIndex: 'invh_frt_amt',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},
            {header: "WEIGHT", dataIndex: 'invwt',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right',hidden : 'true'},

            {header: "Inv type", dataIndex: 'accref_vou_type',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right'},
            ...
            {header: "Seq. No.", dataIndex: 'accref_seqno',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right'},
            {header: "Led Code", dataIndex: 'cust_code',width:120,align:'left',sortable: false,defaultSortable: false,menuDisabled: false,align: 'right'},
        ],
         listeners :{
	        ...
	        flxld.getSelectionModel().selectAll();
		        ...
		        for(var i=0; i<selected_rows.length; i++)
		        {
		          var voouno=selected_rows[i].data.accref_vouno;
                          vou_type  = selected_rows[i].data.accref_vou_type; 
		         }
        ...
        listeners: {
		    ...
		    var p1 = "&compcode="+encodeURIComponent(compcode);      
	            ...
	            var p3 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(finstartdate,"Y-m-  d"));	
                    var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(finenddate,"Y-m-d"));
                    var p5  = "&opt=" + encodeURIComponent(1);
...
//alert(param);
                    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/RepSalsMonthDaywiseSales.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/RepSalsMonthDaywiseSales.rptdesign&__format=xls' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/RepSalsMonthDaywiseSales.rptdesign' + param, '_blank');	
            }
        }
    });
...
//alert(monthenddate.getValue());
	...
	loadSalesDetailsDatastore.removeAll();
	loadSalesDetailsDatastore.load({
	 url: 'ClsViewStatements.php',
                params: {
		...
		callback:function()
       		{
                   var cnt=loadSalesDetailsDatastore.getCount();
                   var dr = 0;
                   var cr = 0;
                   var clo = 0;
...
//alert(loadSalesDetailsDatastore.getAt(j).get('rmonth')); 
//alert(sel[i].data.month); 
          
                     		    if (sel[i].data.month === loadSalesDetailsDatastore.getAt(j).get('rmonth'))
                  		    {
                                        dr = Number(loadSalesDetailsDatastore.getAt(j).get('dramt'));
                                        cr = Number(loadSalesDetailsDatastore.getAt(j).get('cramt'));;
                                        clo = clo + dr - cr;

	                                sel[i].set('debit', Ext.util.Format.number(loadSalesDetailsDatastore.getAt(j).get('dramt'),'0.00'));
                                        sel[i].set('credit', Ext.util.Format.number(loadSalesDetailsDatastore.getAt(j).get('cramt'),'0.00'));
...
var vouchertype = "GSI,OSI";
var vouchername = "SALES REGISTER";
		    ...
		    if (printtype == "PDF") 
			window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRep_ColumnVouTypeSales.rptdesign&__format=pdf&'+ param,  '_blank' );
		    else if (printtype == "XLS") 
	                window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRep_ColumnVouTypeSales.rptdesign&__format=XLS&'+ param , '_blank');
		    else
                        window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRep_ColumnVouTypeSales.rptdesign'+ param , '_blank');
