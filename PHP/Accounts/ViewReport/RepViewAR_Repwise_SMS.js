...
+ '\n' + mailtrailer) +'</table>' +  '\n' +'<br>';
		      ...
		      params :
		      {
                             ...
                             idemail    : email,
                             todaydate  : Ext.util.Format.date(TodayDate.getValue(),"Y-m-d"), 
                             repcode    : grpcode,
   ...
   {
       alert("Error in E-Mail ID.... Please check");
   }    
   }
   }
}); 
*/
    ...
    var TodayDate = new Ext.form.DateField({
	fieldLabel: 'Date',
        id: 'TodayDate',
...
var optprinttype = new Ext.form.FieldSet({
   ...
   function check_password()
   {
      if (txtPassword.getRawValue() == "bsms")
      {
        ...
        Ext.getCmp('btnEMailAll').setDisabled(false);

      }
      else
      {
        Ext.getCmp('btnSMS').setDisabled(true);
        Ext.getCmp('btnEMailAll').setDisabled(true);
      }    
...
var optRepOpt = new Ext.form.FieldSet({
 ...
 var loadPartyOverdueDatastore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        totalProperty: 'total',
        id: 'id' 
      },[ 'ledname', 'acctrail_led_code', 'balanceamt','cust_smsno','cust_overdue_msg','minday','maxday','cust_email'
       ...
       click: function(){
		      ...
		      callback: function(options, success, response)
		      {
	                 ...
	                 var obj = Ext.decode(response.responseText);
                         if (obj['success']==="true")					     
		            {
		                Ext.MessageBox.alert("SMS Send to Customers " );
		            }  
		          else
		            {
                      Ext.MessageBox.alert("SMS Already Sent");                                                  
		            }
...
var btnEMailAll = new Ext.Button({
    id      : 'btnEMailAll',
    style   : 'text-align:center;',
    text    : "SEND E-Mail",
    tooltip : 'Sent E-Mail to Customer...',
...
//alert(Row1);
		...
		for(var i=0;i<Row1;i++)
		{
                     ...
                     if (sel[i].data.odmsg == 'Y' && sel[i].data.emailid.trim() != '' )
                     {   

                        ledgername = sel[i].data.grpname; 
                        ledgercode = sel[i].data.grpcode; 
                        email      = sel[i].data.emailid; 

                        alert('E-Mail send to : ' + ledgername);
//                        Ext.MessageBox.alert("EMAIL Send to  " + ledgername ); 
...
var btnEmailTest= new Ext.Button({
    id      : 'btnEmailTest',
    style   : 'text-align:center;',
    text    : "TEST E-Mail",
    tooltip : 'Sent E-Mail to Customer...',
...
style: {
    ...
    listeners:{	
       ...
       'cellclick': function (flxRepwiseDue, rowIndex, cellIndex, e) {
                Ext.getCmp('btnEmail').setDisabled(false);  
...
style: {
    ...
    [ 	 	
        ...
        {header: "SMS No"  , dataIndex: 'smsno',sortable:false,width:100,align:'center', menuDisabled: true},
        {header: "OD Msg(Y/N)"  , dataIndex: 'odmsg',sortable:false,width:100,align:'center', menuDisabled: true},
        ...
        {header: "Email ID"  , dataIndex: 'emailid',sortable:false,width:250,align:'left', menuDisabled: true},
...
*/
	...
	borderStyle: 'solid',
        ...
        if (repopt =='All Outstanding')
        {
		    ...
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepRepOutstanding.rptdesign&__format=XLS' + param, '_blank');
                    else
        ...
        else
        {
		    ...
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepRep_Overdue_Outstanding.rptdesign&__format=XLS' + param, '_blank');
                    else
...
/*
		if (repopt =='All Outstanding')
		{
		    ...
		    var p2 ="&compcode="+encodeURIComponent(compcode);      
                    var p3 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"));	  
                    var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
		    var p5 = "&maingroup="+encodeURIComponent(RepresentativeCode);
		    var p6 = "&maingrpname="+encodeURIComponent(RepresentativeName);
       
		    var p7 = "&fsdate=" + encodeURIComponent(finstartdate,"Y-m-d");

 		    var param = (p1+p2+p3+p4+p5+p6+p7) ;
                    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepRepPartyOutstanding.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepRepPartyOutstanding.rptdesign&__format=XLS&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepRepPartyOutstanding.rptdesign' + param, '_blank');	
	...
	borderStyle: 'solid',
		...
		if (repopt =='All Outstanding')
          ...
          var p2 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"));	   
		    ...
		    var p6 = "&grpcode="+encodeURIComponent(RepresentativeCode2);
 		    var param = (p1+p2+p3+p4+p5+p6) ;
                    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRepARBillwiseOutstanding.rptdesign&__format=pdf&' + param, '_blank');
		    else if (printtype == "XLS") 
...
*/
	...
	borderStyle: 'solid',
	 ...
	 var p2 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"));	   
		    ...
		    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRep_SelectiveRep_ARBillwise_Outstanding.rptdesign&__format=pdf&' + param, '_blank');
		    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRep_SelectiveRep_ARBillwise_Outstanding.rptdesign&__format=XLS&' + param, '_blank');
		    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Accounts/AccRep_SelectiveRep_ARBillwise_Outstanding.rptdesign' + param, '_blank');	
...
/*
        ...
        var p2 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"));	   
		    ...
		    var p5 = "&allopt="+encodeURIComponent(2);
		    var p6 = "&grpcode="+encodeURIComponent(RepresentativeCode);
                    var p7 = "&overdue="+encodeURIComponent(overdue);
...
*/
	...
	borderStyle: 'solid',
	 ...
	 var p2 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"));	   
                    ...
                    var p5 = "&repdays="+encodeURIComponent(Number(txtDays.getValue()));
		    var param = (p1+p2+p3+p4+p5) ;
			...
			callback:function()
	 		   ...
	 		   { 
				   ...
				   var cr = 0;
	 			   ...
	 			   grpcode2 : loadRepAllDueDatastore.getAt(j).get('repr_accgrp'),
			...
			callback:function()
	 		   ...
	 		   { 
				   ...
				   credit  : cr ,
                                   ...
                                   emailid : loadPartyOverdueDatastore.getAt(j).get('cust_email'),
