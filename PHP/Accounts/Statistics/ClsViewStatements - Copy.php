...
*/
 ...
 function getTB_Ledgers()
	...
	$mgrpcode = $_POST['mgrpcode'];
	$fsdate = $_POST['fsdate'];
    $sql  = "call accspreptrialbalanceclosing_View_Subgroup_levelend($finid ,$compcode, '$startdate', '$enddate','$mgrpcode','$fsdate' )";
	$r = mysqli_query($conn, $sql);
...
/*
        if ($alldueopt == 0) 
    $sql ="call spacc_rep_receivables_Party_overdue_outstanding($compcode,'$startdate', '$enddate',$ledcode,$alldueopt)";
        else
    $sql ="call accsp_rep_ar_billwisedue($compcode,'$startdate', '$enddate',$ledcode,'$alldueopt',0,'$dueopt')";
*/   
//    $sql ="call spacc_rep_receivables_Party_overdue_outstanding($compcode,'$startdate', '$enddate',$ledcode,0,$dueopt)";
...
//echo $r;
    $sql = "call accsp_rep_ap_billwisedue($compcode,'$startdate', '$enddate',$ledcode,'$repopt')";
 ...
 function getRep_All_Outstanding()
    ...
    $sql ="call spacc_rep_receivables_Rep_AllOutstanding_Abstract($compcode, '$asondate')";
	$r = mysqli_query($conn, $sql);
	...
	function getVouNoHistory()
	{
		...
		if ($usrcode == 0) {
			...
			select 
				...
				case when accvou_slno is null then 1 else accvou_slno end accvou_slno,
				case when accvou_date is null then '' else DATE_FORMAT(accvou_date, '%d-%m-%Y') end accvou_date,
				case when accvou_narration is null then '' else accvou_narration end accvou_narration,
				case when usr_name is null then '' else usr_name end usr_name,
				case when usr_login is null then '' else usr_login end usr_login
			from acc_ref
			left join acc_voucher_logs on accref_seqno = accvou_seqno
			left join userMaster on accvou_userid = usr_code
			where accref_comp_code = $compcode
			  and accref_finid = $finid
			  and accref_voudate between '$startdate' and '$enddate'";
		} else {
			$sql = "
			select 
				accref_seqno,
...
$r = mysqli_query($conn, $sql);
 ...
 function getAdjustmentDetails()
	...
	$db_cr     = $_POST['db_cr'];
        if ($db_cr == 'cr')
    $sql ="select *,DATE_FORMAT(ref_docdate, '%d-%m-%Y') adjdate,DATE_FORMAT(ref_invdate, '%d-%m-%Y')  voudate  from acc_adjustments where ref_compcode = $compcode and ref_finid = $finid and  ref_docno = '$vouno' order by ref_invdate";      
        else
    $sql ="select *,DATE_FORMAT(ref_docdate, '%d-%m-%Y') adjdate,DATE_FORMAT(ref_invdate, '%d-%m-%Y')  voudate  from acc_adjustments where ref_compcode = $compcode and ref_finid = $finid and  ref_adjvouno = '$vouno' order by ref_invdate";      
...
$sql ="select
 ...
 concat( DATE_FORMAT(accref_voudate, '%d-%m-%Y') , ' - ' , accref_vouno , ' - ' , cust_name   , ' - ' , acctran_cramt  ) as heading,acctran_dbamt,acctran_cramt,  ref_invno ,  DATE_FORMAT(ref_invdate, '%d-%m-%Y') ref_invdate ,ref_adjamount , ref_paymt_terms, ref_adj_days  from acc_ref ref left join acc_tran trn  on ref.accref_seqno = trn.acctran_accref_seqno left outer join acc_adjustments adj on ref_compcode = accref_comp_code and acctran_accref_seqno = ref_docseqno and acctran_led_code = ref_ledcode  join massal_customer led on led.cust_code = trn.acctran_led_code where acctran_cramt > 0 and   accref_comp_code = $compcode and accref_voudate between '$startdate' and '$enddate' and accref_vou_type in('CHR','BKR')  order by accref_seqno ;";      
      ...
      //  $r = mysqli_query("call acc_sp_rep_collection_payment($compcode,'$startdate','$enddate')";      
...
//echo $sql;
//echo "<br>";
        $r1 = mysqli_query("call spacc_rep_ledger_column_report($compcode ,$finid,'$startdate','$enddate','$ledcode','$tablename','$sp')";     
