...
global $conn;
  ...
  function getVoucherNoDetailDate()
	...
	//$sql = "CALL general_sp_mas_selcontrolmaster('ST','$finyear','IRN','STORES ISSUE RETURN NO','$compcode')";
        $sql = "select ifnull(max(convert(substring(accref_vouno,4),signed)),0) +1 as con_value from acc_ref where accref_vou_type = 'EX' and accref_finid = '$finyear' and accref_comp_code = '$compcode';";
    $r = mysqli_query($conn, $sql);
	...
	$finyear=$_POST['finyear'];
        $compcode=$_POST['compcode'];
	//$sql = "CALL general_sp_mas_selcontrolmaster('ST','$finyear','IRN','STORES ISSUE RETURN NO','$compcode')";
        $sql = "select ifnull(max(convert(substring(accref_vouno,4),signed)),0) +1 as con_value from acc_ref where accref_vou_type = 'CT' and accref_finid = '$finyear' and accref_comp_code = '$compcode';";
    $r = mysqli_query($conn, $sql);
    ...
    function getLedger(){
        ...
        $sql = "select cust_code,cust_name from massal_customer where cust_acc_group in (".$pstgrpcode.") and "
    ...
    function getJournalLedger(){
        $compcode=$_SESSION['gincompcode'];
        $sql = "select cust_code,cust_name from massal_customer where cust_acc_group not in ('43') and "
                . "led_comp_code='$compcode' and led_status='Y' and led_duplicate='N';";
    $r = mysqli_query($conn, $sql);
	...
	$finyear=$_POST['finid'];
        $compcode=$_POST['compcode'];
	//$sql = "CALL general_sp_mas_selcontrolmaster('ST','$finyear','IRN','STORES ISSUE RETURN NO','$compcode')";
        $sql = "select ifnull(max(convert(substring(accref_vouno,4),signed)),0) +1 as con_value from accrefprovision where  accref_finid = '$finyear' and accref_comp_code = '$compcode';";
    $r = mysqli_query($conn, $sql);
    ...
    function getExportInvoiceDetailforRealisation()
    {
        ...
        $sql = "call acc_sp_trn_selexpoinvdetailforrealisation(".$cinvseqno.",".$compcode.";";
