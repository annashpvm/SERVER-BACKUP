...
mysqli_set_charset($conn, "utf8");
 ...
 function getreportgroup()
  ...
  function getLedgerList()
	...
	$sql = "select * from massal_customer , acc_group_master where  cust_type = 'G' and  cust_acc_group = grp_code  and cust_code > 0 order by cust_name";
  ...
  function getGroupList()
	...
	$sql = "select grp_name,grp_code from massal_customer , acc_group_master where  cust_acc_group = grp_code  and cust_code > 0 group by grp_name,grp_code order by  grp_name asc,grp_code";
 ...
 function getSearchLedgerlist()
    {
        ...
        $sql = "select * from massal_customer where cust_type = 'G' and cust_name like '%$ledname%' order by cust_name";
  $r = mysqli_query($conn, $sql);
    $arr = [];
    while ($re = mysqli_fetch_assoc($r)) {
        $arr[] = $re;
