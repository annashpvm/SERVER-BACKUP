<html>
<head>
<title>EXTRA CD DAYS ALLOWED ENTRY </title>
<link rel="stylesheet" type="text/css" href="/ext-3.4.1/resources/css/ext-all.css" />
<link rel="stylesheet" type="text/css" href="/ext-3.4.1/examples/shared/examples.css" />
<script type="text/javascript" src="/ext-3.4.1/adapter/ext/ext-base.js"></script>
<script type="text/javascript" src="/ext-3.4.1/ext-all.js"></script>
<script type="text/javascript" src="/ext-3.4.1/examples/ux/MultiSelect.js"></script>
<script type="text/javascript" src="/ext-3.4.1/examples/ux/ItemSelector.js"></script>
<script type="text/javascript" src="/SHVPM/SALES/AccountsMenu.js"></script>
<script type="text/javascript" src="ExtraCDDays.js"></script>
<?php
    require($_SERVER["DOCUMENT_ROOT"]."/SHVPM/SALES/AccountsMainPage.php");
?>
</head>
<body>
</body>
</html>
