...
var UserId   = localStorage.getItem('ginuserid');
 ...
 var loadLedgerMainGrpListDatastore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
                url: 'ClsLedgerSubGoupLevel2.php',      // File to connect to
                method: 'POST'
            }),
 ...
 var loadSubGrpListDatastore = new Ext.data.Store({
	...
	'subgrpcode', 'subgrpname', 'parentgrpcode', 'parentgrpname','sub2grpcode', 'sub2grpname'
      ]),
    });
  ...
  var loadParentGroupDetaildatastore = new Ext.data.Store({
   ...
   var txtSubGroupName = new Ext.form.TextField({
	...
	listeners:{
     ...
     var cmbLedgerMainGroup = new Ext.form.ComboBox({
        fieldLabel      : 'Select Level-1 Group Name',
        width           : 400,
       ...
       labelStyle      : "font-size:14px;font-weight:bold;color:#0080ff",
        ...
        select: function(){
			...
			loadParentGroupDetaildatastore.load({
                        url: 'ClsLedgerSubGoupLevel2.php',
                        params:
                            {
   ...
   var flxDetail = new Ext.grid.EditorGridPanel({
       ...
       id: 'my-grid',  
		...
		{header: "Level1 Grp Code", dataIndex: 'sub2grpcode',sortable:true,width:90,align:'left'},   
		{header: "Level1 Grp Name", dataIndex: 'sub2grpname',sortable:true,width:330,align:'left'},
       ...
       listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			txtSubGroupName.setRawValue(selrow.get('subgrpname'));
                        cmbLedgerMainGroup.setValue(selrow.get('sub2grpcode'));
...
//save
				...
				else
				{
					...
					if (btn == 'yes')
						{
						Ext.Ajax.request({
		                            	url: 'MasLedgerSubGroupLevel2Save.php',
                		       	        params:
						{
                                                        savetype      : saveflag,
	...
	height      : 600,
        width       : 1280,
        y           : 35,
        title       : 'LEDGER SUB GROUP LEVEL - II',
