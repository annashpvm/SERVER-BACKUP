<?php
    ...
    switch($task){
		...
		break;
             	...
             	case "loadLongPendingCustomerList":
                getLongPendingCustomerList();
		break;
 ...
 function getLongPendingCustomerList()
    {
        mysql_query("SET NAMES utf8");


        $ledname = strtoupper($_POST['ledger']);
        $qry = "select * from massal_longpending_customers , massal_customer, massal_repr where l_custcode = cust_code and cust_repr = repr_code order by l_custname";
        $r=mysql_query($qry);
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
