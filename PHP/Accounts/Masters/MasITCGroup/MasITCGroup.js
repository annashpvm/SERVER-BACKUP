...
//      autoLoad : true,
      proxy: new Ext.data.HttpProxy({
                url: 'ClsITC.php',      // File to connect to
                method: 'POST'
            }),
 ...
 var loadGSTLedgerListDatastore = new Ext.data.Store({
      id: 'loadGSTLedgerListDatastore',
      autoLoad : true,
      proxy: new Ext.data.HttpProxy({
                url: 'ClsITC.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadGSTGroupLedgerList"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
   ...
   var cmbGroupName = new Ext.form.ComboBox({
        ...
        forceSelection  : true,
        triggerAction   : 'all',  
   	store		: loadGSTLedgerListDatastore,     
        selectOnFocus   : false,
        editable        : true,
        allowblank      : false,
       labelStyle      : "font-size:14px;font-weight:bold;color:#0080ff",
        listeners:{
        select: function(){
               flxLedgerSelected.getStore().removeAll();
 ...
 var loadGroupListDatastore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
          ...
          'itc_code', 'itc_grpcode', 'itc_ledcode', 'grpname', 'ledname'
      ]),
    });
...
//				cmbAccountName.setValue(selrow.get('cust_code'));
		txtLedgerName.setValue(selrow.get('cust_name'));
				...
				ledgername = selrow.get('cust_name');
                  		...
                  		flxLedgerSelected.getSelectionModel().selectAll();
			        var selrows = flxLedgerSelected.getSelectionModel().getCount();
			        var sel = flxLedgerSelected.getSelectionModel().getSelections();
				var stkcnt  = 0;
				...
				else
				{
				   ...
				   var RowCnt = flxLedgerSelected.getStore().getCount() + 1;  
                                   flxLedgerSelected.getStore().insert(
                                   flxLedgerSelected.getStore().getCount(),
                                    new dgrecord({
					  led_code    : ledgercode,
					  led_name    : ledgername,
                                    })
                        	...
                        	flxLedgerSelected.getSelectionModel().clearSelections(); 
   ...
   var flxLedgerSelected = new Ext.grid.EditorGridPanel({
        frame: false,
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
		{header: "Ledger Code", dataIndex: 'cust_code',sortable:true,width:60,align:'left'},   
    ...
    listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
         ...
         Ext.Msg.show({
             ...
             fn: function(btn){
               if (btn === 'yes'){
                  var sm = flxLedgerSelected.getSelectionModel();
                  var selrow = sm.getSelected();
                  flxLedgerSelected.getStore().remove(selrow);
                  flxLedgerSelected.getSelectionModel().selectAll();
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
   ...
   var flxDetail = new Ext.grid.EditorGridPanel({
       ...
       id: 'my-grid',  
    	...
    	style      :"border-radius: 5px;textTransform: uppercase; ",  
        columns: [   
		{header: "Ledger Group Name", dataIndex: 'grpname',sortable:true,width:330,align:'left'},   
		{header: "Ledger Group Code", dataIndex: 'itc_grpcode',sortable:true,width:90,align:'left', hidden:true},
		{header: "Ledger Code", dataIndex: 'itc_ledcode',sortable:true,width:90,align:'left',hidden:true},   
		{header: "Ledger Name", dataIndex: 'ledname',sortable:true,width:500,align:'left'},
       ...
       listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			ledmergecode = selrow.get('rep_merge_code');
                        saveflag = "Edit";
			cmbGroupName.setRawValue(selrow.get('rep_merge_name'));
                        getLedgerList();
			}
   ...
   var MasITCPanel = new Ext.FormPanel({
...
//save
				...
				if(cmbGroupName.getRawValue()=="" || cmbGroupName.getRawValue()==0)
				{
				...
				else
				{
					...
					{
					if (btn == 'yes')
		                                var accData = flxLedgerSelected.getStore().getRange();
						...
						{
						Ext.Ajax.request({
		                            	url: 'MasITCGroupSave.php',
                		       	        params:
						{
                                                        savetype   : saveflag,
                                                        griddet : Ext.util.JSON.encode(accupdData),
                                                        cnt     : accData.length,
							grpcode : cmbGroupName.getValue(),
							grpname : cmbGroupName.getRawValue(),
		...
		style:{ border:'1px solid red'},
            ...
            },flxLedger,flxLedgerSelected,flxDetail
            
        ],
    });
    

    var MasITCWindow = new Ext.Window({
	height      : 600,
        width       : 1280,
        y           : 35,
        title       : 'ITC Group',
