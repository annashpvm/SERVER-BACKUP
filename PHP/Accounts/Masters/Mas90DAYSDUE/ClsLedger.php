 ...
 function get90daysCustomerList()
    {
        ...
        $ledname = strtoupper($_POST['ledger']);
        $sql = "select * from overdue_custlist , massal_customer ,massal_repr where cust_code = c_code and cust_repr = repr_code order by repr_name,c_name";
  $r = mysqli_query($conn, $sql);
    $arr = [];
    while ($re = mysqli_fetch_assoc($r)) {
        $arr[] = $re;
 ...
 function getBlackCustomerList()
    {
        ...
        $ledname = strtoupper($_POST['ledger']);
        $sql = "select repr_name,cust_code, cust_ref,DATE_FORMAT(so_seqdate, '%d-%m-%Y')  so_seqdate,DATE_FORMAT(so_upto_allowed, '%d-%m-%Y') so_upto_allowed  from massal_customer ,massal_repr , acc_so_allow where cust_code =so_custcode and cust_repr = repr_code  order by so_seqno desc,repr_name,cust_ref desc";
  $r = mysqli_query($conn, $sql);
    $arr = [];
    while ($re = mysqli_fetch_assoc($r)) {
        $arr[] = $re;
