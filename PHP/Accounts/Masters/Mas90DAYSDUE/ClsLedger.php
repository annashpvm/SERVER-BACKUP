<?php
    ...
    switch($task){
		...
		getSearchLedgerlist();
		break;
             	case "load90daysCustomerList":
		get90daysCustomerList();
		break;
 ...
 function get90daysCustomerList()
    {
        mysql_query("SET NAMES utf8");


        $ledname = strtoupper($_POST['ledger']);
        $qry = "select * from overdue_custlist , massal_customer ,massal_repr where cust_code = c_code and cust_repr = repr_code order by repr_name,c_name";
        $r=mysql_query($qry);
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
 ...
 function getBlackCustomerList()
    {
        ...
        $ledname = strtoupper($_POST['ledger']);
        $qry = "select repr_name,cust_code, cust_ref,DATE_FORMAT(so_seqdate, '%d-%m-%Y')  so_seqdate,DATE_FORMAT(so_upto_allowed, '%d-%m-%Y') so_upto_allowed  from massal_customer ,massal_repr , acc_so_allow where cust_code =so_custcode and cust_repr = repr_code  order by so_seqno desc,repr_name,cust_ref desc";
        $r=mysql_query($qry);
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
