...
;
 ...
 var load90daysCustomerListDatastore = new Ext.data.Store({
      id: 'load90daysCustomerListDatastore',
      autoLoad : true,
      proxy: new Ext.data.HttpProxy({
                url: 'ClsLedger.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"load90daysCustomerList"}, // this parameter asks for listing
	...
	'repr_name','c_code', 'c_name','c_SO_allow'
      ]),
    });
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
   ...
   function RefreshData(){
	...
	txtCustomerName.setValue("");
        saveflag = "Add";
        customercode =0;
        load90daysCustomerListDatastore.removeAll();
	load90daysCustomerListDatastore.load({
          url:'ClsLedger.php',
          params:
       	  {
           task:"load90daysCustomerList"
          }
	});
   };
        ...
        listeners: {
            click: function () {
                ...
                for (var i = 0; i < selrows; i++) {
                    if (sel[i].data.c_code == customercode) {
                        cnt = cnt + 1;
                    }
                }
	        if (cnt > 0){
                ...
                {  
                   ...
                   flxDetail.getStore().insert(
                     ...
                     new dgrecord({
                        slno: RowCnt,
                        c_name  : txtCustomerName.getRawValue(),
                        c_code  : customercode,
                        c_SO_allow : 'N',
                     })
                    );
   ...
   var flxDetail = new Ext.grid.EditorGridPanel({
       ...
       id: 'my-grid',  
		...
		{header: "Cust Code", dataIndex: 'c_code',sortable:true,width:90,align:'left', hidden: true},   
		{header: "Customer Name", dataIndex: 'c_name',sortable:true,width:500,align:'left'},
		{header: "Allow to SO", dataIndex: 'c_SO_allow',sortable:true,width:150,align:'center'},

        ],
       store:load90daysCustomerListDatastore,
       ...
       listeners:{
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			{
				if (selected_rows[i].data.c_SO_allow == 'N')
                                    flag = 'N';
                                else                                   
                                    flag = 'Y';
