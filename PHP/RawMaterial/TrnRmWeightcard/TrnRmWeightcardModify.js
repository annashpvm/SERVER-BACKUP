Ext.onReady(function(){
  ...
  function check_password()
   {
      if (txtPassword.getRawValue() == "weight")
      {
...
var lblParty = new Ext.form.Label({
 ...
 var loadWBTicketDatastore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task:"loadWBTicketNoList2"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
'wc_partyloadwt', 'wc_partyemptywt', 'wc_partynetwt', 'wc_acceptedwt', 'wc_process', 'wt_type'
      ]),
    });
...
//		cmbItemName.focus();
	...
	else
	{
		...
		if (btn == 'yes')
			...
			Ext.Ajax.request({
	            	url: 'TrnRmWeightcardSaveModify.php',
	       	        params:
			{
				compcode	:Gincompcode,
				finid		:GinFinid,
