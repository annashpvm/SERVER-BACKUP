...
var UserId   = localStorage.getItem('ginuserid');
 ...
 var flxchk = 0;
   ...
   {
      if (txtPassword.getRawValue() == "gst")
      {
...
;
 ...
 var loadSupplierListDatastore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
                url: 'ClsGSTBillsSupplier.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadGSTBillSupplierList"}, // this parameter asks for listing
	...
	'sup_code', 'sup_name','sup_wp_gstinv_supplier_yn'
      ]),
    });
...
var txtSearch = new Ext.form.TextField({
	...
	listeners:{
	    keyup: function () {
               flxDetail.getStore().filter('c_name', txtSearch.getValue());  
            }
         }  
    });
...
//      autoLoad : true,
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
          ...
          'sup_code', 'sup_name','led_grp_code','led_type'
      ]),
    });
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
    ...
    listeners:{	
			...
			if ((selrow != null)){
				...
				Suppliercode = selrow.get('sup_code');
    ...
    var btnAdd = new Ext.Button({
        ...
        listeners: {
            click: function () {
                ...
                if (txtSupplierName.getRawValue() == '') {
                    gstInsert = "false";
                    Ext.MessageBox.alert("", "Select Supplier Name");
                }
                else
                {  
                ...
                for (var i = 0; i < selrows; i++) {
                    if (sel[i].data.c_code == Suppliercode) {
                        cnt = cnt + 1;
                    }
                }
	        if (cnt > 0){
	            gstInsert = "false";
	            Ext.MessageBox.alert("","This Supplier Already Entered");
                    gstInsert == "false";
	        }
                else    
                {  
                   ...
                   flxDetail.getStore().insert(
                     ...
                     new dgrecord({
                        slno: RowCnt,
                        sup_name  : txtSupplierName.getRawValue(),
                        sup_code  : Suppliercode,
                        sup_wp_gstinv_supplier_yn : 'Y',
                     })
                    );
   ...
   var flxDetail = new Ext.grid.EditorGridPanel({
       ...
       id: 'my-grid',  
    	...
    	style      :"border-radius: 5px;textTransform: uppercase; ",  
        columns: [   
		{header: "Sup Code", dataIndex: 'sup_code',sortable:true,width:90,align:'left', hidden: true},   
		{header: "Sup  Name", dataIndex: 'sup_name',sortable:true,width:500,align:'left'},
		{header: "GST Bills", dataIndex: 'sup_wp_gstinv_supplier_yn',sortable:true,width:150,align:'center'},
       ...
       listeners:{
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			{
				if (selected_rows[i].data.sup_wp_gstinv_supplier_yn == 'N')
                                    flag = 'N';
                                else                                   
                                    flag = 'Y';
                                             
                               	colname = 'sup_wp_gstinv_supplier_yn';
				if (flag == 'N')
				{
  ...
  function save_click()
{
	...
	else
	{
		...
		if (btn == 'yes')
			...
			Ext.Ajax.request({
                    	url: 'MasGSTBillsSupplierSave.php',
	       	        params:
			{
