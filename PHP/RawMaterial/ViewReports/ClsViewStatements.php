...
switch ($task) {
    ...
    case "loadDatewiseWBDetails":
        getDatewiseWBDetails();
        break;

    default:
        echo '{ "failure": true }'; // Return failure response
        break;
}
...
*/
        ...
        if ($itemname  == "0")
        $sql = "select *, DATE_FORMAT(wc_date, '%d-%m-%Y') as ticketdate from trn_weight_card left join mas_area on wc_area_code = area_code left join mas_wb_item on  trim(wc_item) = trim(item_name) left join mas_wb_itemgroup on item_grpcode = item_group where wc_compcode =  $compcode and wc_fincode =  $finid and wc_date between '$startdate' and '$enddate'  order by wc_ticketno desc";
        else
 ...
 function getDatewiseWBDetails()
	...
	$startdate= $_POST['startdate'];
        ...
        $sql = "select *, DATE_FORMAT(t_wb_date, '%d-%m-%Y') as ticketdate,case when t_wb_1st_loadtype = 'E' then t_wb_1st_weight else t_wb_2nd_weight end as emptywt,case when t_wb_1st_loadtype = 'L' then t_wb_1st_weight else t_wb_2nd_weight end as loadwt , DATE_FORMAT(t_wb_1st_time, '%d-%m-%Y %H:%i:%s') as t_wb_1st_time1  ,
 DATE_FORMAT(t_wb_2nd_time, '%d-%m-%Y %H:%i:%s') as t_wb_2nd_time1   from trn_weighbridge_entry where t_wb_compcode = $compcode and  t_wb_2nd_time between '$startdate' and '$enddate'";
