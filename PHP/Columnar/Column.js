...
var monthstartdate = getParameterByName('monthstartdate');
var monthenddate   = getParameterByName('monthenddate');
var ledgercode     = getParameterByName('ledgercode');
var ledgername     = getParameterByName('ledgername');
var compcode       = getParameterByName('compcode');
var finid          = getParameterByName('finid');


if(monthstartdate){
    fromDate.setValue(Date.parseDate(monthstartdate,'Y-m-d'));
}

if(monthenddate){
    toDate.setValue(Date.parseDate(monthenddate,'Y-m-d'));
}

if(ledgercode){
    txtSearch.setValue(ledgercode);
}

if(ledgername){
    txtLedgerName.setValue(ledgername);
}

if(!fromDate.getValue() || !toDate.getValue()){
    Ext.Msg.alert('Validation','Please select From and To Date');
    return;
}
...
if(!grid){
    ...
    grid = new Ext.grid.GridPanel({
        ...
        title:'COLUMNAR REPORT FOR - ' + (ledgername || ''),
        ...
        listeners:{
            rowdblclick:function(grid,rowIndex,e){
                ...
                if (window.opener) {
                    ...
                    window.opener.loadPanel8(voucherno); // call parent function
...
var heading="COLUMNAR FOR LEDGER : "+ledgername ;
