...
//	$r=mysql_query("select cust_code,cust_ref from massal_customer where sup_acc_group = 78 order by sup_refname");
	$r=mysql_query("select cust_ref ,cust_code from trnrm_order_header , massal_customer where ordh_sup_code = sup_code group by cust_ref ,cust_code order by cust_ref ,cust_code");
 ...
 function getthird()
    {
        mysql_query("SET NAMES utf8");
	//$supplier_id = $_POST['supplierid'];
	$r=mysql_query("select cust_code,cust_ref from massal_customer where cust_acc_group = 143 order by sup_refname");
	//$r=mysql_query("call sp_pur_supplier_actgrp($supplier_id)");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
...
//	$r=mysql_query("call sprm_sel_recheddet ('$grnno') ");
	$r=mysql_query(" select * from trnrm_receipt_header a ,massal_customer s , mas_RMFU_purchasetax p
