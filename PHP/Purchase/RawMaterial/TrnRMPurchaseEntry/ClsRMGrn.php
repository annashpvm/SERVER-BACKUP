...
//	$sql = "select cust_code,cust_ref from massal_customer where sup_acc_group = 78 order by sup_refname");
	$sql = "select cust_ref ,cust_code from trnrm_order_header , massal_customer where ordh_sup_code = sup_code group by cust_ref ,cust_code order by cust_ref ,cust_code");
 ...
 function getthird()
    {
        mysqli_set_charset($conn, "utf8");
	//$supplier_id = $_POST['supplierid'];
	$sql = "select cust_code,cust_ref from massal_customer where cust_acc_group = 143 order by sup_refname");
	//$sql = "call sp_pur_supplier_actgrp($supplier_id)");
    $r = mysqli_query($conn, $sql);
...
//	$sql = "call sprm_sel_recheddet ('$grnno') ");
	$sql = " select * from trnrm_receipt_header a ,massal_customer s , mas_RMFU_purchasetax p
 where rech_seqno= $grnno and  s.cust_code=a.rech_sup_code  and rech_purledger = tax_purcode");
...
function getuserdet()
{
 ...
 function getQCEntryNoDetail()
	...
	$entryno  = $_POST['entryno'];
        ...
        $sql = "select * from trn_qc_rm_inspection , masrm_item_header,massal_customer , mas_area ,mas_areagroup ,mas_RMFU_purchasetax   where  qc_rm_pur_ledger =tax_purcode  and area_grpcode = areagrp_code and qc_rm_supcode = sup_code and qc_rm_area = area_code  and qc_rm_itemcode = itmh_code and qc_rm_compcode = '$compcode' and qc_rm_fincode = '$finid' and qc_rm_entryno = $entryno order by qc_rm_slno");
