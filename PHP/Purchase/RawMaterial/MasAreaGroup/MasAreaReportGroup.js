	...
	labelStyle  : "font-size:14px;font-weight:bold;color:#ff33cc",
    	...
    	listeners : {
            keyup: function () {
                  ...
                  flxArea.getStore().filter('mas_area_repgrp_name', txtAreaGroup.getRawValue());  
            }
        }
...
var GetAreaListDatastore = new Ext.data.Store({
  ...
  var dgrecord = Ext.data.Record.create([]);
   var flxArea = new Ext.grid.EditorGridPanel({
        ...
        columns: [    
            {header: "Area Group code" , Id: 'mas_area_repgrp_code', sortable:true,width:100,align:'left', menuDisabled: true},       
            {header: "Area Group Name", Id: 'mas_area_repgrp_name', sortable:true,width:250,align:'left', menuDisabled: true},

                       
           ],
viewConfig: {
getRowClass: function(record) {
    var red = record.get('mas_area_repgrp_name') // shows if a CPE is dead or not (HS = Dead)
    if (red == txtAreaGroup.getValue()) {
    return 'mas_area_repgrp_name'
    }
}
},
store:GetAreaListDatastore,
    listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			editrow = selrow;
                        ...
                        saveflag = "Edit";
			areagrpcode = selrow.get('mas_area_repgrp_code');
			
			txtAreaGroup.setValue(selrow.get('mas_area_repgrp_name'));
   ...
   var MasAreaformpanel = new Ext.FormPanel({
            ...
            items: [
				...
				else
				{
					...
					if (btn == 'yes')
						{
						Ext.Ajax.request({
		                            	url: 'FrmMasAreaReportGroupSave.php',
                		       	        params:
						{
		                                        savetype    : saveflag,  
	...
	height      : 550,
        width       : 1000,
        y           : 35,
        title       : 'AREA REPORT GROUP MASTER',
