<?php
 ...
 function getsupplier()
    {
        mysql_query("SET NAMES utf8");
	$r=mysql_query("select cust_ref,cust_code from trnpur_purchase_header  ,massal_customer  where phd_sup_code = cust_code  group by cust_ref,cust_code order by  cust_ref");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
    ...
    {
        mysql_query("SET NAMES utf8");
	$r=mysql_query("select cust_ref,cust_code from massal_customer  where cust_type != 'G' group by cust_ref,cust_code order by  cust_ref");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
 ...
 function getIndnoDetail()
	...
	$finid = $_POST['finid'];
        ...
        $r=mysql_query("select ind_no,ind_fin_code from trnpur_indent where ind_hod_auth = 'Y' and ind_comp_code = '$cmpcode' and ind_fin_code = '$finid' and ind_qty > ind_rec_qty and  ind_cancel_status <> 'C' group by ind_no,ind_fin_code order by ind_no,ind_fin_code desc");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
 ...
 function getPonoList()
	...
	$compcode = $_POST['compcode'];
        $finid= $_POST['finid'];
        $vendor = $_POST['vendor'];
        $r=mysql_query("select * from  trnpur_purchase_header where phd_comp_code = '$compcode' and phd_fin_code = '$finid' and phd_sup_code = $vendor order by phd_pono desc ");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
 ...
 function getvendoradd()
    {
        mysql_query("SET NAMES utf8");
	$vendor = $_POST['vendor'];
        $r=mysql_query("select cust_add1,cust_add2,cust_add3,cust_city,cust_taxtag from massal_customer where cust_code = $vendor");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
 ...
 function getitem()
	...
	$finid = $_POST['finid'];
	$compcode = $_POST['compcode'];
//        $r=mysql_query("select itmh_code,itmh_name from trnpur_indent,masrm_item_header 
...
//where ind_item_code = itmh_code and ind_no = '$indno' and ind_fin_code = '$finid' and ind_comp_code = '$compcode' and ind_item_code = '$itmcode'");
        ...
        $r=mysql_query("select ind_machine, ind_qty-ind_rec_qty as ind_qty ,uom_short_name from trnpur_indent,maspur_item_header, mas_uom  
...
function getpoitemdetails()
 ...
 function getAmendmentnoDetail()
	...
	$cmpcode = $_POST['cmpcode'];
        $finid= $_POST['finid'];
        $r=mysql_query("select ifnull(max(amd_no),0)+1 as amendno from   trnpur_amendment_header where amd_comp_code = '$cmpcode' and amd_fin_code = '$finid' ");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
