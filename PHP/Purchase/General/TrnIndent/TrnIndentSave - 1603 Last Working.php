...
if ($savetype == "Add") {
       ...
       mysqli_query($conn,"LOCK TABLES trnpur_indent WRITE");
...
$rate       = ($griddet[$i]['qty'] != 0) ? ($griddet[$i]['value']/$griddet[$i]['qty']) : 0;
...
$remarks = mysqli_real_escape_string($conn, $griddet[$i]['remarks']);
$purpose = mysqli_real_escape_string($conn, $griddet[$i]['purpose']);
$Reason  = mysqli_real_escape_string($conn, $griddet[$i]['Reason']);
