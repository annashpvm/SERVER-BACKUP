...
try {
    ...
    if ($savetype == "Add") {
        ...
        $stmtRun = $conn->prepare($update);
        $stmtRun->bind_param("ii", $compcode, $finid);
        $stmtRun->execute();

        if ($stmtRun->affected_rows == 0) {

            // First time
            $insertRun = "INSERT INTO trn_runningno (compcode, finid, doctype, lastno)
                          VALUES (?, ?, 'IND', 1)";
            $stmtIns = $conn->prepare($insertRun);
            $stmtIns->bind_param("ii", $compcode, $finid);
            $stmtIns->execute();
