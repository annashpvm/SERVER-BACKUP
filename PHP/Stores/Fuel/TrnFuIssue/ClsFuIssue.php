...
function getbatch()
 ...
 function getitem()
	...
	$fincode = $_POST['fincode'];
      $r=mysql_query("call spfu_itemdetails('$compcode','$fincode')");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
function getissno()
	...
	if ($AEDFlag === "Add")
	{
		...
		$r=mysql_query("select ifnull(max(issh_no),0) + 1 as issno from trnfu_issue_header where issh_compcode='$compcode' and issh_fincode='$finid'");
	}
	else if ($AEDFlag === "Edit")
	{
		$r=mysql_query("select issh_no,issh_seqno from trnfu_issue_header where issh_compcode=$compcode and issh_fincode=$finid");
	}
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
...
function getissdetail()
	...
	$AEDFlag = $_POST['AEDFlag'];
		...
		$r=mysql_query("call spfu_sel_issue ('$compcode','$finid','$issno')");
 ...
 function getRate()
    {
        ...
        $qry = "select rect_costrate from trnfu_receipt_header , trnfu_receipt_trailer where rech_seqno = rect_hdseqno and rech_compcode = 1  and rech_fincode >= 25 and rect_item_code = $itemcode group by rect_costrate order by rect_costrate desc";
