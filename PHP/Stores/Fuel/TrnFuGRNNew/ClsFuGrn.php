...
function getitemqty()
{
	...
	$sql = "call spfu_sel_orditem_dets ('$ordcode','$itemcode','$status')";
    $r = mysqli_query($conn, $sql);
...
//        $sql = "select * from acc_ledger_master  where led_type = 'G' and  led_code in (1756,1745,1746,2258)";
 ...
 function getFuelTicketList()
  ...
  //      $sql = "select wc_vehicleno from trn_weight_card where wc_date = '$wbdate' and wc_fincode = '$finid' And wc_compcode ='$compcode' and wc_process = 'N' group by wc_vehicleno order by wc_vehicleno";
      ...
      $sql = "select wc_ticketno from trn_weight_card ,  masfu_item_header  where wc_compcode = $compcode and  itmh_code = wc_itemcode  and    wt_grn_process = 'N' and wc_date >= '2024-04-01' and wc_itemcode > 0 and wc_fincode = '$finid' group by wc_ticketno  order by wc_ticketno desc";
	  $r = mysqli_query($conn, $sql);
        $nrow = mysqli_num_rows($r);
	while($re = mysqli_fetch_array($r))
	{
...
function get_QC_Ticket_Detail()
{
	...
	$sql="select a.*,b.*,c.*,deg.itmh_name as deg_item_name from trn_qc_fuel_inspection a , masfu_item_header b , massal_customer c , masfu_item_header deg  where a.qc_fuel_itemcode = b.itmh_code and a.qc_fuel_supcode = c.cust_code and  a.qc_fuel_degrade_item = deg.itmh_code and qc_fuel_compcode  = $compcode and qc_fuel_fincode = $finid  and qc_fuel_ticketno = $ticketno ";
