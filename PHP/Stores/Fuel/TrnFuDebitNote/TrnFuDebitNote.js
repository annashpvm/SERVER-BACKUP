...
scope: this,
	...
	{header: "Description",     dataIndex: 'description',sortable:true,width:300,align:'left'},
    ],
    store: [],
    listeners:{	
    }
...
specialkey:function(f,e){
	    ...
	    keyup:function()
	    {
      txtRemarks.setRawValue("BEING THE AMOUNT DEBITED TO YOUR ACCOUNT TOWARDS FOR YOUR BILL NO. " + txtPartyBillNo.getRawValue() + " DT. " + Ext.util.Format.date(dtpPartyBilldate.getValue(), "d-m-Y") + ' '+ txtDiffRemarks.getRawValue()+ ' '  + txtMoisRemarks.getRawValue() + ' ' + txtFinesRemarks.getRawValue() + '' + txtSandRemarks.getRawValue()  + ' ' + txtOthDedRemarks.getRawValue()  );
            },
    }
...
var txtTotDedQty = new Ext.form.TextField({
        fieldLabel  : 'Total Deduction Qty & Taxable Value',
        id          : 'txtTotDedQty',
        name        : 'txtTotDedQty',
        width       :  80,
        style       :  {textTransform: "uppercase"},
...
//   if (dntype == 'DNN')
   //    txtTotalDedValue.setRawValue(Ext.util.Format.number(Math.round(totvalue), "0.00"));
//   else
...
function rateChange()
{
	...
	txtDiffRate.setRawValue(txtRate.getValue());
	txtMoisRate.setRawValue(txtRate.getValue());
	txtSandRate.setRawValue(txtRate.getValue());
	txtOthDedRate.setRawValue(txtRate.getValue())
	if (Number(txtRate.getValue()) > 2000)
//	    txtFinesRate.setRawValue(Number(txtRate.getValue())-2000);
	    txtFinesRate.setRawValue(2000.00);

	else
	    txtFinesRate.setRawValue(0);
...
var LoadDebitNoteUpdatechkDataStore = new Ext.data.Store({
      id: 'LoadDebitNoteUpdatechkDataStore',
//      autoLoad : true,
      proxy: new Ext.data.HttpProxy({
                url: 'ClsFuDebitNote.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"LoadDebitNoteUpdateCheck"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
'qc_fuel_billqty','qc_fuel_millqty','qc_fuel_debitnote_date' ,'dbcr_seqno','cust_taxtag'
 ...
 var loadSearchLedgerListDatastore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
          ...
          'cust_code', 'cust_name','qc_fuel_supcode', 'cust_taxtag' 
      ]),
    });
...
'qc_fuel_billqty','qc_fuel_millqty','qc_fuel_debitnote_date','itmh_hsncode','qc_fuel_itemrate'
          ...
          var Cnt = loadQCFuelEntryNoDetailDatastore.getCount();
			...
			dtEntDate.setRawValue(Ext.util.Format.date(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_entrydate'),'d-m-Y'));
	                txtTruckNo.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_truck'));
                        ...
                        txtHSNCode.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('itmh_hsncode'));
			...
			txtMillQty.setRawValue(millwt);
                        ...
                        txtRate.setValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate'));
                        txtDiffRate.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate'));
                        txtMoisRate.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate'));
                        txtSandRate.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate'));
                        txtOthDedRate.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate'))
                        if (loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate') > 2000)
//                            txtFinesRate.setRawValue(loadQCFuelEntryNoDetailDatastore.getAt(0).get('qc_fuel_itemrate')-2000);
                            txtFinesRate.setRawValue(2000.00);
...
var cmbDNNo = new Ext.form.ComboBox({
        fieldLabel: 'Debit Note No',
        width           : 120,
        displayField    : 'qc_fuel_debitnote_no', 
        valueField      : 'qc_fuel_debitnote_no',
        hiddenName      : '',
        id              : 'cmbDNNo',
        typeAhead       : true,
        mode            : 'local',
...
/*
		LoadDebitNoteUpdatechkDataStore.removeAll();
		LoadDebitNoteUpdatechkDataStore.load({
		url: 'ClsFuDebitNote.php',
		params:
		{
			task:"LoadDebitNoteUpdateCheck",
			fincode : ginFinid,
			compcode: ginCompcode,
	        ...
	        callback: function () {
		      ...
		      var cnt = LoadDebitNoteUpdatechkDataStore.getCount();
		      if (cnt>0)
		      {
                        if (LoadDebitNoteUpdatechkDataStore.getAt(0).get('dbcr_accseqno') == 0)
                            Ext.getCmp('save').setDisabled(false);  
                        else
                        {
                            alert("Accounts Updation has been Done. You Can't Modify ");
		...
		params:
		{
	        ...
	        callback: function () {
		      ...
		      if (cnt>0)
		      {
             		...
             		ledcode = LoadDebitNoteVoucherDetailDataStore.getAt(0).get('qc_fuel_supcode');
...
//	alert(LoadDebitNoteVoucherDetailDataStore.getAt(0).get('qc_fuel_rounding'));
...
var ledgercode2 = '';
        if (dngsttype == "G")
        { 
            var final_ded_value =   Number(txtTotalDedValue.getRawValue())+  Number(txtRounding.getRawValue());
   ledgername2 = 'CREDIT NOTE TO BE RECEIVED';
   ledgercode2 = '4993';
        } 
        else
        {  
            var final_ded_value =   Number(txtDebitValue.getRawValue());
...
function save_clickNew()
{
				...
				Ext.Msg.prompt('Reason for Modification', '', function(btn, text){
				    ...
				    {
					Ext.Msg.alert('Debit Note','Grid should not be empty.. Please click on LEDGER POSTING tab and continue');
					gstSave="false";
				    }
                            else if(Number(txttotDebit.getValue())!=Number(txttotCredit.getRawValue())){
                            Ext.MessageBox.alert("DN","The Transactions Debit and Credit Amount are not  Equal");
                        }
                                               ...
                                               Ext.each(accData, function (record) {
                                                ...
                                                Ext.Ajax.request({
                                                    ...
                                                    params: {
                                                        ...
                                                        narration: txtRemarks.getRawValue(),
                                                        voutype: dntype,
                                                        paymode: "",
                                                        output: 'N',
                                                        payno: "",
...
//-- Additional 
                                                    ...
                                                    callback: function (options, success, response)
                                                    {
                                                        ...
                                                        if (obj['success'] === "true") {
                                                            RefreshData();
							    Ext.Msg.alert("Record saved with Voucher No - -" + obj['vouno']);
							    DebitNoteFormPanel.getForm().reset();
							    flxAccounts.getStore().removeAll();
							    RefreshData();
...
//Edit
                    ...
                    listeners: {
                        click: function () {
                               ...
                               if (dngsttype == "G")
                                   cmbDNNo.setRawValue('DNG/');
                               else
                                   cmbDNNo.setRawValue('DNN/');
