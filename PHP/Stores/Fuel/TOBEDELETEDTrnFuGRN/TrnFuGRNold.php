...
/*
	...
	$freval=$gridfre[$i]['freval'];
	 $query5= "Update trnfu_receipt_trailer Set rect_freightvalue='$freval' Where rect_hdseqno='$rech_seqno' AND rect_seqno='$sno'"; $result5=mysqli_query($conn, $query5);
	}
*/
	}
...
if ($gstFlaggrn === "Confirm"){
			...
			$accupd3 = mysqli_query($conn, $accqry3);
			$gst =0;
			...
			if ($itemval2 > 0){
				...
				$accupd4 = mysqli_query($conn, $accqry4);
			}
			...
			if ($frtval > 0){
				...
				$accupd5 = mysqli_query($conn, $accqry5);
			...
			if (($billqtyt * $handlingmt) > 0){
				...
				$accupd6 = mysqli_query($conn, $accqry6);
			...
			if ($unloadexpt > 0){
				...
				$recjk = mysqli_fetch_array($resjk);
				...
				$accupd7 = mysqli_query($conn, $accqry7);
