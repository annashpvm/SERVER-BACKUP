...
//	$sql = "call spfu_sel_recitems ('$compcode','$grnno','$ordno','0')");
 ...
 function getgrnno()
	...
	if($gstFlag === "Add")
	{
           if ($finid <24)
        	...
        	$sql = "select ifnull(max(convert(substring(rech_no,3),signed)),0) +1 as grnno from trnfu_receipt_header where rech_fincode = $finid and rech_compcode = $compcode");
 ...
 function getgrneddt()
	...
	$sql = "select * from trnfu_receipt_header ,massal_customer where rech_seqno= '$grnno' and  cust_code= rech_sup_code");

	$sql = "select * from trnfu_receipt_header ,massal_customer,mas_RMFU_purchasetax where rech_purgrp = tax_purcode and rech_seqno= '$grnno' and  cust_code= rech_sup_code");
...
//        $sql = "select * from acc_ledger_master  where led_type = 'G' and  led_code in (1756,1745,1746,2258)");
 ...
 function getFuelTruckList()
	...
	$finid    = $_POST['finid'];
      ...
      $sql = "select wc_ticketno from trn_weight_card left join mas_wb_item on  replace(wc_item,' ','')  = replace(item_name,' ','')   left join mas_wb_itemgroup on item_grpcode = item_group  where wc_compcode = '$compcode'  and wc_fincode = '$finid' and  replace(item_grpname,' ','') in ('BIOMASS','COALITEMS') and  wt_grn_process = 'N' and wc_date >= '2024-01-01' group by wc_ticketno  order by wc_ticketno desc
 ");

      $sql = "select wc_ticketno from trn_weight_card ,  masfu_item_header  where wc_compcode = $compcode and  itmh_code = wc_itemcode  and    wt_grn_process = 'N' and wc_date >= '2024-04-01' group by wc_ticketno  order by wc_ticketno desc");
        $nrow = mysqli_num_rows($r);
	while($re = mysqli_fetch_array($r))
	{
	$arr[]= $re ;
