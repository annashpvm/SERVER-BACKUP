...
//	$r=mysql_query("call spfu_sel_recitems ('$compcode','$grnno','$ordno','0')");
 ...
 function getgrneddt()
	...
	$r=mysql_query("select * from trnfu_receipt_header ,massal_customer where rech_seqno= '$grnno' and  cust_code= rech_sup_code");

	$r=mysql_query("select * from trnfu_receipt_header ,massal_customer,mas_RMFU_purchasetax where rech_purgrp = tax_purcode and rech_seqno= '$grnno' and  cust_code= rech_sup_code");
...
//        $r=mysql_query("select * from acc_ledger_master  where led_type = 'G' and  led_code in (1756,1745,1746,2258)");
 ...
 ");
      ...
      $r=mysql_query("select wc_ticketno from trn_weight_card ,  masfu_item_header  where wc_compcode = $compcode and  itmh_code = wc_itemcode  and    wt_grn_process = 'N' and wc_date >= '2024-04-01' group by wc_ticketno  order by wc_ticketno desc");
        $nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
