...
//bind: '{myValue}',
  ...
  var loadInvoicelistDataStore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task:"loadEndInvNo"}, // this parameter asks for listing
  ...
  var LoadCustomerDetailDatastore = new Ext.data.Store({
	...
	'cust_gstin','cust_state','cust_led_code'
      ])
    });
...
//      autoLoad : true,
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
          ...
          'cust_code', 'cust_name','cust_gstin','cust_state','cust_led_code'
...
function PartySearch()
{
  ...
  var LoadCustomerDetailDatastore2 = new Ext.data.Store({
	...
	'cust_gstin','cust_state','cust_code','cust_gst_type','os_date','U_TCSStatus','E_inv_confirm'
      ])
    });
...
'salitem_cgstper', 'salitem_sgstper','salitem_igstper','salitem_salesledcode_tn', 'salitem_salesledcode_os', 'salitem_cgstledcode', 'salitem_sgstledcode',  'salitem_igstledcode', 'tn_sal_ledger','os_sal_ledger', 'cgst_ledger', 'sgst_ledger', 'igst_ledger', 'os_seqno', 'os_rounding','cust_name','os_acc_seqno'
      ])
    });
...
//					alert(cgst_val);
				...
				txtnetamt.setRawValue(Ext.util.Format.number(totamt,'0.00'));
			...
			LoadCustomerDetailDatastore2.load({
					...
					compcode:Gincompcode,
                                	invno:cmbSalInvNo.getRawValue(),
                                        gsttype : cmbSalInvNo.getRawValue().substring(0,4),
				...
				txtGSTIN.setValue(LoadCustomerDetailDatastore2.getAt(0).get('cust_gstin'));
                                 ...
                                 if (LoadCustomerDetailDatastore2.getAt(0).get('E_inv_confirm') == "Y" &&  LoadCustomerDetailDatastore2.getAt(0).get('U_TCSStatus') == "S")
                                 {
                                     alert("E-Invoice Generated.  You can't Delete..")
                                     Ext.getCmp('delete').setDisabled(true);
                                     txtPassword.hide();
                                 }
                                 else
...
//				scope: this,
                  		    ...
                  		    {  
                                           ...
                                           custledcode = LoadsaleinvnoDetailDatastore.getAt(0).get('os_custcode');
...
function flxaccupdation() {
	...
	flxDetail.getSelectionModel().selectAll();
        ...
        for(var i=0;i<Row;i++)
        {
            ...
            salamt    =  Number(sel[i].data.taxablevalue); 
            ...
            cgstamt   =  Number(sel[i].data.cgstval);
            sgstamt   =  Number(sel[i].data.sgstval);
            igstamt   =  Number(sel[i].data.igstval);
...
alert(j);
//                  dr=dr+Number(sel1[j].data.debit);
//                  cr=cr+Number(sel1[j].data.credit);
                  dr=dr+Number(sel1[j].get('debit'));
                  cr=cr+Number(sel1[j].get('credit'));
...
function datecheck()
  {
        ...
        var dt_invoice = dtsales.getValue();
        var diffdays = (dt_today.getDate()- dt_invoice.getDate());

        if (diffdays >1)
        {     
             alert("You are Not Allowed to Cacnel the invoice in the date of " +  Ext.util.Format.date(dtsales,"d-m-Y"));
...
//            {header: "S.No  ", dataIndex: 'slno',sortable:true,width:30,align:'left'},    
		...
		{header: "ledcode", dataIndex: 'cust_led_code',sortable:true,width:330,align:'left',hidden:true},
...
var dgrecord = Ext.data.Record.create([]);
      var flxDetail = new Ext.grid.EditorGridPanel({
		...
		{dataIndex:'totamt',header: "Total Amt",width: 60,align: 'right',sortable: true},
		{dataIndex:'editcode',header: "Edit Qty",width: 60,align: 'right',sortable: true},
		{dataIndex:'salledcode',header: "S.Ledcode",width: 60,align: 'right',sortable: true},
		{dataIndex:'salledname',header: "S.LedName",width: 60,align: 'right',sortable: true},
		{dataIndex:'cgstlcode',header: "CGST.Ledcode",width: 60,align: 'right',sortable: true},
		{dataIndex:'cgstlname',header: "CGST.Ledname",width: 60,align: 'right',sortable: true},
		{dataIndex:'sgstlcode',header: "SGST.Ledcode",width: 60,align: 'right',sortable: true},
		{dataIndex:'sgstlname',header: "SGST.Ledname",width: 60,align: 'right',sortable: true},
		{dataIndex:'igstlcode',header: "IGST.Ledcode",width: 60,align: 'right',sortable: true},
...
var myFormPanel = new Ext.form.FormPanel({
        width        :  1050, 
        title        : 'DELETE - MISCELLANEOUS SALES',
        style        : 'margin: 5px ',
        height       : 560,
        frame        : false,
...
//alert(txtSalesInvNo.getValue());
			...
			{   
		        		   ...
		        		   buttons: Ext.MessageBox.YESNO,
		                           msg: 'Are You sure Want to Delete this Invoice',
		                    	   fn: function(btn)
					   {         
					      if (btn == 'yes')
			                      { 
      					       ...
      					       var finupdData = new Array();
                                               ...
                                               Ext.Ajax.request({
				               url: 'TrnOtherSalesSaveDelete.php',
				               params:
						{
	
	           				snhinvno     : cmbSalInvNo.getRawValue(),  
						...
						{    
	                                    ...
	                                    Ext.MessageBox.alert("Other Sales Invoice Deleted -" + obj['saleno']);
					   ...
					   // RefreshGridData(); 
						...
						{
            Ext.MessageBox.alert("Other Sales Invoice Not Cancelled!- " + obj['saleno']);                                                  
                                     }
                                }
		         //-- loop Z end           
					...
					callback:function()
					{
                                       ...
                                       if (LoadsaleinvnoDatastore.getAt(0).get('ss_invno') < 10)
                                        ...
                                        {  
                                         ...
                                         txtSalesInvNo.setRawValue(gsttype+'/'+ino+'/'+invfin);
