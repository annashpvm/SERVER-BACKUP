     ...
     var loadGSTLedgerDatastore = new Ext.data.Store({
          ...
          reader: new Ext.data.JsonReader({
            ...
            id: 'id'
              ...
              'tax_code', 'tax_name', 'tax_cgstper', 'tax_sgstper', 'tax_igstper', 'tax_cgst_ledcode', 'tax_sgst_ledcode', 'tax_igst_ledcode', 'tax_cgst_ledname', 'tax_sgst_ledname', 'tax_igst_ledname', 'tax_gst'
          ]),
        });
...
*/    
    ...
    'minh_frt_ledger', 'minh_frt_cgstledger', 'minh_frt_sgstledger', 'minh_frt_igstledger' ,'mint_billrate', 'mint_povalue'
...
//alert("TEst") ;      
    ...
    // alert(landing); 
           ...
           txtigstvalDiff.setRawValue(diffigst.toFixed(2)) ;
            ...
            txtTotRebate1.setValue(Ext.util.Format.number(rebate,'0.00'));
            
            txtTotRebate2.setValue(Ext.util.Format.number(rebate2,'0.00'));
                    ...
                    callback: function(){
                         ...
                         vno =  vno.trim() +'/'+ invfin; 
                            ...
                            if (gstFlag == "Add")
                               ...
                               txtDNVouNo.setValue(GRNdetailsLoadDataStore.getAt(0).get('rech_dnno'));    
    ...
    function calculateItemValue(){
        ...
        //       value2 = value.toFixed(2);
          ...
          txtTCSVal.setRawValue(0); 
           ...
           value = Number(value) - Number(txtdisval.getRawValue()) - Number(txtRebate.getRawValue()); 
           value2 = Number(value2) - Number(txtdisval.getRawValue()) - Number(txtRebate.getRawValue()); 
    ...
    */
    ...
    //       invitemvalue =  Number(taxable)+Number(cgst)+Number(sgst)+Number(igst)+Number(txtothers.getRawValue())+Number(txtothersPM.getRawValue())+Number(txtinward.getRawValue())+Number(txtclrfreight1.getRawValue());
           ...
           invitemvalue =  Number(taxable)+Number(cgst)+Number(sgst)+Number(igst)+Number(txtothersPM.getRawValue())+Number(txtinward.getRawValue()) +Number(txtTransport.getRawValue()) ;
    ...
    //       sumitemvalue =  Number(taxable)+Number(txtothersPM.getRawValue())+Number(txtinward.getRawValue())+Number(txtclrfreight1.getRawValue())  +Number(txtothers.getRawValue()) +  Number(txtTCSVal.getRawValue()) -  Number(txtRebate.getValue()) ;
           ...
           sumitemvalue =  Number(taxable)+Number(txtothersPM.getRawValue())+Number(txtinward.getRawValue())+Number(txtclrfreight1.getRawValue()) +  Number(txtTCSVal.getRawValue()) -    Number(txtRebate2.getValue()) ;
    
    
           invitemvalue = Number(invitemvalue) +  Number(txtTCSVal.getRawValue()) -   Number(txtRebate2.getValue()); 
    ...
    //alert(GRNdetailsLoadDataStore.getAt(j).get('mint_item_spec'));
                                   ...
                                   flxDetail.getStore().getCount(),
                                     ...
                                     sno               : sno1,
                                         ...
                                         cgstled           : GRNdetailsLoadDataStore.getAt(j).get('tax_cgst_ledcode'),
                                         sgstled           : GRNdetailsLoadDataStore.getAt(j).get('tax_sgst_ledcode'),
                                         igstled           : GRNdetailsLoadDataStore.getAt(j).get('tax_igst_ledcode'),
                                         cgstledname       : GRNdetailsLoadDataStore.getAt(j).get('tax_cgst_ledname'),
                                         sgstledname       : GRNdetailsLoadDataStore.getAt(j).get('tax_sgst_ledname'),
                                         igstledname       : GRNdetailsLoadDataStore.getAt(j).get('tax_igst_ledname'),
                                         cgstpm            : mcgstpm,
                                         sgstpm            : msgstpm,
                                         igstpm            : migstpm,
...
//                                 Ext.getCmp('save2').setDisabled(true);
   ...
   //                              Ext.getCmp('save2').setDisabled(true);
    ...
    //          flxAccounts.getSelectionModel().selectAll();
              ...
              if ( Number(txtTotRebate2.getValue()) >0) {
                      var RowCnt1 = flxAccounts.getStore().getCount() + 1;
                      flxAccounts.getStore().insert(
                          flxAccounts.getStore().getCount(),
                          new dgrecord({
    ...
    //			      debit     : rebate,
                      ...
                      credit    : Ext.util.Format.number(txtTotRebate2.getValue(),'0.00'),
    ...
    */
    ...
    specialkey:function(f,e){
       ...
       var txtRebate2 = new Ext.form.NumberField({
        fieldLabel  : 'Rebate2',
        id          : 'txtRebate2',
        width       : 60,
        name        : 'txtRebate2',
    ...
    var txtTotRebate1 = new Ext.form.NumberField({
        fieldLabel  : 'Tot Rebate1',
        id          : 'txtTotRebate1',
        width       : 75,
        name        : 'txtTotRebate1',
    labelStyle  : "font-size:14px;font-weight:bold;color:#0080ff",
    ...
    var txtTotRebate2 = new Ext.form.NumberField({
        fieldLabel  : 'Tot Rebate2',
        id          : 'txtTotRebate2',
        width       : 75,
        name        : 'txtTotRebate2',
    labelStyle  : "font-size:14px;font-weight:bold;color:#0080ff",
    ...
    else
    {
            ...
            var tax =   Number(txtCGSTPer.getValue()) +Number(txtSGSTPer.getValue())+Number(txtIGSTPer.getValue());  
                       ...
                       callback: function(records, operation, success) 
                   {
                    ...
                    if (!success || loadGSTLedgerDatastore.getCount() === 0) {
                        Ext.Msg.alert("Error", "No GST details found for the selected group.");
                        return;
                    }
        

            cgstledcode = loadGSTLedgerDatastore.getAt(0).get('tax_cgst_ledcode');
            sgstledcode = loadGSTLedgerDatastore.getAt(0).get('tax_sgst_ledcode');
            igstledcode = loadGSTLedgerDatastore.getAt(0).get('tax_igst_ledcode');
            cgstledname = loadGSTLedgerDatastore.getAt(0).get('tax_cgst_ledname');
            sgstledname = loadGSTLedgerDatastore.getAt(0).get('tax_sgst_ledname');
            igstledname = loadGSTLedgerDatastore.getAt(0).get('tax_igst_ledname');
