...
'mint_insurance','acctrail_adj_value','mint_recdqty_bill','mint_item_unit','minh_grn_status','mint_item_spec'
...
function grid_tot(){
	...
	var value2 = 0;
        ...
        for(var i=0;i<Row;i++)
        {
            ...
            value=value+ (Number(sel[i].data.mintinvqty) * Number(sel[i].data.mintunitrate));
            value2=value2+ (Number(sel[i].data.mintinvqty) * Number(sel[i].data.mintunitrate));
...
//alert(grnrounding);
     //      txtroundoff.setValue(grnrounding);
           totvalue = value - disc + pf + frt + cgst + sgst + igst + others + otherpm - rebate + Number(txtroundoff.getValue()) + ins + tcs;
            }
            else
            {
           totvalue = value - disc + pf + frt + cgst + sgst + igst + others + otherpm - rebate + Number(txtroundoff.getValue()) + ins + tcs;
        ...
        {
           totvalue = value - disc + pf + frt + cgst + sgst + igst + others + otherpm - rebate + ins + tcs;
        }
...
function add_btn_click()
{
	    ...
	    if (mintype == "I")
         ...
         //       indfincode = cmbindno.getValue();
            ...
            if (Number(txtStockQty.getValue())===0  ){
...
//alert(cmbItem.getRawValue());
			...
			sel[idx].set('igstpm'   , '0');
                 	sel[idx].set('mintitemcode'   , pitemcode);
                  	...
                  	sel[idx].set('itemspec', Number(txtItemSpec.getValue()));
