...
*/
        $r=mysql_query("select ifnull(max(iss_no),0) + 1 as issno from trnpur_item_issues where iss_comp_code=$compcode and iss_fin_code=$finid and iss_type = '$isstype'");
 ...
 function getSearchitemlist()
	...
	$compcode = $_POST['compcode'];
        ...
        if ($item == '')   
           ...
           $qry = "select * from maspur_item_header h , maspur_item_trailer t   where item_comp_code = $compcode and item_fin_code = $finid  and h.item_code = t.item_code and item_stock > 0 and replace(replace(item_name,' ','')  ,'.','') like '%$item%' order by item_name";         
