...
var loadMonthWiseGRNData = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task:"loadGroupMonthGRNS"}, // this parameter asks for listing
...
//        header : false,
    ...
    listeners:{	
        'cellclick' : function(flxDesc, rowIndex, cellIndex, e){
			...
			var chkitem = (selrow.get('grp_name'));
                        itemcode = 0;
			if ((selrow != null)){
...
var lblGroup = new Ext.form.Label({
 ...
 var loadGroupPurchaseDataStore = new Ext.data.Store({
      id: 'loadGroupPurchaseDataStore',
      proxy: new Ext.data.HttpProxy({
                url: '/SHVPM/Stores/ClsGSRep.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadGroupMonthGRNDETAILS"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
function find_dates(mmon)
{
    ...
    loadGroupPurchaseDataStore.removeAll();
    loadGroupPurchaseDataStore.load({
        url: '/SHVPM/Stores/ClsGSRep.php',
        params: {
    	task: 'loadGroupMonthGRNDETAILS',
        compcode:GinCompcode,
        finid:GinFinid,
        startdate: Ext.util.Format.date(monthstartdate,"Y-m-d"),  
	...
	{
     //     grid_tot_party();
        //  alert(loadGroupPurchaseDataStore.getAt(0).get('purtype'));
        }
    });
...
var flxGRN = new Ext.grid.EditorGridPanel({
    ...
    [ 	 	
        ...
        {header: "Type" , dataIndex: 'purtype',sortable:false,width:70,align:'left', hidden: false},
     ...
     store: loadGroupPurchaseDataStore,
    listeners:{	
...
ReppreprintWindow.hide();
	...
	x	    : 80,
        y           : 35,
        title       : 'GROUP WISE STORES RECEIPT',
