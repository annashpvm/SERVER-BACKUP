...
function getAdjNodetails()
	...
	$isstype  = $_POST['isstype'];
        ...
        $sql = "call sp_pur_issue_details($compcode,$finid,$issno,'$isstype')";
    $r = mysqli_query($conn, $sql);
...
function getAdjNo()
	...
	$adjtype = $_POST['adjustmenttype'];
        ...
        $sql = "select ifnull(max(iss_no),0) + 1 as adjdocno from trnpur_item_issues where iss_comp_code=$compcode and iss_fin_code=$finid and iss_type = '$adjtype'";
    $r = mysqli_query($conn, $sql);
 ...
 function getSearchitemlist()
	...
	$compcode = $_POST['compcode'];
        ...
        if ($item == '')   
           ...
           $sql = "select * from maspur_item_header h , maspur_item_trailer t   where item_comp_code = $compcode and item_fin_code = $finid  and h.item_code = t.item_code and  replace(replace(item_name,' ','')  ,'.','') like '%$item%' order by item_name";         
