Ext.onReady(function(){
    ...
    var txtGRNyr = new Ext.form.NumberField({
            fieldLabel  : 'Year',
            id          : 'txtGRNyr',
            width       : 30,
            name        : 'txtGRNyr',
    ...
    //              url: 'TrnGRNReturnSave.php',
            ...
            taxable    : txtTotalValue.getValue(),
    ...
    function save_click()
    {
        ...
        todate = "03/31/"+gstfinyear.substring(5,9);
            ...
            for (var i = 0; i < selrows; i++) 
            {
                ...
                if (Number(rec.get('mintinvqty')) < Number(rec.get('rejectedqty')))
                {
                rec.set('rejectedvalue',0);
                chkqty = 1;
                }
            }  
        ...
        if(Ext.util.Format.date(dtpRR.getValue(),"Y-m-d") < Ext.util.Format.date(fromdate,"Y-m-d")){
                Ext.MessageBox.alert("Alert","GRN Return Date is not in current finance year. Please check");
        }
    
    
        else if(Number(txtOthers.getValue()) > 0 && cmbOtherLedger.getValue() == 0 ){
    
            Ext.MessageBox.alert("GRN Return","Select Other Charges Ledger Name");
        }
            
    
        else if(Ext.util.Format.date(dtpRR.getValue(),"Y-m-d") > Ext.util.Format.date(todate,"Y-m-d")){
                Ext.MessageBox.alert("Alert","GRN Return Date is not in current finance year. Please check");
        }
            ...
            Ext.MessageBox.alert("GRN Return","The Transaction of Debit and Credit Totals are not  Equal");
        }
            ...
            {
                   Ext.Msg.alert('GRN Return','Return Qty is Greather then GRN QTY');
            }
        ...
        {               
           ...
           {         
            ...
            igst_amount     : txttotigst.getValue(),
                    ...
                    sgst_ledcode  : sgst_ledcode,
                    igst_ledcode  : igst_ledcode,
            addnl_amt     : Number(txtOthers.getValue()),
            otherspm      : Number(txtTotOthersPM.getValue()),
            addnl_ledcode : cmbOtherLedger.getValue(),
        ...
        autoCreate:{tag:'input',type:'text',size:'20',autocomplete:'off',maxlength:'5'},
            ...
            listeners:{
              change:function(){
                  ...
                  if (Number(txtCGSTPM.getValue()) > 15)
                  {   
                      txtCGSTPM.setValue(0);
                      txtSGSTPM.setValue(0);
                  } 
        ...
        listeners:{
            select: function () {
                           ...
                           callback: function () 
                   {
                               ...
                               txtGRNyr.setValue(ReceiptReturnHeaderLoadDataStore.getAt(0).get('debh_grn_year'));  
    ...
    //alert(cnt);
                     ...
                     {  
                         ...
                         cgst_ledcode  = GRNdetailsLoadDataStore.getAt(0).get('tax_cgstledcode');
                     sgst_ledcode  = GRNdetailsLoadDataStore.getAt(0).get('tax_sgstledcode');
                     igst_ledcode  = GRNdetailsLoadDataStore.getAt(0).get('tax_igstledcode');
    
    
                    cgst_percentage = GRNdetailsLoadDataStore.getAt(0).get('mint_cgst_per');
                    sgst_percentage = GRNdetailsLoadDataStore.getAt(0).get('mint_sgst_per');
                    igst_percentage = GRNdetailsLoadDataStore.getAt(0).get('mint_igst_per');
    ...
    */
                             ...
                             totqty:'0',
                                      itc:'N',
                                         ...
                                         mintinsurance     : GRNdetailsLoadDataStore.getAt(j).get('mint_insurance'), 
    ...
    // Return details are loaded
                   ...
                   for(var ii=0; ii<cntrr; ii++)
                       { 
    
                             if (itemcode == ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_item_code'))
                             {
    
        
    
                                var acceptqty =  Number(rec.get('mintacceptqty')) + Number(ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_qty'));
    
                            rec.set('mintacceptqty',acceptqty);
                    rec.set('rejectedqty',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_qty'));
                    rec.set('rejectedvalue',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_value'));
                    rec.set('disc',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_discount'));
                    rec.set('discamt',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_disamt'));
                    rec.set('pfamt',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_pfamt'));
                    rec.set('cgstper',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_cgst_per'));
                    rec.set('sgstper',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_sgst_per'));
                    rec.set('igstper',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_igst_per'));
                    rec.set('cgstamt',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_cgst_amt'));
                    rec.set('sgstamt',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_sgst_amt'));
                    rec.set('igstamt',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_igst_amt'));
                    rec.set('others',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_others'));
                    rec.set('freight',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_freight'));
    
                    rec.set('tcsper',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_tcsper'));
                    rec.set('tcsval',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_tcsamt'));
    
                    rec.set('pfyesno',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_pf_yn'));
                    rec.set('otheryesno',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_other_yn'));
                    rec.set('freightyesno',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_frt_yn'));
                    rec.set('insyesno',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_ins_yn'));
                    rec.set('rebate',0);
                    rec.set('itemvalue',ReceiptReturnHeaderLoadDataStore.getAt(ii).get('debt_item_value'));
                             }      
    ...
    //alert(NewDays);
            if (diffdays > 2)
            {     
                 alert("You are Not Allowed to Raise the GRN RETURN in the date of " +  Ext.util.Format.date(dtgrn,"d-m-Y"));
                 if (gstFlag == "Add")
            ...
            if (diffdays <= 0)
            {     
                 alert("System will not allow to raise the GRN RETURN in Future date");
                 dtpgrn.setRawValue(Ext.util.Format.date(dt_today,"d-m-Y"));
    ...
    function editdatecheck()
      {
            ...
            if (diffdays > 2)
            {     
                 alert("You are Not Allowed to Modify the GRN RETURN in the date of " +  Ext.util.Format.date(dtgrn,"d-m-Y"));
                 if (gstFlag == "Add")
                    dtpRR.setRawValue(Ext.util.Format.date(dt_today,"d-m-Y"));
                 else
                 {    
                 Ext.getCmp('save').setDisabled(true);         
    ...
    var cmbGSTYN = new Ext.form.ComboBox({
            fieldLabel      : 'GST(Y/N)',
            width           : 80,
            displayField    : '', 
            valueField      : '',
    ...
    */
    ...
    var othcharge =  Number(txttotothval.getValue())  + Number(txtTotOthersPM.getValue());
                  if ( Number(othcharge) >0) {
                        var RowCnt1 = flxAccounts.getStore().getCount() + 1;
                        flxAccounts.getStore().insert(
                            flxAccounts.getStore().getCount(),
                            new dgrecord({
    ...
    //-- For Purchase Ledger
        ...
        //        flxAccounts.getSelectionModel().selectAll();
            ...
            for (var i1 = 0; i1 < selrows; i1++) {
                ...
                {
                    cramt =  puramt + Number(rec.get('credit'));
                    k = 1;
                    rec.set('credit', Ext.util.Format.number(cramt,"0.00"));
    ...
    //-- For CGST Ledger
            ...
            for (var i1 = 0; i1 < selrows; i1++) {
                ...
                {
                    cramt =  cgstamt + Number(rec.get('credit'));
                    k = 1;
                    rec.set('credit', Ext.util.Format.number(cramt,"0.00"));
    ...
    //-- For SGST Ledger
            ...
            for (var i1 = 0; i1 < selrows; i1++) {
                ...
                {
                    cramt =  sgstamt + Number(rec.get('credit'));
                    k = 1;
                    rec.set('credit', Ext.util.Format.number(cramt,"0.00"));
    ...
    //-- For IGST Ledger
            ...
            for (var i1 = 0; i1 < selrows; i1++) {
                ...
                {
                    cramt =  igstamt + Number(rec.get('credit'));
                    k = 1;
                    rec.set('credit', Ext.util.Format.number(cramt,"0.00"));
    ...
    //alert(selrows)
            for (var i = 0; i < selrows; i++) {
                ...
                if (isNaN(rec.get('rejectedqty')))
                {
                rec.set('rejectedqty',0);
                rec.set('rejectedvalue',0);
                rec.set('discamt',0);
                rec.set('rebate',0);
                rec.set('tcsval',0);
                ...
                {
                retvalue =   Number(rec.get('mintunitrate')) * Number(rec.get('rejectedqty'));
//                rec.set('rejectedvalue',retvalue);
                rec.set('rejectedvalue', parseFloat(retvalue).toFixed(2));
                ...
                else  if ( Number(rec.get('mintdisamt'))  > 0 && retvalue > 0 )
                {   
        ...
        if (rec.get('otheryesno') == 'Y')
        {
               if ( Number(rec.get('mintothers'))  > 0 && retvalue > 0  )  {
                   othervalue = Number(rec.get('mintothers')) / Number(rec.get('mintrcvdqty'))  * Number(rec.get('rejectedqty'));
               rec.set('others',othervalue.toFixed(2));
                } 
            }
        ...
        if (rec.get('freightyesno') == 'Y')
        {
    
               if ( Number(rec.get('mintfreight'))  > 0 && retvalue > 0  )  {
         ...
         rec.set('otherpm',otherPMvalue);
               if ( Number(rec.get('mintotherpm'))  > 0 && retvalue > 0  )  {
                   otherPMvalue = Number(rec.get('mintotherpm')) / Number(rec.get('mintrcvdqty'))  * Number(rec.get('rejectedqty'));
               rec.set('otherpm',otherPMvalue.toFixed(2));
               } 
    
    insval = 0;
    
        if (rec.get('insyesno') == 'Y')
        {
    
                if ( Number(rec.get('mintinsurance'))  > 0 && retvalue > 0 )  {
                   insval  = Number(rec.get('mintinsurance')) / Number(rec.get('mintrcvdqty'))  * Number(rec.get('rejectedqty'));
           ...
           else
           {
               ...
               if (Number(rec.get('mintrebate')) > 0 && Number(rec.get('rejectedqty')) > 0)
...
//                rec.set('itemvalue', Number(landvalue));
                rec.set('itemvalue', parseFloat(landvalue).toFixed(2));
    ...
    */
                        focus: function () {
                                ...
                                rate = Number(selrow.get('mintunitrate'));
                                 grnqty = Number(selrow.get('mintinvqty'));
    ...
    */
            ...
            {dataIndex:'discamt', header: "Dis Value",width: 60,align: 'center',sortable: true},
             ...
             {dataIndex:'otheryesno', header: "OTHER YES/NO",width: 60,align: 'center',sortable: true},
            ...
            {dataIndex:'igstper', header: "IGST %",width: 60,align: 'center',sortable: true},
             ...
             {dataIndex:'freightyesno', header: "FRT YES/NO",width: 60,align: 'center',sortable: true},
            ...
            {dataIndex:'insurance', header: "insurance",width: 50,align: 'center',sortable: true}, 
               {dataIndex:'insyesno', header: "INS YES/NO",width: 60,align: 'center',sortable: true},
            ...
            {dataIndex:'mintinsurance', header: "Insurance",width: 50,align: 'center',sortable: true},           
         ],
        store: [],
        listeners:{	
                    ...
                    for (var i = 0; i < selected_rows.length; i++)
                {
                    ...
                    for (var i = 0; i < selected_rows.length; i++)
                {
                    if (selected_rows[i].data.otheryesno == '')
                                        flag = 'N';
                                    else                                   
                                        flag = selected_rows[i].data.otheryesno;
                    ...
                    for (var i = 0; i < selected_rows.length; i++)
                {
                    if (selected_rows[i].data.freightyesno == '')
                                        flag = 'N';
                                    else                                   
                                        flag = selected_rows[i].data.freightyesno;
                                                 
    
                                       colname = 'freightyesno';
                    if (flag == 'N')
                    {
                        selected_rows[i].set(colname, 'Y');
                    } else 
                    ...
                    for (var i = 0; i < selected_rows.length; i++)
                {
                    if (selected_rows[i].data.insyesno == '')
                                        flag = 'N';
                                    else                                   
                                        flag = selected_rows[i].data.insyesno;
    ...
    */
    ...
    //save
              ...
              var p4 = "&vouno=" + encodeURIComponent(txtVouNo.getValue());
