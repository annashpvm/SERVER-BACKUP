Ext.onReady(function(){
 ...
 var loadPartyGRNDataStore = new Ext.data.Store({
      id: 'loadPartyGRNDataStore',
      proxy: new Ext.data.HttpProxy({
                url: 'ClsViewStatements.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadAllParty_Item_GRNwise_Arrivals"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
        id: 'id' 
      },['rech_date', 'rech_no', 'rech_billno', 'rech_billdate',  'cust_name', 'itmh_name','rect_itemrate','rect_grnqty','rect_itemvalue','rech_sup_code','rect_item_code'
]),
    });
...
var lblItem2 = new Ext.form.Label({
        fieldLabel  : '',
        id          : 'lblItem2',
        name        : 'lblItem2',
	style: {
 ...
 var loadItemArrivalsDataStore = new Ext.data.Store({
      id: 'loadItemArrivalsDataStore',
      proxy: new Ext.data.HttpProxy({
                url: 'ClsViewStatements.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadItemwiseArrivals"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
        id: 'id' 
      },['itmh_name', 'itmh_code', 'grnqty', 'grnvalue','avgrate' ]),
    });
...
function grid_tot_itemparty(){

        var twt = 0;
        var tval = 0;

         txtItemPartyPurchaseQty.setValue('');
         txtItemPartyPurchaseValue.setValue('');

        var Row= flxGRNItemwise.getStore().getCount();
        flxGRNItemwise.getSelectionModel().selectAll();
        var sel=flxGRNItemwise.getSelectionModel().getSelections();
       ...
       for(var i=0;i<Row;i++)

        {
              ...
              {
		      twt =twt+Number(sel[i].data.rect_grnqty);
		      tval=tval+Number(sel[i].data.rect_itemvalue);
              }
         }
         txtItemPartyPurchaseQty.setValue(twt);
         txtItemPartyPurchaseValue.setValue(tval);
...
//    alert(monthenddate);  
	...
	loadItemArrivalsDataStore.removeAll();
	loadItemArrivalsDataStore.load({
	 url: 'ClsViewStatements.php',
                params: {
	    	task: 'loadItemwiseArrivals',
...
//    y:100,
    ...
    [ 	 	
        ...
        {header: "QTY"    , dataIndex: 'grnqty',sortable:false,width:100,align:'right', menuDisabled: true},
        {header: "VALUE" , dataIndex: 'grnvalue',sortable:false,width:120,align:'right', menuDisabled: true},
        {header: "AVG RATE" , dataIndex: 'avgrate',sortable:false,width:90,align:'right', menuDisabled: true},


    ],
    store:loadItemArrivalsDataStore,
    listeners:{	
...
//            lblDetail1.getEl().setStyle('font-weight', 'bold');     
            ...
            lblItem2.setText("Detail for  : " + itemname);
            loadItem_Party_ArrivalsDataStore.removeAll();
	    loadGRNDataStore.removeAll();
	    ...
	    loadPartyGRNDataStore.removeAll();
	    loadPartyGRNDataStore.load({
		url: 'ClsViewStatements.php',
		params: {
	    	task: 'loadAllParty_Item_GRNwise_Arrivals',
		compcode:Gincompcode,
		finid:GinFinid,
                startdate: Ext.util.Format.date(monthstartdate.getValue(),"Y-m-d"), 
                enddate: Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"), 
...
var btnProcess = new Ext.Button({
    ...
    var btnItem_Partywise_Local = new Ext.Button({
        style: 'text-align:center;',
        text: "LOCAL",
        width: 100,
        id: 'btnItem_Partywise_Local',
	border: 1,
	style: {
	borderColor: 'blue',
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p4 ="&grntype="+encodeURIComponent('L');       
    ...
    var btnItem_Partywise_Import = new Ext.Button({
        style: 'text-align:center;',
        text: "IMPORT",
        width: 100,
        id: 'btnItem_Partywise_Import',
	border: 1,
	style: {
	borderColor: 'blue',
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p1 ="&compcode="+encodeURIComponent(Gincompcode);      
	            var p2 = "&fromdate=" + encodeURIComponent(Ext.util.Format.date(monthstartdate.getValue(),"Y-m-  d"));	   
                    var p3 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
		    var p4 ="&grntype="+encodeURIComponent('I');       

 		    var param = (p1+p2+p3+p4) ;
                    if (printtype == "PDF") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/repRMItem_All_PartywiseArrivals.rptdesign&__format=pdf&' + param, '_blank');
                    else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/repRMItem_All_PartywiseArrivals.rptdesign&__format=XLS&' + param, '_blank');
                    else
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/repRMItem_All_PartywiseArrivals.rptdesign' + param, '_blank');	
                }
            }
    ...
    var btnItem_PartywisePrint = new Ext.Button({
        style: 'text-align:center;',
        text: "Item-Parywise",
        width: 100,
        id: 'btnItem_PartywisePrint',
	border: 1,
	style: {
	borderColor: 'blue',
	borderStyle: 'solid',
        ...
        listeners: {
		    ...
		    var p4 ="&grntype="+encodeURIComponent('A');       
    ...
    var btnItemPartyGRNwisePrint = new Ext.Button({
        style: 'text-align:center;',
        text: "GRN wise Print",
        width: 100,
        id: 'btnItemPartyGRNwisePrint',
	border: 1,
	style: {
	borderColor: 'blue',
	borderStyle: 'solid',
   ...
   var txtItemPartyPurchaseQty = new Ext.form.NumberField({
        fieldLabel  : 'Total Qty',
        id          : 'txtItemPartyPurchaseQty',
        name        : 'txtItemPartyPurchaseQty',
        width       :  100,
	readOnly : true,
        labelStyle      : "font-size:14px;font-weight:bold;color:#0080ff",
        tabindex : 2,
   ...
   var txtItemPartyPurchaseValue = new Ext.form.NumberField({
        fieldLabel  : 'Total Value',
        id          : 'txtItemPartyPurchaseValue',
        name        : 'txtItemPartyPurchaseValue',
        width       :  130,
	readOnly : true,
        labelStyle      : "font-size:14px;font-weight:bold;color:#0080ff",
        tabindex : 2,
...
var flxGRNItemwise = new Ext.grid.EditorGridPanel({
    frame: false,
    sm: new Ext.grid.RowSelectionModel(),
    stripeRows : true,
    scrollable: true,
    ...
    [ 	 
        ...
        {header: "Date" , dataIndex: 'rech_date',sortable:false,width:110,align:'left', menuDisabled: true},
        {header: "GRN No" , dataIndex: 'rech_no',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "Bill No" , dataIndex: 'rech_billno',sortable:false,width:110,align:'left', menuDisabled: true},
        {header: "Bill Date" , dataIndex: 'rech_billdate',sortable:false,width:110,align:'left', menuDisabled: true},
        {header: "Supplier"  , dataIndex: 'cust_name',sortable:false,width:220,align:'left', menuDisabled: true},
        {header: "Item Name" , dataIndex: 'itmh_name',sortable:false,width:220,align:'left', menuDisabled: true},
        {header: "Rate/MT" , dataIndex: 'rect_itemrate',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "Qty"   , dataIndex: 'rect_grnqty',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "Value" , dataIndex: 'rect_itemvalue',sortable:false,width:120,align:'left', menuDisabled: true},
    ],
     store:loadPartyGRNDataStore,
    listeners:{	
...
//			layout  : 'hbox',
...

		{
		    ...
		    width       : 120,
                    labelWidth  : 50,
		    items : [btnItem_Partywise_Import]
		},
    ...
    {    
        xtype: 'panel',
        title: 'ITEM PARTYWISE - GRN ARRIVALS',
        bodyStyle: {"background-color": "#ffe6f7"},
        layout: 'absolute',
        items: [
			...
			{ 
		             ...
		             x       : 800,
			     y       : 490,
                             items: [txtItemPartyPurchaseValue]
                        },
...

		{
		    ...
		    width       : 120,
                    labelWidth  : 50,
		    items : [btnItemPartyGRNwisePrint]
		},
