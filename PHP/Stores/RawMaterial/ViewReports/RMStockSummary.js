...
var lblClosing = new Ext.form.Label({
 ...
 var loadItemClosingStockDataStore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        totalProperty: 'total',
        id: 'id' 
      },['itmh_type', 'itmh_name', 'itemcode', 'opstk', 'opval', 'recpt_qty', 'recpt_val', 'iss_qty', 'iss_val', 'sal_qty', 'sal_val', 'ret_qty', 'ret_val']),
    });
 ...
 var loadItem_LedgerDataStore = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        totalProperty: 'total',
        id: 'id' 
      },['trntype', 'docdate', 'docno', 'billno','billdate', 'supname', 'itemname', 'itmh_code', 'qty', 'itemvalue', 'rate' ]),
    });
...
//              {
		      topwt = Number(topwt)+Number(sel[i].data.opqty);
		      topval= Number(topval)+Number(sel[i].data.opvalue);
		      trecptwt =Number(trecptwt)+Number(sel[i].data.recptqty);
		      trecptval=Number(trecptval)+Number(sel[i].data.recptvalue);
		      tissuewt =Number(tissuewt)+Number(sel[i].data.issueqty);
		      tissueval=Number(tissueval)+Number(sel[i].data.issuevalue);
		      tclosewt =Number(tclosewt)+Number(sel[i].data.cloqty);
		      tcloseval=Number(tcloseval)+Number(sel[i].data.clovalue);
...
var qtytype = '0';
var optQtytype = new Ext.form.FieldSet({
    xtype: 'fieldset',
    title: '',
    fieldLabel: '',
    layout : 'hbox',
    ...
    items: [
    {
        ...
        id: 'optQtytype',
        items: [
		{boxLabel: 'GRN Qty', name: 'optQtytype', id:'optGRNQty', inputValue: 1,checked:true,
			listeners:{
		...
		{boxLabel: 'Party Qty', name: 'optQtytype', id:'optPartyQty', inputValue: 2,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
					    qtytype = '1';
...
var optReptype = new Ext.form.FieldSet({
    ...
    items: [
    {
        ...
        id: 'optReptype',
        items: [
//		{boxLabel: 'All', name 'optReptype', id:'optAll', inputValue: 1,checked:true,
		{boxLabel: 'All Items', name: 'optReptype', id:'optAll', inputValue: 1,checked:true,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
					    Reptype = 'ALL';
		...
		{boxLabel: 'Pre-Consumer', name: 'optReptype', id:'optPRE', inputValue: 2,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
					    Reptype = 'PRE';
		...
		{boxLabel: 'Post-Consumer', name: 'optReptype', id:'optPOST', inputValue: 3,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
					    Reptype = 'POST';
...
//    y:100,
    ...
    [ 	 	
        ...
        {header: "Bill No."  ,  dataIndex: 'billno',sortable:false,width:110,align:'center', menuDisabled: true},
        {header: "Bill Date"    , dataIndex: 'billdate',sortable:false,width:110,align:'center', menuDisabled: true},
        {header: "Supplier" , dataIndex: 'supname',sortable:false,width:200,align:'left', menuDisabled: true},
        {header: "Item Name" , dataIndex: 'itemname',sortable:false,width:170,align:'left', menuDisabled: true,hidden:true},
        ...
        {header: "Value " , dataIndex: 'itemvalue',sortable:false,width:105,align:'right', menuDisabled: true},
...
var btnProcess = new Ext.Button({
    ...
    var btnItemwisePrint = new Ext.Button({
        ...
        text: "Stock Abstract - Import/Local ",
        width: 150,
        id: 'btnItemwisePrint',
	border: 1,
	style: {
	...
	borderStyle: 'solid',
		...
		var p5 = "&ptype=" + encodeURIComponent(qtytype);
		var p6 = "&itemtype=" + encodeURIComponent('ALL');
...
//alert(param);
    ...
    var btnItemwisePrint2 = new Ext.Button({
        style: 'text-align:center;',
        text: "Stock Abstract - Pre/Post Consumer ",
        width: 150,
        id: 'btnItemwisePrint2',
	border: 1,
	style: {
	borderColor: 'blue',
	borderStyle: 'solid',
		...
		var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
		var p5 = "&ptype=" + encodeURIComponent(qtytype);
		var p6 = "&itemtype=" + encodeURIComponent(Reptype);
                var param = (p1+p2+p3+p4+p5+p6);
//alert(param);
                if (printtype == "PDF") 
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs2.rptdesign&__format=pdf&' + param, '_blank'); 

                else if (printtype == "XLS") 
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs2.rptdesign&__format=XLS&' + param, '_blank'); 

                else  
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs2.rptdesign&' + param, '_blank'); 
            }
        }
    })
    var btnStockAbstract = new Ext.Button({
        style: 'text-align:center;',
        text: "Stock Abstract Format ",
        width: 100,
        id: 'btnStockAbstract',
	border: 1,
	style: {
...
//alert(param);
                if (printtype == "PDF") 
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs_format.rptdesign&__format=pdf&' + param, '_blank'); 

                else if (printtype == "XLS") 
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs_format.rptdesign&__format=XLS&' + param, '_blank'); 

                else  
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs_format.rptdesign&' + param, '_blank'); 
            }
        }
    });
   ...
   var btnItemLedger = new Ext.Button({
	...
	borderStyle: 'solid',
		...
		var p7 = "&qtytype=" + encodeURIComponent(qtytype);
