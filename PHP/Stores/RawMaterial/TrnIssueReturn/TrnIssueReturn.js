...
var Issdetaildatastore = new Ext.data.Store({
  ...
  reader: new Ext.data.JsonReader({
    ...
    'isst_hdseqno', 'isst_seqno', 'isst_itmcode',  'isst_qty', 'isst_rate',  'isst_values', 'issh_type', 'issh_date', 'isrh_seqno', 'itmh_name', 'itmt_clqty','isst_varty'
  ])
});
...
var btnSubmit = new Ext.Button({
 ...
 border: 1,
	...
	{
		Ext.MessageBox.alert("Issue Return", "Select Variety");
                gstadd="false";
    ...
    listeners:{	
		...
		callback:function()
		{
			...
			if (Number(loadItemStockDataStore.getAt(0).get('stock_bags')) < 3 ){
				...
				actstk = (loadItemStockDataStore.getAt(0).get('stock'));
			}
			else { 
				txtstock.setValue(Ext.util.Format.number(loadItemStockDataStore.getAt(0).get('stock'),"0.000"));
				actstk = (loadItemStockDataStore.getAt(0).get('stk_billqty'));
			}
			var issval = Number(txtretqty.getValue())*Number(txtcostrate.getValue());
			txtretval.setValue(issval);			
				...
				else
				{
					...
					if (btn == 'yes')
                		       	        ...
                		       	        params:
						{
							...
							retval : txtvalue.getValue(),
							usrcode : userid,
							AEDFlag : SaveFlag,
							isspno  : txtissretno.getRawValue(),
							seqnoed : seqnoed
						},
                                            	...
                                            	if (obj['success'] === "true") 
						{
                                                Ext.MessageBox.show({
                                                    ...
                                                    msg: 'Issue No : ' + obj['issretno'] + '  Saved',
                                                    fn: function (btn) {
						    if (btn === 'ok') 
							{
