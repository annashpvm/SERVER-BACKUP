<?php
 ...
 function getsupplier()
	...
	$sql = "select cust_ref ,cust_code from trnrm_order_header , massal_customer where ordh_sup_code = cust_code group by cust_ref ,cust_codeorder by cust_ref ,cust_code");
 ...
 function getthird()
    {
        mysqli_set_charset($conn, "utf8");
	//$supplier_id = $_POST['supplierid'];
	$sql = "select cust_code,cust_ref from massal_customer where cust_acc_group = 143 order by cust_ref");
 ...
 function getgrndetail()
	...
	$grnno   = $_POST['grnno'];
//	$sql = " select * from trnrm_receipt_header a ,massal_customer s , mas_RMFU_purchasetax p where rech_seqno= $grnno and  s.cust_code=a.rech_sup_code  and rech_purledger = tax_purcode");
	$sql = " select * from trnrm_receipt_header a ,massal_customer s , mas_RMFU_purchasetax p  , acc_trail t 
...
where b.rect_item_code = c.itmh_code and a.rech_seqno = b.rect_hdseqno and
 ...
 function getgrnno()
	...
	{
		$sql = "call sprm_sel_receiptnos ('$compcode','$finid','L')");
	}
...
function getuserdet()
{
 ...
 function getQCEntryList()
	...
	$supcode = $_POST['supcode'];
        ...
        $sql = "select qc_rm_entryno from trn_qc_rm_inspection where qc_rm_supcode = $supcode and qc_rm_grn_status = 'N' and qc_rm_compcode = '$compcode' and qc_rm_fincode = $finid group by qc_rm_entryno order by qc_rm_entryno desc");
...
/*
        if ($ledname == '')
	        ...
	        $qry = "select * from ( select  cust_code,cust_name,qc_rm_supcode,cust_taxtag,cust_wp_gstinv_supplier_yn from trn_qc_rm_inspection,massal_customer  where   cust_acc_group in (58, 60) and  cust_code = qc_rm_supcode group by cust_code,cust_name, qc_rm_supcode, cust_taxtag,cust_wp_gstinv_supplier_yn ) a1 where replace(replace(cust_name,' ','')  ,'.','')  like '%$ledname%'  order by cust_name";

*/

        if ($ledname == '')
	        $qry = "select * from (select  cust_code,cust_name,qc_rm_supcode,cust_state,sup_wp_gstinv_supplier_yn from trn_qc_rm_inspection,massal_customer  where    cust_code = qc_rm_supcode group by cust_code,cust_name,qc_rm_supcode,cust_state,sup_wp_gstinv_supplier_yn) a1 order by cust_name";
        else

	        $qry = "select * from ( select  cust_code,cust_name,qc_rm_supcode,cust_state,cust_wp_gstinv_supplier_yn from trn_qc_rm_inspection,massal_customer  where   cust_code = qc_rm_supcode group by cust_code,cust_name, qc_rm_supcode, cust_state,cust_wp_gstinv_supplier_yn ) a1 where replace(replace(cust_name,' ','')  ,'.','')  like '%$ledname%'  order by cust_name";
