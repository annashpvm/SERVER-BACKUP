...
var lblDebitNote = new Ext.form.Label({
   ...
   {
      if (txtPasswordRate.getRawValue() == "rate1234")
      {
         chkratediff = 1;
   ...
   function check_password_QCRate()
   {
      if (txtPasswordQCRate.getRawValue() == "QC1234")
      {
         chkqcratediff = 1;
         Ext.getCmp('save').setDisabled(false);
      }
...
//            obj.setRawValue(newValue.toUpperCase());
   ...
   var txtPasswordQCRate = new Ext.form.TextField({
        fieldLabel  : 'PassWord',
        id          : 'txtPasswordQCRate',
        name        : 'txtPasswordQCRate',
        inputType   : 'password',
        fieldStyle  : 'text-transform:uppercase',
        width       :  80,
 //	readOnly    : true,
...
//        qctax = parseFloat(qctaxamt.toFixed(0));

        qctax   = Number(txtTotalDiffValue.getValue()).toFixed(0);
    ...
    if(Ext.util.Format.date(dtpGRNDate.getValue(),"Y-m-d") > Ext.util.Format.date(todate,"Y-m-d")){
        ...
        gstSave="false";
            ...
            else if ( Number(difftax) > 0 && (Number(qctax)  !=  Number(difftax)) && chkqcratediff == 0 )
...
//                Ext.Msg.alert('GRN','Taxable Value is different between GRN Different Taxable and QC Taxable. Please Check QC entry ');
		  ...
		  msg: 'Taxable Value is different between GRN Taxable and QC Taxable.  CONFIRM - YES to Proceed -  NO to EXIT',
		  fn: function(btn){
		  if (btn === 'yes')
		  {
                      Ext.getCmp('save').setDisabled(true);
                      txtPasswordQCRate.show();
		  } 
		  }   
		  }); 
                  gstSave="false";
...
//            alert(dndateValue);
		...
		status          : '',
		usrcode         : GinUserid,
		entrydate       : Ext.util.Format.date(new Date(), "Y-m-d"),
		...
		gatedate        : Ext.util.Format.date(dtpgatedate.getValue(),"Y-m-d"),
                ...
                shortageqty     : txtWTShortageQty.getRawValue(),
                shortagevalue   : txtWTShortageValue.getRawValue(),
                ratediffvalue   : Number(txtRateDiffValue.getValue())  ,
                valuedifferance : Number(txtTotalItemDiffValue.getValue())  ,
		},
...
'areagrp_code', 'areagrp_name','tax_purcode','tax_purname','tax_cgstper', 'tax_sgstper', 'tax_igstper', 'tax_cgstledcode', 'tax_sgstledcode', 'tax_igstledcode', 'tax_cgstledger', 'tax_sgstledger', 'tax_igstledger', 'tax_gst', 'tax_purtype', 'tax_state','qc_rm_millboard','qc_rm_taxable','qc_rm_act_moisure_qty','qc_rm_mois_tolarance','qc_rm_shortage'
      ]),
...
'rech_dn_other_charges','rech_dn_other_ledger','rech_dnno','rech_dnseqno', 'rech_dnaccseqno','rech_grn_status', 'cust_wp_gst_dnote_yn','rech_dndate'
...
'rect_act_mois','rect_mois_tolarance','rect_short_qty'
 ]),
    });
...
function parseDateDMY(dateStr) {
    ...
    if (parts.length !== 3) {
        console.warn("Date format incorrect (expected dd/MM/yyyy or dd-MM-yyyy):", dateStr);
        return null;
    }
    ...
    if (isNaN(day) || isNaN(month) || isNaN(year)) {
        console.warn("Failed to parse day/month/year:", parts);
        return null;
    }
...
function stripTime(date) {
    if (!(date instanceof Date)) {
        console.warn("Invalid date object passed to stripTime:", date);
        return null;
    }
...
var ratediff = 0;
        ...
        {
	    var rec     = sel[i],
		billwt  = Number(rec.get('billwt'));
		billrate= Number(rec.get('billrate'));
		itemval = Number(rec.get('itemvalue'));


                ratediff = 0;
		
		if (Number(sel[i].data.billrate) > Number(sel[i].data.itemrate) && Number(sel[i].data.billwt) > 0)
		{
		       ratediff =  (Number(sel[i].data.billrate) - Number(sel[i].data.itemrate)) * Number(sel[i].data.billwt);
		       var ratediff2 = ratediff.toFixed(2);  
		       sel[i].set('ratediffvalue', ratediff2);
		} 
		else
		{
		       sel[i].set('ratediffvalue',0);
		 } 


		billval = billwt * billrate;
 		diff    = (billval - itemval - ratediff);
                var diffStr = Ext.util.Format.number(diff, '0.00');
		billStr = Ext.util.Format.number(billval, '0.00');





	    rec.set('billvalue',    Ext.util.Format.number(billval, '0.00'));
	    rec.set('itemdiffvalue',diff);
...
/*          
		var billwt = Number(sel[i].data.billwt);
		var billrate = Number(sel[i].data.billrate);
		var billvalue = billwt * billrate;

		sel[i].set('billvalue', Ext.util.Format.number(billvalue,'0.00')); 

 
         var valuex = Number(sel[i].data.billvalue);
         var valuey = Number(sel[i].data.itemvalue);
   

          value3 = 0;
          if ( Number(sel[i].data.itemrate) == Number(sel[i].data.billrate) && 
                  Number(sel[i].data.ticketwt) == Number(sel[i].data.grnqty))
          { 
             value3 = 0;
          }               
          else
    ...
    //      value3 = Number(value1) -  Number(value2) + Number(ratediff);

          value3 = Number(billvalue) -  Number(valuey) ;


          }
          value3 =  Ext.util.Format.number(value3,'0.00');
          sel[i].set('itemdiffvalue', value3);


console.log( " lvalue:", value3 );
console.log("Row " + i + " diffvalue:", sel[i].get.ItemDiffValue);

*/

//          if (Number(sel[i].data.qctaxable) > 0) 
//                 value3 = 0;             
          
//          if (Number(sel[i].data.qctaxable) > 0) 
//             value4 = Number(Number(sel[i].data.qctaxable)) +  Number(value3);
//          else



   //   value3 = Number(valuex) -  Number(valuey) ;
...
/*
                ...
                {
                   brate = Number(sel[i].data.billrate);
                   grate = Number(sel[i].data.itemrate);
                 }    
*/
                if ( Number(sel[i].data.billwt) > 0)
                {
                   brate = Number(sel[i].data.billrate);
                   grate = Number(sel[i].data.itemrate);
                 }    
                ...
                tickwt = Number(tickwt) + Number(sel[i].data.ticketwt); 
                degqty = Number(degqty) + Number(sel[i].data.degradeqty); 
                grnwt = Number(grnwt) + Number(sel[i].data.grnqty); 
                qcshortqty = Number(qcshortqty) + Number(sel[i].data.shortqty); 
                ItemDiffValue = Number(ItemDiffValue) + Number(sel[i].data.itemdiffvalue); 
 
                rateValueDiff  = Number(rateValueDiff) + Number(sel[i].data.ratediffvalue); 


                if (Number(sel[i].data.billvalue) > 0) 
                { 
	...
	txtQCShortQty.setRawValue(Ext.util.Format.number(Number(qcshortqty)/1000,"0.000"));




var grnDateStr = dtpGRNDate.getRawValue(); // "15/06/2025"
//console.log("GRN DATE:", grnDateStr);
var parsedDate = parseDateDMY(grnDateStr);
var grnDate     = stripTime(parsedDate);
var cutoffDate  = new Date(2025, 5, 17); 
...
if ( grnDate && grnDate  >  cutoffDate) {


        if (Number(partybillqty) >  (Number(tickwt) - Number(degqty)) - Number(txtQCShortQty.getValue()) )
        { 




            shortqty = Number(partybillqty) -  (Number(tickwt) - Number(degqty)) - Number(txtQCShortQty.getRawValue()) ;
            if (Number(shortqty) < 0)  
               shortqty = 0;
  
            shortvalue = Number(shortqty) * Number(grate);
...
/*
        ...
        if (Number(brate) >  Number(grate))
        { 

             Ratediff = Number(brate) -  Number(grate);
             Diffvalue = Number(partybillqty) * Number(Ratediff);
        }
...
//alert("less");
        ...
        ItemDiffValue = Math.abs(ItemDiffValue) - Number(shortvalue) - Number(qctaxamt);

        if (Number(ItemDiffValue) < 0)
           ItemDiffValue = 0;    


        txtTotalItemDiffValue.setRawValue(Ext.util.Format.number(Number(ItemDiffValue),"0.00")); 


// New Changes on 20/06/2025 - Start




 //           qctaxamt = Number(qctaxamt) + Number(shortvalue)+ Number(Diffvalue) + Number(ItemDiffValue);
    


//            qctaxamt =   Number(shortvalue) + Number(rateValueDiff)+ Number(ItemDiffValue);


// New Changes on 20/06/2025 - End

	txtTotalDNTaxValue.setRawValue(Ext.util.Format.number(Number(qctaxamt),"0.00"));

	txtWTShortageQty.setRawValue(Ext.util.Format.number(Number(shortqty),"0.000"));
	txtWTShortageValue.setRawValue(Ext.util.Format.number(Number(shortvalue),"0.00"));
        txtRateDiffValue.setRawValue(Ext.util.Format.number(Number(rateValueDiff),"0.00"));

        var qctaxamt2 = Number(txtWTShortageValue.getValue()) + Number(txtRateDiffValue.getValue()) + Number(txtTotalItemDiffValue.getValue()) + Number(txtTotalDNTaxValue.getValue()) ;

        qctaxamt2 = parseFloat(qctaxamt2.toFixed(2));



 
        txtTotalDiffValue.setRawValue(Ext.util.Format.number(Number(qctaxamt2),"0.00")); 
...
chkratediff = 0
    ...
    //                    cmbPurchaseLedger.setValue(loadgrndetaildatastore.getAt(0).get('rech_purledger'));
			...
			dtpGRNDate.setRawValue(Ext.util.Format.date(loadgrndetaildatastore.getAt(0).get('rech_date'),'d-m-Y'));
                        ...
                        DateOfGrn = loadgrndetaildatastore.getAt(0).get('rech_date');
			...
			txtNewBillNo.setValue(loadgrndetaildatastore.getAt(0).get('rech_billno'));
...
//						tarqty: loadgrnitemdetaildatastore.getAt(i).get('rect_tareqty'),
						...
						shortqty  : loadgrnitemdetaildatastore.getAt(i).get('rect_short_qty'),
...
//						lotno: loadgrnitemdetaildatastore.getAt(i).get('lot_refno'),
						...
						actmois : loadgrnitemdetaildatastore.getAt(i).get('rect_act_mois'),
                                                moistol : loadgrnitemdetaildatastore.getAt(i).get('rect_mois_tolarance'),
                    		        shortagevalue  : 0,
                    		        ratediffvalue  : 0,
		...
		callback:function(){
                       ...
                       chkratediff = 0;
                        if (loadQCEntryNoDetailDatastore.getAt(0).get('qc_rm_billdate') != null)
                           ...
                           dtpGRNDate.setRawValue(Ext.util.Format.date(loadQCEntryNoDetailDatastore.getAt(0).get('qc_rm_ticketdate'),'d-m-Y'));

                        DateOfGrn = loadQCEntryNoDetailDatastore.getAt(0).get('qc_rm_ticketdate');
...
var ival = Number(grnwt) *  loadQCEntryNoDetailDatastore.getAt(i).get('qc_rm_rate');
				...
				new dgrecord({
					...
					remarks     :  loadQCEntryNoDetailDatastore.getAt(i).get('qc_rm_remarks'),
                                        ...
                                        itemvalue   :  Ext.util.Format.number(ival,'0.00') ,
                                        costrate    :  0,
                                        costvalue   :  0,
                    		        ...
                    		        actmois     :  loadQCEntryNoDetailDatastore.getAt(i).get('qc_rm_act_moisure_qty'),
                    		        moistol     :  loadQCEntryNoDetailDatastore.getAt(i).get('qc_rm_mois_tolarance'),
                    		        shortqty    :  loadQCEntryNoDetailDatastore.getAt(i).get('qc_rm_shortage'),
                    		        shortagevalue  : 0,
                    		        ratediffvalue  : 0,
...
var txtWTShortageQty = new Ext.form.NumberField({
        fieldLabel  : 'WT Short.',
        id          : 'txtWTShortageQty',
        name        : 'txtWTShortageQty',
        width       :  60,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
       // style       :  {textTransform: "uppercase"},
        allowBlank  :  false,
...
var txtWTShortageValue = new Ext.form.NumberField({
        fieldLabel  : 'Value',
        id          : 'txtWTShortageValue',
        name        : 'txtWTShortageValue',
        width       :  70,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
       // style       :  {textTransform: "uppercase"},
        allowBlank  :  false,
...
var txtRateDiffValue = new Ext.form.NumberField({
        fieldLabel  : 'Rate Diff',
        id          : 'txtRateDiffValue',
        name        : 'txtRateDiffValue',
        width       :  70,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
       // style       :  {textTransform: "uppercase"},
        allowBlank  :  false,
...
var txtQCShortQty = new Ext.form.NumberField({
        fieldLabel  : 'Short Qty',
        id          : 'txtQCShortQty',
        name        : 'txtQCShortQty',
        width       :  50,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
...
var txtTotalDNTaxValue = new Ext.form.NumberField({
        fieldLabel  : 'DN TAX Value',
        id          : 'txtTotalDNTaxValue',
        name        : 'txtTotalDNTaxValue',
        width       :  70,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
       // style       :  {textTransform: "uppercase"},
        allowBlank  :  false,
...
var txtTotalItemDiffValue = new Ext.form.NumberField({
        fieldLabel  : 'Diff Value',
        id          : 'txtTotalItemDiffValue',
        name        : 'txtTotalItemDiffValue',
        width       :  70,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
       // style       :  {textTransform: "uppercase"},
        allowBlank  :  false,
...
var txtTotalDiffValue = new Ext.form.NumberField({
        fieldLabel  : 'Total Diff Value',
        id          : 'txtTotalDiffValue',
        name        : 'txtTotalDiffValue',
        width       :  80,
labelStyle : "font-size:14px;font-weight:bold;color:#0080ff",
       // style       :  {textTransform: "uppercase"},
        allowBlank  :  false,
...
//Party Account - Credit
       ...
       var dnledcode = '4993';
       var dnledname = 'CREDIT NOTE TO BE RECEIVED';
     
       if (supwp_GST_DN_Exemption== "Y")
       {
            dnledcode = '2652';
            dnledname = 'REBATE AND DISCOUNT RECEIVED';
       }
...
function diffRenderer(value, meta, record) {
  // calculate
  var bill = record.get('billrate') *  record.get('billwt'); 
  var diff = bill - record.get('itemvalue');
  // format two decimals + commas
  return Ext.util.Format.number(diff, '00.00');
}
...
*/
        ...
        {header: "Bill Rate", dataIndex: 'billrate',sortable:true,width:80,align:'right',
        editor: {
            xtype: 'numberfield',
            allowBlank: true,
   //         id : 'tickwt',        
...
//		            grid_tot();
   ...
   //                  grid_tot();
        ...
        {header: "Bill Value", dataIndex: 'billvalue',sortable:true,width:80,align:'right',
/*
                renderer: function(v, params, record){
                    var retval;

                    retval=Number(record.data.billrate) * Number(record.data.billwt) ;
                    return retval;
                },
*/
        },
        ...
        {header: "Act Mois", dataIndex: 'actmois',sortable:true,width:120,align:'left' },
        {header: "Mois Tol", dataIndex: 'moistol',sortable:true,width:100,align:'left'},

        {header: "QC DN taxable", dataIndex: 'qctaxable',sortable:true,width:110,align:'left'},
        {header: "Short Qty", dataIndex: 'shortqty',sortable:true,width:80,align:'left'},
      //  {header: "Short Value", dataIndex: 'shortagevalue',sortable:true,width:80,align:'left'},
        {header: "Rate Diff", dataIndex: 'ratediffvalue',sortable:true,width:80,align:'left'},

        {header: "Diff Value", dataIndex: 'itemdiffvalue' , type: 'float',sortable:true,width:80,align:'left',
