...
function getitemqty()
{
	...
	$pono = $_POST['pono'];
	$status = $_POST['status'];	
	$sql = "call sprm_sel_orditem_dets('$pono','$item','$status')";
...
/*
        if ($ledname == '')
	        $sql = "select * from (select  cust_code,cust_name,qc_rm_supcode,cust_taxtag,sup_wp_gstinv_supplier_yn from trn_qc_rm_inspection,massal_customer  where  cust_acc_group in (58, 60) and  cust_code = qc_rm_supcode group by cust_code,cust_name,qc_rm_supcode,cust_taxtag,sup_wp_gstinv_supplier_yn) a1 order by cust_name";
        else
...
*/
        ...
        if ($ledname == '')
	        $sql = "select * from (select  cust_code,cust_name,qc_rm_supcode,cust_state,cust_wp_gst_dnote_yn,
cust_cr_days,cust_grace_days from trn_qc_rm_inspection,massal_customer  where    cust_code = qc_rm_supcode group by cust_code,cust_name,qc_rm_supcode,cust_state,cust_wp_gst_dnote_yn ,cust_cr_days,cust_grace_days ) a1 order by cust_name";
	        ...
	        $sql = "select * from ( select  cust_code,cust_name,qc_rm_supcode,cust_state,cust_wp_gst_dnote_yn,cust_cr_days,cust_grace_days from trn_qc_rm_inspection,massal_customer  where   cust_code = qc_rm_supcode group by cust_code,cust_name, qc_rm_supcode, cust_state,cust_wp_gst_dnote_yn,cust_cr_days,cust_grace_days  ) a1 where replace(replace(cust_name,' ','')  ,'.','')  like '%$ledname%'  order by cust_name";
