...
$valuedifferance = (float)$_REQUEST['valuedifferance'];
...
$dncgstper    = (float)$_REQUEST['sgstper'];
$dnsgstper    = (float)$_REQUEST['cgstper'];
$dnigstper    = (float)$_REQUEST['igstper'];
...
//echo "<br>";
      ...
      $resultStock1=mysqli_query($conn, $queryStock1);
     ...
     $resultStock2=mysqli_query($conn, $queryStock2);
...
$cresulta4 = mysqli_query($conn, $cquerya4);
...
if ($dntype == "DNG")
	$DNquery8 = "call acc_sp_insdbcrnotetrailer('$gindbcrseq','$billno','$billdate','$dntaxable' ,'$debitnoteamount','$dnigst', '$dncgst','$dnsgst','$dnigstper','$dncgstper','$dnsgstper','$dnigstledcode','$dncgstledcode','$dnsgstledcode',$dntcs, $tcsper,$tcsledger,$dnother ,$dnotherledcode,0,'0',0,'$dntaxable')";
else
	$DNquery8 = "call acc_sp_insdbcrnotetrailer('$gindbcrseq','$billno','$billdate','$debitnoteamount' ,'$debitnoteamount','0', '0','0','$dnigstper','$dncgstper','$dnsgstper','$dnigstledcode','$dncgstledcode','$dnsgstledcode',$dntcs, $tcsper,$tcsledger,$dnother ,$dnotherledcode,0,'0',0,'$debitnoteamount')";
...
if (trim($DNdate) == '') {
    $DNdate = NULL;
	$DNdate = var_dump($DNdate);  // Should show: string(0) ""
}
// Should show: string(0) ""

echo  $DNdate;
 echo "<br>";
 */

 $DNdate_sql    = ($DNdate === null || $DNdate === '') ? "NULL" : "'$DNdate'";


// Now $date is NULL if nothing was selected



 $queryQC1 = "update trn_qc_rm_inspection set  qc_rm_grn_status = 'Y',qc_rm_grnno = '$rech_no',qc_rm_cgst_per =$cgstper, qc_rm_sgst_per = $sgstper, qc_rm_igst_per = $igstper, qc_rm_cgst_value = $dncgst, qc_rm_sgst_value = $dnsgst,   qc_rm_igst_value = $dnigst,  qc_rm_debitamount =$debitnoteamount, qc_rm_pur_ledger = $purledger , qc_rm_debitnote_no = '$vouno',qc_rm_debitnote_date = $DNdate_sql ,qc_rm_dn_raised  = 'Y' , qc_rm_billno = '$billno' , qc_rm_billdate = '$billdate' , qc_rm_billvalue = $totamt ,  qc_rm_tcs_per = $tcsper, qc_rm_tcs_amount = $dntcs  where qc_rm_compcode = '$compcode' and qc_rm_fincode = '$finid' and  qc_rm_entryno = '$qcinsno'";

//echo $queryQC1;
//echo "<br>";

$resultQC1 = mysqli_query($conn, $queryQC1);



 $queryQC5 = "update trn_qc_rm_inspection set qc_rm_ratediff =  $ratediffvalue,  qc_rm_valuediff =  $shortagevalue + $valuedifferance ,qc_rm_shortage = qc_rm_shortage + $shortageqty where qc_rm_compcode = '$compcode' and qc_rm_fincode = '$finid' and  qc_rm_entryno = '$qcinsno' and qc_rm_slno = 1 ";

//echo $queryQC1;
//echo "<br>";

$resultQC5 = mysqli_query($conn, $queryQC5);
...
if ($dntaxable > 0)
{
 $queryQC3 = "select $dntaxable-sum(qc_rm_taxable)-sum(qc_rm_ratediff) -sum(qc_rm_valuediff)   difftaxable from trn_qc_rm_inspection where qc_rm_compcode = '$compcode' and qc_rm_fincode = '$finid' and qc_rm_entryno = '$qcinsno'";

//echo $queryQC3;
//echo "<br>";

	$resultQC3 = mysqli_query($conn, $queryQC3);
	$rec4      = mysqli_fetch_array($resultQC3);
...
//echo "<br>";
 ...
 $queryQC4 = "update trn_qc_rm_inspection set qc_rm_other_deductions = '$valuediff'  where qc_rm_compcode = '$compcode' and qc_rm_fincode = '$finid' and qc_rm_entryno = '$qcinsno' ";
	$resultQC4 = mysqli_query($conn, $queryQC4);
