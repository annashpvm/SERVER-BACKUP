Ext.onReady(function(){
 ...
 var loadpreprint = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
                url: '/DPM/Import/ClsImportRep.php',      // File to connect to
                method: 'POST'
            }),
 ...
 var cmbReport = new Ext.form.ComboBox({
	...
	emptyText       : '--Select Report Type--',
        mode            : 'local',
        store           : [['1','GRN CUM INVOICE - IMPORT'],['2','PURCHASE ORDER - IMPORT'],['3','PO - ANNEXURE']],
        forceSelection  : true,
        triggerAction   : 'all',
        selectOnFocus   : false, 
        editable        : false,
	...
	listeners:{
			...
			if (cmbReport.getValue() == "1") {
				...
				loadpreprint.load({
					url:'/DPM/Import/ClsImportRep.php',
					params:
					{
					task:"loadrepno",
					finid : GinFinid,
			...
			else if (cmbReport.getValue() == "2" || cmbReport.getValue() == "3" ) {
            ...
            items: [
				...
				else
				{
					...
					if(cmbReport.getValue() == "1"){
						...
						var p4 = "&pono=" + encodeURIComponent(cmbstrepno.getValue());
						var param = (p1+p2+p4) ;                         
						window.open('http://192.168.11.14:8080/birt/frameset?__report=Import/RepRMInvoice.rptdesign' + param, '_blank'); 
					}
						...
						var p2 = "&finid=" + encodeURIComponent(GinFinid);
						var p3 = "&ordfrom=" + encodeURIComponent(d2);
						var p4 = "&ordhno=" + encodeURIComponent(pono);
						var param = (p1+p2+p3+p4) ;                         
						window.open('http://192.168.11.14:8080/birt/frameset?__report=Import/RepIMRMPurchaseOrder.rptdesign' + param); 
					}
						...
						var p1 = "&compcode=" + encodeURIComponent(GinCompcode);
						var p2 = "&finid=" + encodeURIComponent(GinFinid);
						var p3 = "&ordhfrom=" + encodeURIComponent(d2);
						var p4 = "&ordhno=" + encodeURIComponent(pono);
						var param = (p1+p2+p3+p4) ; 	
					window.open('http://192.168.11.14:8080/birt/frameset?__report=Import/RepIMRMAnnexure.rptdesign' + param); 										
					}
...
ReppreprintWindow.hide();
	...
	x	    : 200,
        y           : 35,
        title       : 'Raw Material PrePrinted Reports',
