Ext.onReady(function(){
 ...
 var loadgeneral = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
                url: '/DPM/RawMaterial/ClsRMRep.php',      // File to connect to
                method: 'POST'
            }),
            ...
            items: [
				...
				else
				{
					...
					var param =(compcode+fromdate+todate);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMDailyArrivals.rptdesign' + param, '_blank'); 
				    }
				    if (Rpttype == "recP") {
					var param =(compcode+finid+fromdate+todate);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMPartywiseReceipt.rptdesign' + param, '_blank'); 
				    }
					
				    if (Rpttype == "stkqty") {
					var param =(compcode+compcode1+finid+fromdate+todate+ptype);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMQtyStockAbs.rptdesign' + param, '_blank'); 
				    }
				    if (Rpttype == "stkqtyval") {
					var param =(compcode+compcode1+finid+fromdate+todate+ptype);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMQtyValStockAbs.rptdesign' + param, '_blank'); 
				    }
				    if (Rpttype == "issReg") {
					var param =(compcode+finid+fromdate+todate);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMIssueRegister.rptdesign' + param, '_blank'); 
				    }	
				    if (Rpttype == "issAbs") {
					var param =(compcode+finid+fromdate+todate);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMIssueAbstract.rptdesign' + param, '_blank'); 
				    }		
				    if (Rpttype == "optItemPartywiseDegrade") {
					var param =(compcode+finid+fromdate+todate);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMDegradeItemwise.rptdesign' + param, '_blank'); 
				    }	
				    if (Rpttype == "recptareaitem1") {
					var param =(compcode+finid+fromdate+todate);
				    	window.open('http://192.168.11.14:8080/birt/frameset?__report=Rawmaterial/RepRMAreaItemwise_Arrivals_GRNAMT.rptdesign' + param, '_blank'); 
				    }	
