...
var newcode = 0;
    ...
    var Addrchange = 'N';
    var chkAddrChange = new Ext.form.Checkbox({
        name: 'chkAddrChange',
        //boxLabel: 'Do you Want to Change the Address',
        boxLabel: '<span style="font-size:18px; font-weight:bold; color:#0080ff;">Do you Want to Change the Address</span>',
        id: 'chkAddrChange',
    	labelStyle : "font-size:18px;font-weight:bold;color:#0080ff",

        checked: false,
        width: 400,
        listeners: {
            check: function (rb, checked) {
		Addrchange = checked ? 'Y' : 'N';
                if ( Addrchange == "Y")
...
var btnGST = new Ext.Button({
    ...
    listeners:{
       click: function(){
          Ext.getCmp('cmbGST').setDisabled(false);  
       }
   }
}); 
...
//      autoLoad : true,      
      ...
      reader: new Ext.data.JsonReader({
        ...
        id: 'id'
            ...
            'cust_code',  'cust_ref', 'cust_name', 'cust_add1', 'cust_add2', 'cust_add3', 'cust_city', 'cust_state', 'cust_country', 'cust_zip', 'cust_phone', 'cust_email', 'cust_web', 'cust_contact', 'cust_taxtag', 'cust_cr_days',  'cust_grace_days',  'cust_repr', 'cust_panno','cust_gstin','cust_dealer','cust_smsno',
'cust_group','cust_partygroup','cust_distance','area_name','area_code',
...
/*
                 ...
                 {    
		        flxSalesCustDetail.getStore().removeAll();
                 } 
*/
             ...
             flxSalesCustDetail.getSelectionModel().selectRow(0)
             flxSalesCustDetail.focus;
             flxSalesCustDetail.getView().focusRow(0);
             }
             if (e.getKey() == e.UP)
             {
	    ...
	    keyup: function () {
                ...
                if  (newcode == 0)
                {
                flxSalesCustDetail.getEl().setStyle('z-index','10000');
                flxSalesCustDetail.show();
                loadsalcustlistdatastore.removeAll();
	...
	tabindex : 1,
        ...
        listeners: {
            change: function(field, newValue) {
                if (!field.isValid()) {
                    console.log('Invalid Email:', newValue);
                }
            }
        }
});
...
function cust_flx_grid_click()
{
   ...
   var flxSalesCustDetail = new Ext.grid.EditorGridPanel({
        ...
        columns: [    
            ...
            {header: "Ref Name",  Id: 'cust_ref', sortable:true,width:300,align:'left'}, 
...
store:loadsalcustlistdatastore,
    listeners:{	
             ...
             'cellDblclick' : function(flxSalesCustDetail, rowIndex, cellIndex, e){
                     cust_flx_grid_click();
             }
   }
	...
	bodyStyle   :{"background-color":"#F1F5EA"},
		...
		//disabled : true,
		    ...
		    listeners:{
		        click: function () {
                                saveflag = "Edit";
                                Ext.getCmp('chkAddrChange').setDisabled(false);
                                Ext.getCmp('Save').setDisabled(true);
		//		RefreshData();
...
//save
		   ...
		   if (!txtsalcustemail.isValid()) {
		          alert('Invalid Email');
		          gstSave="false";
		   }

                   else if (txtsalrefname.getRawValue()==0 || txtsalrefname.getRawValue()=="")
                    {
