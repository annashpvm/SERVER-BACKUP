            ...
            items: [
                {
                    ...
                    listeners: {
                        click: function () {
                            ...
                            var vu = "&voushort=" + encodeURIComponent(v);

			    if(ledcod>0){
				ledcod=cmbledger.getRawValue();
			    }else{
				ledcod='%';
			    }	

                            var ledseq = "&ledcode=" + encodeURIComponent(ledcod);

                            var param = (cp + asdate + vu + ledseq);	

			    if(output==="N"){	
                            window.open('http://denimsoft.kgdenim.com:8080/birt-viewer/frameset?__report=accounts/PayAge.rptdesign' + param, '_blank');	
		 	   }else if(output==="Y" && cmbledger.getValue()>0){	
                            window.open('http://denimsoft.kgdenim.com:8080/birt-viewer/frameset?__report=accounts/PayAgeledger.rptdesign' + param, '_blank');	
		 	   }else if(output==="Y" && cmbledger.getValue()<=0){	
                            window.open('http://denimsoft.kgdenim.com:8080/birt-viewer/frameset?__report=accounts/PayAgeledger.rptdesign' + param, '_blank');	
		 	   }		 
