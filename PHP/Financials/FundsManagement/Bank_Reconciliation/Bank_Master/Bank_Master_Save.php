...
if ($action == "Add") {
    $query1 = "call acc_sp_bankrecon_bankmaster_insert('$bankcode','$bank_name','$branch_name','$acc_no','$as_on_month','$as_on_year','$bank_bal','$kgdl_bal','$cclimit','$active_status','$userid');";
    $result1 = mysqli_query($conn, $query1);

    for ($i = 0; $i < $comprowcnt; $i++) {

        $queryseq = "select ifnull(max(seqno),0)+1  as seq from bank_master_trailerone";
        $ress = mysqli_query($conn, $queryseq);
        $rec2 = mysqli_fetch_array($ress);
        $seqno2 = $rec2['seq'];
    ...
    $query1 = "call acc_sp_bankrecon_bankmaster_update('$hseqno','$bank_name','$branch_name','$acc_no','$as_on_month','$as_on_year','$bank_bal','$kgdl_bal','$cclimit','$active_status','$userid');";
    $result1 = mysqli_query($conn, $query1);
