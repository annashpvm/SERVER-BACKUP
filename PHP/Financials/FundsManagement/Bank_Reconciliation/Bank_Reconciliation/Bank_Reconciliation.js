...
Ext.onReady(function () {
        ...
        Ext.MessageBox.show({
            ...
            fn: function (btn) {
                if (btn === 'yes') {
                    ...
                    Ext.Ajax.request({
                        url: '/SHVPM/Financials/FundsManagement/Bank_Reconciliation/Bank_Reconciliation/Bank_Reconciliation_Save.php',
                        params: {
 ...
 var select_labelsum = new Ext.form.TextField({
        fieldLabel: '',
        id: 'select_labelsum',
        name: 'select_labelsum',
        width: 130,
        hidden: true,
        labelSeparator: "",
        labelStyle: 'font-size: 15px; color:red;'
...
Ext.MessageBox.alert("Alert",'Reverse option selected.');
		...
		Datagridd.getSelectionModel().selectRow();
                var selected_rows = Datagridd.getSelectionModel().getSelections();
                ...
                for (var k = 0; k < selected_rows.length; k++) {
                    db += parseFloat(selected_rows[k].data.acctran_dbamt);
                    cr += parseFloat(selected_rows[k].data.acctran_cramt);

                }
                summ = db - cr;
		select_labelsum.show();
                select_labelsum.setValue(summ);
//alert(parseFloat(select_labelsum.getValue()));
                Math.abs(summ);
            }
        }
