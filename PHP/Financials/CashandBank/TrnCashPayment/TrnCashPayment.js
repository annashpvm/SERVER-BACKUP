        ...
        listeners: {
            click: function () {
                ...
                if (cmbAccountName.getValue() == 0) {
                    ...
                    gstInsert = "false";
                    Ext.MessageBox.alert("Payment", "Select Currency");
                } else if (cmbCurrency.getRawValue() != "INR") {
                    if (txtAmt.getRawValue() == "") {
                        gstInsert = "false";
                        Ext.MessageBox.alert("Payment", "Enter the Amount");
                    }
                    if (txtExgRate.getRawValue() == "") {
                        gstInsert = "false";
                        Ext.MessageBox.alert("Payment", "Enter Exchange Rate");
                    }
                } else if (cmbType.getValue() == 2) {
                    if (txtDebit.getRawValue() == "") {
                        gstInsert = "false";
                        Ext.MessageBox.alert("Payment", "Enter Debit Amount");
                    }
                } else if (cmbType.getValue() == 1) {
                    if (txtCredit.getRawValue() == "") {
                        gstInsert = "false";
                        Ext.MessageBox.alert("Payment", "Enter Credit Amount");
                    }
                } else if ((Number(txtTotCredit) + Number(txtCredit)) > (Number(txtTotDebit) + Number(txtDebit))) {
                    gstInsert = "false";
                    Ext.MessageBox.alert("Payment", "Debit should be greater than credit");
                }
                ...
                if (cnt > 0) {
                    gstInsert = "false";
                    Ext.MessageBox.alert("Payment", "This Ledger Already Entered");
                }
...
*/
    ...
    var flxTransactionDetails = new Ext.grid.EditorGridPanel({
        ...
        columns: [
            ...
            {header: "Curseqno", dataIndex: 'curseq', sortable: true, width: 40, align: 'left', hidden: false},
    ...
    var btnSave = new Ext.Button({
        ...
        listeners: {
            click: function () {
                ...
                var fromdate;
		var crdr=Number(txtTotDebit.getRawValue()) - Number(txtTotCredit.getRawValue());
                var todate;
    ...
    function RefreshGridData() {
        ...
        cmbAccountName.setValue("");
        gstFlag = "Add";
        if (gstfinyear.substring(5, 9) === '2017') {
...
//                }
                ...
                if (gstfincompcode == "1") {
                    ...
                    cmbHeadAccount.setRawValue("CASH BOOK");
                    headcode =149;
                } else if (gstfincompcode == "3") {
                    lblCompany.setText("VJPM");
                    lblCompany.getEl().setStyle('color', 'white');
                    cmbHeadAccount.setRawValue("CASH BOOK");
                    headcode = 149;
                }
