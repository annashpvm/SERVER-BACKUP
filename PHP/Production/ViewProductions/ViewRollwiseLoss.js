...
//    alert(monthenddate);  
	...
	loadRollLossListDataStore.removeAll();
	loadRollLossListDataStore.load({
	 url: 'ClsViewStatements.php',
                params: {
	    	task: 'loadRollLossDetails',
...
['7','JULY'],['8','AUGUEST'],['9','SEPTEMBER'],[10,'OCTOBER'],['11','NOVEMBER'],['12','DECEMBER']],
 ...
 var loadRollLossListDataStore = new Ext.data.Store({
      id: 'loadRollLossListDataStore',
      proxy: new Ext.data.HttpProxy({
                url: 'ClsViewStatements.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadRollLossDetails"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
        id: 'id' 
      },['prd_date', 'prdh_date', 'prd_shift', 'prd_rollno', 'quality', 'prd_deckle', 'prd_rwdeck', 'lossdeck', 'loss_per' ]),
    });
...
var btnDatewise = new Ext.Button({
	...
	listeners:{
		...
		var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
                ...
                if (printtype == "PDF")
          	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_Rollwise_Deckle_Loss.rptdesign&__format=pdf&' + param, '_blank');
                else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_Rollwise_Deckle_Loss.rptdesign&__format=XLS' + param, '_blank');
                else
        	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_Rollwise_Deckle_Loss.rptdesign' + param, '_blank');
...
var flxProduction = new Ext.grid.EditorGridPanel({
    ...
    [ 	 	
        ...
        {header: "DATE"    ,  dataIndex: 'prdh_date',sortable:false,width:110,align:'left', menuDisabled: true},

        {header: "SHIFT"    , dataIndex: 'prd_shift',sortable:false,width:77,align:'center', menuDisabled: true},
        {header: "Roll No" , dataIndex: 'prd_rollno',sortable:false,width:77,align:'center', menuDisabled: true},
        {header: "Quality" , dataIndex: 'quality',sortable:false,width:150,align:'center', menuDisabled: true},
        {header: "M/C Deckle" , dataIndex: 'prd_deckle',sortable:false,width:77,align:'right', menuDisabled: true},
        {header: "RW Deckle" , dataIndex: 'prd_rwdeck',sortable:false,width:77,align:'right', menuDisabled: true},
        {header: "Deck.Loss" , dataIndex: 'lossdeck',sortable:false,width:77,align:'right', menuDisabled: true},
        {header: "Loss Per" , dataIndex: 'loss_per',sortable:false,width:77,align:'right', menuDisabled: true},

    ],
    store:loadRollLossListDataStore,
    listeners:{	
