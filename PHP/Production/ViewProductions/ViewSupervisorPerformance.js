...
var Rpttype="Date-Shift wise Report";
var optRpttype = new Ext.form.FieldSet({
    ...
    items: [
    {
        ...
        id: 'optRpttype',
        items: [
		{boxLabel: 'Date-Shift wise Report', name: 'optRpttype', id:'optDS', inputValue: 1,checked:true,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
						Rpttype="Date-Shift wise Report";

					}
				}
			}
		},
		{boxLabel: 'Shift Incharge-Datewise Report', name: 'optRpttype', id:'optPS', inputValue: 2,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
						Rpttype="Shift Incharge-Datewise Report";

					}
				}
			}
		},
		{boxLabel: 'Shift Incharge-GSMwise Report', name: 'optRpttype', id:'optqlywise', inputValue: 3,
			listeners:{
				check:function(rb,checked){
					if(checked==true){
						Rpttype="Shift Incharge-GSMwise Report";
...
['7','JULY'],['8','AUGUEST'],['9','SEPTEMBER'],[10,'OCTOBER'],['11','NOVEMBER'],['12','DECEMBER']],
 ...
 var loadMCProductionListDataStore = new Ext.data.Store({
      ...
      proxy: new Ext.data.HttpProxy({
            ...
            baseParams:{task:"loadPerformance"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
        id: 'id' 
      },['spvr_name', 'prdh_shift','prddate', 'prd_date', 'var_gsm', 'actprodn', 'runmins', 'breakmins', 'tspeed', 'tprdn_hr', 'tdeckle', 'tprdn_min', 'tar_prodn','runspeed', 'rundeckle' ]),
    });
...
var btnReport = new Ext.Button({
	...
	listeners:{
		...
		var p4 = "&todate=" + encodeURIComponent(Ext.util.Format.date(monthenddate.getValue(),"Y-m-d"));
               ...
               if (Rpttype == "Date-Shift wise Report") 
               {

                if (printtype == "PDF")
          	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_datewise_performance.rptdesign&__format=pdf&' + param, '_blank');
                else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_datewise_performance.rptdesign&__format=XLS' + param, '_blank');
                else
        	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_datewise_performance.rptdesign' + param, '_blank');
               }
               if (Rpttype == "Shift Incharge-Datewise Report") 
               {

                if (printtype == "PDF")
          	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_shiftincharge_performance_Datewise.rptdesign&__format=pdf&' + param, '_blank');
                else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_shiftincharge_performance_Datewise.rptdesign&__format=XLS' + param, '_blank');
                else
        	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_shiftincharge_performance_Datewise.rptdesign' + param, '_blank');
               }

               if (Rpttype == "Shift Incharge-GSMwise Report") 
               {

                if (printtype == "PDF")
          	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_shiftincharge_performance_GSMwise.rptdesign&__format=pdf&' + param, '_blank');
                else if (printtype == "XLS") 
		    window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_shiftincharge_performance_GSMwise.rptdesign&__format=XLS' + param, '_blank');
                else
        	    	window.open('http://10.0.0.251:8080/birt/frameset?__report=Production/rpt_shiftincharge_performance_GSMwise.rptdesign' + param, '_blank');
               }     
       	 }
        }   
});
...
var flxProduction = new Ext.grid.EditorGridPanel({
    ...
    [ 	 
        ...
        {header: "Shift Incharge"    , dataIndex: 'spvr_name',sortable:false,width:150,align:'left', menuDisabled: true},
        {header: "Shift" , dataIndex: 'prdh_shift',sortable:false,width:77,align:'center', menuDisabled: true},
	
        {header: "DATE"    ,  dataIndex: 'prddate',sortable:false,width:110,align:'left', menuDisabled: true },

        {header: "DATE"    ,  dataIndex: 'prd_date',sortable:false,width:110,align:'left', menuDisabled: true,hidden:true},
        {header: "GSM" , dataIndex: 'var_gsm',sortable:false,width:77,align:'center', menuDisabled: true},
        {header: "T.Speed" , dataIndex: 'tspeed',sortable:false,width:77,align:'center', menuDisabled: true},
        {header: "T.Prodn" , dataIndex: 'tar_prodn',sortable:false,width:77,align:'right', menuDisabled: true},
        {header: "Speed(Avg)" , dataIndex: 'runspeed',sortable:false,width:90,align:'center', menuDisabled: true},
        {header: "RunHrs" , dataIndex: 'runmins',sortable:false,width:77,align:'center', menuDisabled: true},
        {header: "Act.Prodn" , dataIndex: 'actprodn',sortable:false,width:90,align:'right', menuDisabled: true},
...
//view
	...
	x	    : 0,
        y           : 30,
        title       : 'Shift Incharge Performance',
