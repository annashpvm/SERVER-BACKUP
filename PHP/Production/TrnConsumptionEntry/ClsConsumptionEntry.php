...
function getEntryNo()
	...
	$finid = $_POST['fincode'];
        ...
        $sql = "select ifnull(max(iss_no),0) + 1 as issno from trn_dayprod_chemicals_cons where iss_comp_code=$compcode and iss_fin_code=$finid";
...
function getEntryNoDetails()
{
	...
	$issno    = $_POST['issno'];
        ...
        $sql = "select * from  trn_dayprod_chemicals_cons , maspur_item_header  , mas_uom  where item_uom = uom_code and iss_item_code = item_code and iss_comp_code = $compcode and iss_fin_code = $finid and iss_no = $issno order by iss_slno";
    $r = mysqli_query($conn, $sql);
...
if ($isstype == "IS")
{
        $sql = "select ind_no,ind_fin_code from trnpur_indent where ind_comp_code = $compcode and ind_fin_code = $finid and ind_dept_code = $deptcode  and ind_hod_auth = 'Y' and ind_bal_qty <> 0 group by ind_no,ind_fin_code order by ind_no desc";
}
else
{
        $sql = "select iss_indno as ind_no,iss_fin_code from trnpur_item_issues where iss_comp_code = $compcode and iss_fin_code = $finid and iss_dept_code = $deptcode group by iss_indno,iss_fin_code order by iss_indno desc";
}
    $r = mysqli_query($conn, $sql);
...
if ($isstype === "IS")
{
    if ($savetype == "Add") 
    {
        $sql = "select a.*,b.item_name as item_name,c.app_name as app_name,e.*,f.* ,g.*,item_avg_rate,item_stock                      
...
{
$sql = "select * from trnpur_item_issues  a, maspur_item_header b,mas_bud_section c , mas_equipment d , mas_uom e ,  maspur_item_trailer f where   iss_item_code = b.item_code and iss_comp_code = $compcode  and iss_fin_code =  $finid  and  iss_indno = $indno and iss_section = section_code and iss_equip = equip_code and item_uom = uom_code and iss_item_code = f.item_code and iss_comp_code = item_comp_code   and iss_fin_code = f.item_fin_code";
...
function getloadcategory()
	...
	$finid = $_POST['finid'];
	$deptcode = $_POST['deptcode'];*/
        $sql = "select cat_code,cat_name from maspur_category where cat_code in(2,3,4)";
    $r = mysqli_query($conn, $sql);
...
function getItemStock()
 ...
 function getSearchitemlist()
	...
	$compcode = $_POST['compcode'];
        ...
        $sql = "select * from maspur_item_header where item_group_code = 1 and  replace(replace(item_name,' ','')  ,'.','') like '%$item%'   order by item_name";
