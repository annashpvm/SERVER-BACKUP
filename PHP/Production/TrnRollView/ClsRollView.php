...
//	$sql = "select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end) ) as sizecode from massal_variety ,trnsal_order_trailer , masprd_variety where  var_grpcode = var_groupcode and var_grpcode = $varietycode and ordt_var_code = var_code and  ordt_comp_code = $compcode and ordt_sono = $sono order by sizecode");
   ...
   function getReelDetails()
	...
	$compcode = $_POST['compcode'];
        $rollno   = $_POST['rollno'];
	$yymm     = $_POST['yymm'];
	$sql = "select stk_sr_no,stk_wt,var_name , var_size2,var_inchcm, stk_ent_date from trnsal_finish_stock , massal_variety where stk_var_code = var_code and  stk_comp_code = '$compcode' and stk_finyear = '$finid' and stk_yymm = '$yymm' and stk_rollno = '$rollno' order by stk_sr_no");
	$nrow = mysqli_num_rows($r);
	while($re = mysqli_fetch_array($r))
	{
	$arr[]= $re ;
