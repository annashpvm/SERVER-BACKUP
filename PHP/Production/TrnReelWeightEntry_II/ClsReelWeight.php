...
//      $r=mysql_query("select ordh_sono from trnsal_order_header, trnsal_order_trailer,massal_variety,masprd_variety where ordh_comp_code = ordt_comp_code and  ordh_fincode  = ordt_fincode and ordh_sono = ordt_sono and  ordt_var_code = var_code and var_grpcode = var_groupcode and    ordh_comp_code = $compcode  and  ordh_fincode = $finid and var_grpcode = $varietycode group by ordh_sono order by ordh_sono desc");
 ...
 function getReelNumberDetail()
	...
	$reelno    = $_POST['reelno'];
        ...
        $r=mysql_query("select * , concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end) ) as sizecode from trnsal_finish_stock , massal_variety ,trnsal_order_header,massal_customer where stk_sono = ordh_sono and  ordh_party = cust_code and stk_var_code  = var_code and stk_sr_no = '$reelno'");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
for ($i ; $i < $sno+1; $i++)
{
	...
	$r=mysql_query("select * from tmp_reelno where reelno not in (select stk_sr_no from trnsal_finish_stock)");
		$nrow = mysql_num_rows($r);
		while($re = mysql_fetch_array($r))
		{
		$arr[]= $re ;
   ...
   function getReelNumbers_ofthe_Roll()
	...
	$rollno   = $_POST['rollno']; 
        ...
        $r=mysql_query("select stk_sr_no as reelno from trnsal_finish_stock WHERE   length(stk_sr_no) = 10 and  SUBSTR(stk_sr_no,6,3)  = $rollno and month(stk_ent_date) = '$mon' and year (stk_ent_date)= '$yr' ");
   ...
   function getFinishedDetails()
	...
	$r=mysql_query("select  var_grpcode,sum(stk_wt)/1000 as wt from trnsal_finish_stock , massal_variety where stk_var_code = var_code and SUBSTR(stk_sr_no,6,3)  = '$rollno' and SUBSTR(stk_sr_no,1,2)  = '$yr' and  SUBSTR(stk_sr_no,3,2) = '$mon' group by var_grpcode");
