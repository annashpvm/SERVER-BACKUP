...
if ( $weight > 0)
{
  ...
  if ($savetype == "Add")
  {
	   $query1 = "select count(*) as nos from trnsal_finish_stock WHERE stk_sr_no = $reelno";
	   ...
	   if ( $cnt1 == 0)
	   {
		$query2= "insert into trnsal_finish_stock  (stk_comp_code,stk_finyear,stk_ent_no,stk_ent_date,stk_var_code,stk_sr_no,stk_wt,stk_sono,stk_joints) VALUES ('$compcode','$fincode','100','$entrydate','$sizecode','$reelno','$weight',$sono,$joints)";
...
//	$query4= "update trn_dayprod_roll_details set prd_finprod = $finwt + ($weight/1000) , prd_roll_status = 'P' where prd_rollno = $rollno  and prd_date = '$pdate'  and prd_shift = '$shift' and prd_rollno = '$rollno'  and prd_variety = '$qlycode' and prd_seqno = '$seqno' ";
  ...
  {
		$query2= "update trnsal_finish_stock set stk_sono = '$sono' , stk_var_code = '$sizecode',stk_wt = '$weight',stk_joints = $joints where stk_sr_no =  '$reelno' and stk_comp_code = $compcode  and stk_finyear = $fincode";
		$result2=mysql_query($query2);
