...
$sql = "select prd_rollno  from trn_dayprod_roll_details where prd_compcode = '$compcode' and prd_fincode = '$finid'  and month(prd_date) = '$mon' and year(prd_date)= '$yr' and prd_roll_status = 'P'  group by prd_rollno  order by prd_rollno desc";
...
//	$sql = "select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,var_inchcm ) as sizecode from massal_variety ,masprd_variety where var_grpcode = var_groupcode and var_grpcode = '$varty'";
	...
	$sql = "select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end) ) as sizecode from massal_variety ,masprd_variety where var_grpcode = var_groupcode and var_grpcode = '$varty' order by sizecode ";
...
for ($i ; $i < $sno+1; $i++)
{
	...
	$sql = "select * from tmp_reelno where reelno not in (select stk_sr_no from trnsal_finish_stock where stk_comp_code = $compcode and stk_finyear = $finid)";
	    $r = mysqli_query($conn, $sql);
		$nrow = mysqli_num_rows($r);
		while($re = mysqli_fetch_array($r))
		{
   ...
   function getReelNumbers_ofthe_Roll()
	...
	$rollno   = $_POST['rollno']; 
        ...
        $sql = "select stk_sr_no as reelno from trnsal_finish_stock WHERE  stk_comp_code = $compcode and stk_finyear = $finid and  length(stk_sr_no) = 10 and  SUBSTR(stk_sr_no,6,3)  = $rollno and month(stk_ent_date) = '$mon' and year (stk_ent_date)= '$yr' ";

	
        $sql = "select stk_sr_no as reelno from trnsal_finish_stock WHERE   stk_comp_code = $compcode and stk_finyear = $finid and  length(stk_sr_no) = 10 and  SUBSTR(stk_sr_no,6,3)  = $rollno and  SUBSTR(stk_sr_no,3,2)  = '$mon' and  SUBSTR(stk_sr_no,1,2)  = '$yr' ";
		$r = mysqli_query($conn, $sql);
		$nrow = mysqli_num_rows($r);
		while($re = mysqli_fetch_array($r))
		{
   ...
   function getFinishedDetails()
	...
	$sql = "select  var_grpcode,sum(stk_wt)/1000 as wt from trnsal_finish_stock , massal_variety where stk_comp_code = $compcode and stk_finyear = $finid and stk_var_code = var_code and SUBSTR(stk_sr_no,6,3)  = '$rollno' and SUBSTR(stk_sr_no,1,2)  = '$yr' and  SUBSTR(stk_sr_no,3,2) = '$mon' group by var_grpcode";
   ...
   function getSOSizeDetails()
	...
	$sql = "select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end) ) as sizecode from massal_variety ,trnsal_order_trailer where  var_grpcode = $varietycode and var_code = ordt_var_code and ordt_var_code = var_code and  ordt_comp_code = $compcode and ordt_sono = $sono order by sizecode";

	$sql = "select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end),space(2) ,(case when var_shade = 'NS' then 'NAT' when var_shade = 'GY' then 'GYT' when var_shade = 'DP' then 'DP' when var_shade = 'SY' then 'SHYS' when var_shade = 'GB' then 'GB'   else 'OTH' end) ) as sizecode from massal_variety ,trnsal_order_trailer , masprd_variety where  var_grpcode = var_groupcode and var_grpcode = $varietycode and ordt_var_code = var_code and  ordt_comp_code = $compcode and ordt_sono = $sono order by sizecode";

	$sql = "select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end),space(2) ,var_shade ) as sizecode from massal_variety ,trnsal_order_trailer , masprd_variety where  var_grpcode = var_groupcode and var_grpcode = $varietycode and ordt_var_code = var_code and  ordt_comp_code = $compcode and ordt_sono = $sono order by sizecode";
   ...
   function getChangedVarietyDetails()
	...
	$sql = "select var_bf,var_gsm from masprd_variety where var_groupcode = '$varty'";
   ...
   function getSizeDetails()
	    {
			global $conn;
	$size    = $_POST['size'];
	$sql = "select * from massal_variety where var_code = '$size'";
	$nrow = mysqli_num_rows($r);
	while($re = mysqli_fetch_array($r))
	{
	$arr[]= $re ;
