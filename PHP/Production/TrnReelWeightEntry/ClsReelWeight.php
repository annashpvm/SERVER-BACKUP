...
for ($i ; $i < $sno+1; $i++)
{
   ...
   function getSOSizeDetails()
	...
	$r=mysql_query("select var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end),space(2) ,var_shade ) as sizecode from massal_variety ,trnsal_order_trailer , masprd_variety where  var_grpcode = var_groupcode and var_grpcode = $varietycode and ordt_var_code = var_code and  ordt_comp_code = $compcode and ordt_sono = $sono order by sizecode");
