...
var txtWTpass = new Ext.form.TextField({
        fieldLabel  : 'Manual Wt.Password',
        id          : 'txtWTpass',
        name        : 'txtWTpass',
        inputType: 'password',
        width       :  100,
	...
	listeners:{
        keyup: function(){
             if (txtWTpass.getRawValue() == "weightchange21")
             {    
...
var txtpass = new Ext.form.TextField({
	...
	listeners:{
        keyup: function(){
             if (txtpass.getRawValue() == "adminpass")
                btnChange.enable();
             else
                btnChange.disable();
...
//	autoLoad : true,
  ...
  function datecheck()
  {
        ...
        var dt_entdate = dtEntryDate.getValue();

        var diffdays = dt_today.getTime()-dt_entdate.getTime();
        diffdays = Math.ceil(diffdays / (1000 * 60 * 60 * 24)); 
        if (diffdays >2)
        {     
             alert("You are Not Allowed to enter data in the date of " +  Ext.util.Format.date(dt_entdate,"d-m-Y"));
             dtEntryDate.setRawValue(Ext.util.Format.date(dt_today,"d-m-Y"));

        }
        if (diffdays <= 0)
        {     
             alert("System will not allow to raise the entries in Future date");
             dtEntryDate.setRawValue(Ext.util.Format.date(dt_today,"d-m-Y"));
...
//	autoLoad : true,
 ...
 var cmbCustomerName = new Ext.form.ComboBox({
	...
	listeners:{
      ...
      var txtProdYear = new Ext.form.NumberField({
   	fieldLabel  :'Fin year',
   	id	    :'txtProdYear',
   	name	    :'txtProdYear',
   	width	    : 40,
        ...
        listeners:{
          specialkey:function(f,e){
			...
			{ 
                             if (destag == "")
                             {
                                if (txtWTpass.getRawValue() == "weight")  
                                save_data();
                             } 
                  }
             }
...
//   style      : "'color' : 'red','font-size' : '15px','font-weight' : 'bold'",
   ...
   listeners:{
	   ...
	   if(txtRollNoFrom.getRawValue()=="" || txtRollNoFrom.getValue()==0)
           {
			   ...
			   {         
			      if (btn == 'yes')
					...
					fincode  : GinFinid,
                                        ...
                                        rollnofrom  : txtRollNoFrom.getValue(),                                  
                                        rollnoto    : txtRollNoTo.getValue(),                                  
                                        yymm        : txtProdMon2.getValue(),    
						    ...
						    if (obj['success']==="true")
						    {                                
							   Ext.MessageBox.alert("Date Changed in Rolls -" + obj['msg']);
		                                     }else
						     {
...
//	disabled	: true,
	...
	listeners:{
		...
		{
                 gstFlag = "Add";
                 txtRollNo.setValue(cmbRollNo.getValue());
                 getvariety_details();
                 lblVariety.update('');
                 cmbSONo.clearValue();         
                 cmbSize.clearValue();         
...
//	disabled	: true,
 ...
 listeners:{
			...
			params: {
				...
				{
                                 dtEntryDate.disable();
                                 dtEntryDate.setRawValue(Ext.util.Format.date(loadViewReelNo.getAt(0).get('stk_ent_date'),'d-m-Y'));
				 ...
				 txtOldWeight.setValue(loadViewReelNo.getAt(0).get('stk_wt'));
                                 ...
                                 oldsono = loadViewReelNo.getAt(0).get('stk_sono');
                                 oldsize = loadViewReelNo.getAt(0).get('stk_var_code');
		...
		params: {
		   ...
		   finid    : GinFinid,   
	           mon      : mm, //Ext.util.Format.date(dtProdDate.getValue(),"n"),   
                   yr       : "20"+yy, //Ext.util.Format.date(dtProdDate.getValue(),"Y"),     
		 },
		 callback:function()
		   {
...
//alert(gstFlag);
        ...
        items:[
    		...
    		{
    		xtype	:'fieldset',
    		title	:'Date Change in Finished Production',
                id      :'btnchg',
    		width	:700,
    		height	:90,
    		x	:600,       
...
onEsc:function(){
 	...
 	listeners:{
               ...
               Ext.getCmp('btnchg').setDisabled(true);  
