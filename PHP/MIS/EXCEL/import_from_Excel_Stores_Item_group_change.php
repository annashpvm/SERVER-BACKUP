 ...
 $filename=$_FILES["file"]["tmp_name"];
	...
	$cnnt=0;
        while (($packData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            $colE = intval($packData[4]);  // grp code
           ...
           if ($colA != '')
           {
           $itemcode = $pdo->query("select item_code as itemcode from maspur_item_header where replace(REPLACE(item_name, ' ',''),'  ','')  = '$colA'")->fetchColumn();
...
//echo "<br>";
            ...
            if ($itemcode > 0 ) 
            {
        $qry = "update  maspur_item_header set item_group_code  = $colE  WHERE item_code = '$itemcode' ";
