        ...
        while (($packData = fgetcsv($file, 10000, ",")) !== FALSE) {
           ...
           $colA =  date('Y-m-d', strtotime($colA));
            ...
            if ($colB == 'BHOMIAJI UNIQUE PACKAGING PVT LTD')
              ...
              if ($colB == 'LEO PRINT & PACK (SVK)')
                 $ledgercode = 2604; 
              else
              if ($colB == 'THE SIRPUR PAPER MILLS LIMITED (J K GROUP)')
                 $ledgercode = 2191; 
              else
              if ($colB == 'SAIRAM PACKAGINGS')
                 $ledgercode = 2202; 
              else
              if ($colB == 'SUNDARAVEL MATCH INDUSTRIES PRIVATE LIMITED')
                 $ledgercode = 2715; 
              else
              if ($colB == 'S.J PAPERS AND BOARDS')
                 $ledgercode = 2738; 
...
//echo "<br>";   
	 ...
	 $insert_ref = $pdo->prepare("insert into acc_ref values ( $ginaccrefseq,'$ginvouno',$compcode,'$finid','$colA', 'GJV','', '', '', '$colA','')");
...
//echo "<br>"; 
         ...
         $sno = $sno+1;
	 $insert_tran = $pdo->prepare("insert into acc_tran values ( $ginaccrefseq,'$sno',$ledcode,'$colG','$colH', '$totamt','GJV')");
		...
		<div class="row">
			...
			<div class="span6" id="form-login">
				<form class="form-horizontal well"  action="" method="POST" name="upload_excel" enctype="multipart/form-data">
					<fieldset>
						<center><legend>Excel Upload - for Journals </legend></center>
