...
//echo $colC;
        ...
        if (trim($colF) == 'Payment'  && trim($colF_Actual) == 'Payment'  ) 
        {
                ...
                $sno = 0;
		$query1       = "select ifnull(max(accref_seqno),0) + 1 as con_value from tmp_acc_ref;";
		$result1      = mysql_query($query1);
		$rec1         = mysql_fetch_array($result1);
		$ginaccrefseq = $rec1['con_value'];


                $query2   = "select ifnull(max(convert(substring(accref_vouno,4),signed)),0) +1 as vou_no from tmp_acc_ref where accref_vou_type = '$voutype' and accref_finid = '$finid' and accref_comp_code = '$compcode';";
	 ...
	 $insert_ref = $pdo->prepare("insert into tmp_acc_ref values ( $ginaccrefseq,'$ginvouno',$compcode,'$finid','$colA', '$voutype','', '', '', '$colA','',1,1)");
...
echo $cashamt;
          $sno = $sno+1;
	 $insert_tran = $pdo->prepare("insert into tmp_acc_tran values ( $accseq,'$sno',2139,'0','$cashamt', '$cashamt','$voutype')");
...
//         $insert_tran->bindParam(':columnI', $packData[8]);  //NAME
	 ...
	 if ($insert_tran->execute()) {
        ...
        {
          $sno = $sno+1;
	 $insert_tran = $pdo->prepare("insert into tmp_acc_tran values ( $accseq,'$sno',2139,'$cashamt','0', '$cashamt','$voutype')");
	 ...
	 if ($insert_tran->execute()) {
       ...
       $cr = 0;
        ...
        if ($colB == 'By')
         ...
         $sno = $sno+1;
//	 $insert_tran = $pdo->prepare("insert into tmp_acc_tran values ( $ginaccrefseq,'$sno',$ledcode,'$colI','$colH', '$totamt','$voutype')");

	 $insert_tran = $pdo->prepare("insert into tmp_acc_tran values ( $ginaccrefseq,'$sno',$ledcode,'$dr','$cr', '$totamt','$voutype')");
	 ...
	 if ($insert_tran->execute()) {
        ...
        if (trim($colF_Actual) == 'Payment') 
        {
          $sno = $sno+1;
	 $insert_tran = $pdo->prepare("insert into tmp_acc_tran values ( $ginaccrefseq,'$sno',2139,'0','$totamt', '$totamt','$voutype')");
	 ...
	 if ($insert_tran->execute()) {
        ...
        {
          $sno = $sno+1;
	 $insert_tran = $pdo->prepare("insert into tmp_acc_tran values ( $ginaccrefseq,'$sno',2139,'$totamt','0', '$totamt','$voutype')");
	 ...
	 if ($insert_tran->execute()) {
         ...
         {
		 $insert_trail = $pdo->prepare("insert into tmp_acc_trail values ( $ginaccrefseq,$sno,'$ginvouno','$colA','$totamt',0,'$ledcode','$drcr','0' )");
		...
		<div class="row">
			...
			<div class="span6" id="form-login">
				<form class="form-horizontal well"  action="" method="POST" name="upload_excel" enctype="multipart/form-data">
					<fieldset>
						<center><legend> CASH VOUCHERS - Excel Upload</legend></center>
