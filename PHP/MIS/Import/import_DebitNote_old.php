	...
	$cnnt=0;
        while (($packData = fgetcsv($file, 10000, ",")) !== FALSE) {
           ...
           if ($packData[5] != '')      // VOUCHER NUMBER   
            ...
            if  (substr($colF,0,4) == 'DB/G' || substr($colF,0,3) == 'SDN' )
                $voutype = "DNG";
            else
                $voutype = "DNN";
...
//echo $colC;
        ...
        if ($colE == 'Debit Note'   && trim($colE_Actual) == 'Debit Note'  ) 
        {
                $sno = 0;
...
// For voucher Number
	 ...
	 $insert_ref = $pdo->prepare("insert into acc_ref values ( $ginaccrefseq,'$ginvouno',$compcode,'$finid','$colA', '$voutype','', '', '', '$colA','',1,1)");
...
//echo "<br>"; 
	 ...
	 if ($insert_tran->execute()) {
         ...
         {
	 $insert_trail = $pdo->prepare("insert into acc_trail values ( $ginaccrefseq,$sno,'$ginvouno','$colA','$totamt',0,'$ledcode','$drcr','0' )");
