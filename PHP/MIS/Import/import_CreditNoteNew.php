 ...
 $filename=$_FILES["file"]["tmp_name"];
	...
	$cnnt=0;
        ...
        while (($packData = fgetcsv($file, 10000, ",")) !== FALSE) {
           ...
           $colE = 0; 
           if ($packData[4] != '')      //  CREDIT NOTE AMOUNT  
            { 
              $colE = $packData[4];
            }
...
//		$query1 = "select count(*) as nos from acc_ref where accref_comp_code = $compcode  and accref_finid =23 and accref_vouno = '$colK_Actual'";
		...
		$query1 = "select count(*) as nos from tmp_import_cndn_purchase where compcode = $compcode  and fincode = 23 and voutype = '$colJ' and vouno = '$colK'";
...
//echo "<br>"; 
        ...
        if ($recfound == 0)     
        { 
        if ($colJ == 'Credit Note'   && trim($colJ_Actual) == 'Credit Note'  ) 
        {
                $sno = 0;
...
//for updating  --  tmp_import_cndn_purchase 
        ...
        while (($packData2 = fgetcsv($file2, 10000, ",")) !== FALSE) {
           ...
           if ($packData2[9] != '')      // CREDIT NOTE WORD
            { 
              $colJ = $packData2[9];
            }
           ...
           $colE = 0; 
           if ($packData2[4] != '')      //  CREDIT NOTE AMOUNT  
            { 
              $colE = $packData2[4];
            }
...
//echo "<br>"; 
            ...
            if ( $colK_Actual != '')
            {  
		$query1 = "select count(*) as nos from tmp_import_cndn_purchase where compcode = $compcode  and fincode = 23 and voutype = '$colJ' and vouno = '$colK_Actual'";
//echo $query1;
//echo "<br>"; 
		...
		$recfound = $rec1['nos'];
                if ($recfound == 0)     
                { 
	$query123  = "insert into tmp_import_cndn_purchase value($compcode,$finid,'$colJ','$colK_Actual','$colA','$colB')";
		...
		<div class="row">
			...
			<div class="span6" id="form-login">
				<form class="form-horizontal well"  action="" method="POST" name="upload_excel" enctype="multipart/form-data">
					<fieldset>
						<center><legend>Excel Upload  - CREDIT NOTE</legend></center>
