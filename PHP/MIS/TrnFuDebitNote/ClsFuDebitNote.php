 ...
 function getSearchLedgerlist()
    {
        ...
        if ($ledname == '')
	        $qry = "select * from (select  led_code,led_name,qc_fuel_supcode,sup_type from trn_qc_fuel_inspection,maspur_supplier_master, acc_ledger_master  where  sup_code = qc_fuel_supcode and  sup_led_code = led_code group by led_code,led_name,qc_fuel_supcode,sup_type) a1 order by led_name";
        else
	        $qry = "select * from (select  led_code,led_name,qc_fuel_supcode,sup_type from trn_qc_fuel_inspection,maspur_supplier_master, acc_ledger_master  where  sup_code = qc_fuel_supcode and  sup_led_code = led_code group by led_code,led_name,qc_fuel_supcode,sup_type) a1 where replace(replace(led_name,' ','')  ,'.','')  like '%$ledname%'  order by led_name";
...
//        $r=mysql_query("select * from acc_ledger_master  where led_type = 'G' and  led_code in (1756,1745,1746,2258)");
        ...
        $r=mysql_query("select qc_fuel_entryno from trn_qc_fuel_inspection where qc_fuel_compcode = $compcode and qc_fuel_dn_raised ='N' and qc_fuel_supcode = $suppcode and qc_fuel_tot_ded_qty > 0 order by  qc_fuel_entryno desc");
	$nrow = mysql_num_rows($r);
	while($re = mysql_fetch_array($r))
	{
	$arr[]= $re ;
...
*/
 ...
 function getDebitNoteVoucherList()
	...
	$finid    = $_POST['fincode'];
        $gsttype =$_POST['gsttype'];
        if ($gsttype == 'G')
        $r=mysql_query("select max(dbcr_vouno) as vouno from tmpacc_dbcrnote_header where dbcr_comp_code = '$compcode' and dbcr_finid = '$finid' and dbcr_type = 'DNG' and  dbcr_vouno in (select qc_fuel_debitnote_no  from trn_qc_fuel_inspection where qc_fuel_compcode = '$compcode' and qc_fuel_fincode = '$finid' ) ");
        else
        $r=mysql_query("select max(dbcr_vouno) as vouno from tmpacc_dbcrnote_header where dbcr_comp_code = '$compcode' and dbcr_finid = '$finid' and dbcr_type = 'DNN' and  dbcr_vouno in (select qc_fuel_debitnote_no  from trn_qc_fuel_inspection where qc_fuel_compcode = '$compcode' and qc_fuel_fincode = '$finid' ) ");
 ...
 function getDebitNoteVoucherListAccounts()
	...
	$finid    = $_POST['fincode'];
        $gsttype =$_POST['gsttype'];
        $r=mysql_query("select dbcr_vouno from  tmpacc_dbcrnote_header where dbcr_comp_code = '$compcode' and dbcr_finid ='$finid'  and dbcr_accseqno = 0  order by dbcr_vouno asc ");
...
qc_fuel_debitnote_no = dbcr_vouno and dbcr_comp_code = '$compcode' and dbcr_finid = '$finid' and dbcr_accseqno = 0  order by dbcr_vouno asc");
 ...
 function getDebitNoteUpdateCheck()
	...
	$vouno    = $_POST['vouno'];
        ...
        $r=mysql_query("select * from  tmpacc_dbcrnote_header where dbcr_comp_code = '$compcode' and dbcr_finid ='$finid'  and dbcr_vouno = '$vouno'");
