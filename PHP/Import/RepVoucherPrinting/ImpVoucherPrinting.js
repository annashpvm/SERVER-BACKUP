Ext.onReady(function(){
 ...
 var loadMonthInvoiceListDataStore = new Ext.data.Store({
      id: 'loadMonthInvoiceListDataStore',
      proxy: new Ext.data.HttpProxy({
                url: '/SHVPM/Import/ClsImportRep.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"loadMonthInvoiceList"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
        id: 'id' 
      },[ 'invh_invoicerefno', 'invh_date', 'sup_refname', 'itmh_name', 'invt_qty', 'invt_recqty', 'invt_itemUSDrate', 'invt_exrate', 'invt_itemvalue' 
 ...
 var loadMonthPOData = new Ext.data.Store({
      ...
      reader: new Ext.data.JsonReader({
        ...
        totalProperty: 'total',
        id: 'id' 
      },[ 'ordh_no' , 'ordh_date','sup_refname','itmh_name','ordt_qty','ordt_usdrate','ordt_usdvalue' 
 ...
 var loadMonthInvoiceDataStore = new Ext.data.Store({
      id: 'loadMonthInvoiceDataStore',
      proxy: new Ext.data.HttpProxy({
                url: '/SHVPM/Import/ClsImportRep.php',      // File to connect to
                method: 'POST'
            }),
            baseParams:{task:"getMonthInvoices"}, // this parameter asks for listing
      reader: new Ext.data.JsonReader({
                  // we tell the datastore where to get his data from
        root: 'results',
        totalProperty: 'total',
...
var optprinttype = new Ext.form.FieldSet({
 ...
 var cmbReport = new Ext.form.ComboBox({
	...
	emptyText       : '--Select Report Type--',
        mode            : 'local',
        store           : [['1','CONTRACT DETAILS'],['2','INVOICES DETAILS']],
        forceSelection  : true,
        triggerAction   : 'all',
        selectOnFocus   : false, 
        editable        : false,
...
//alert(cmbReport.getValue());
	        ...
	        if (cmbReport.getValue() == "1") {
			    ...
			    loadPOData.load({
				url: '/SHVPM/Import/ClsImportRep.php',
				params: {
			    	task: 'loadMonthPOs',
		...
		//                       grid_tot();
                          ...
                          Ext.getCmp('flxInvoice').show();  

			    loadMonthInvoiceDataStore.removeAll();
			    loadMonthInvoiceDataStore.load({
				url: '/SHVPM/Import/ClsImportRep.php',
				params: {
			    	task: 'loadMonthInvoices',
				compcode:GinCompcode,
				finid:GinFinid,
				startdate: Ext.util.Format.date(finstartdate,"Y-m-d"),  
				},
				...
				callback:function()
				{
				   var cnt=loadMonthInvoiceDataStore.getCount();
				   if(cnt>0)
				   {
				       flxMonth.getSelectionModel().selectAll();
				       var selrows = flxMonth.getSelectionModel().getCount();
				       ...
				       {  
		   		       for (var i=0;i<selrows;i++){    
			    		    ...
			    		    if (sel[i].data.rmonth === loadMonthInvoiceDataStore.getAt(j).get('rmonth'))
			      		    {
						sel[i].set('nos', Ext.util.Format.number(loadMonthInvoiceDataStore.getAt(j).get('nos'),'0'));
						sel[i].set('purvalue', Ext.util.Format.number(loadMonthInvoiceDataStore.getAt(j).get('purvalue'),'0.00'));
					    }
					}
					}
		//                       grid_tot();
...
function find_dates(mmon)
{
    ...
    monthenddate = yr+"-"+rmon+"-"+mdays;
            ...
            loadMonthInvoiceListDataStore.removeAll();
     ...
     if (cmbReport.getValue() == "1") {      
	    ...
	    loadMonthInvoiceListDataStore.load({
		url: '/SHVPM/Import/ClsImportRep.php',
		params: {
	    	task: 'loadMonthInvoiceList',
		compcode:GinCompcode,
		finid:GinFinid,
		startdate: Ext.util.Format.date(monthstartdate,"Y-m-d"),  
		enddate: Ext.util.Format.date(monthenddate,"Y-m-d"),  
...
var flxPO = new Ext.grid.EditorGridPanel({
    ...
    columns:
    [ 	 	
        {header: "Contract.No" , dataIndex: 'ordh_no',sortable:false,width:60,align:'left', menuDisabled: true},
        {header: "Date" , dataIndex: 'ordh_date',sortable:false,width:80,align:'left', menuDisabled: true},
        {header: "Supplier" , dataIndex: 'sup_refname',sortable:false,width:160,align:'left', menuDisabled: true},
        ...
        {header: "Rate" , dataIndex: 'ordt_usdrate',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "PO Amount" , dataIndex: 'ordt_usdvalue',sortable:false,width:100,align:'left', menuDisabled: true},
    ...
    listeners:{	
		...
		var param = (p1+p2+p3) ;   
                if (printtype == "PDF")                       
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Import/RepImp_Contract.rptdesign&__format=pdf&' + param); 
                else

		window.open('http://10.0.0.251:8080/birt/frameset?__report=Import/RepImp_Contract.rptdesign' + param); 
...
var flxInvoice = new Ext.grid.EditorGridPanel({
    ...
    columns:
    [ 	 	
        {header: "Inv.No" , dataIndex: 'invh_invoicerefno',sortable:false,width:150,align:'left', menuDisabled: true},
        {header: "Date" , dataIndex: 'invh_date',sortable:false,width:80,align:'left', menuDisabled: true},
        {header: "Supplier" , dataIndex: 'sup_refname',sortable:false,width:160,align:'left', menuDisabled: true},
        {header: "Item" , dataIndex: 'itmh_name',sortable:false,width:200,align:'left', menuDisabled: true},
        {header: "Inv. Qty" , dataIndex: 'invt_qty',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "Recd. Qty" , dataIndex: 'invt_recqty',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "RATE" , dataIndex: 'invt_itemUSDrate',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "USD/INR" , dataIndex: 'invt_exrate',sortable:false,width:100,align:'left', menuDisabled: true},
        {header: "INV Amount" , dataIndex: 'invt_itemvalue',sortable:false,width:100,align:'left', menuDisabled: true},

    ],

     store: loadMonthInvoiceListDataStore,
    listeners:{	

            'cellclick': function (flxInvoice, rowIndex, cellIndex, e) {
                  var sm = flxInvoice.getSelectionModel();
		  var selrow = sm.getSelected();
                  var invno = selrow.get('invh_invoicerefno')
		...
		var param = (p1+p2+p3) ;   
                if (printtype == "PDF")                       
		window.open('http://10.0.0.251:8080/birt/frameset?__report=Import/RepImp_InvoiceDetails.rptdesign&__format=pdf&' + param); 
                else

		window.open('http://10.0.0.251:8080/birt/frameset?__report=Import/RepImp_InvoiceDetails.rptdesign' + param); 
